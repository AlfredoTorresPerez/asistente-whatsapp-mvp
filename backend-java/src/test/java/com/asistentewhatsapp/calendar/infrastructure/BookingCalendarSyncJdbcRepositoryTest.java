...
class BookingCalendarSyncJdbcRepositoryTest {
    ...
    void updateSyncSuccessSetsStatusToSynced() {
        ...
        ArgumentCaptor<String> sql = ArgumentCaptor.forClass(String.class);
        verify(jdbcTemplate).update(sql.capture(), any(MapSqlParameterSource.class));
        assertThat(sql.getValue()).contains("sync_status = 'SYNCED'")
    ...
    void updateSyncFailedIncrementsRetryCount() {
        ...
        ArgumentCaptor<String> sql = ArgumentCaptor.forClass(String.class);
        verify(jdbcTemplate).update(sql.capture(), any(MapSqlParameterSource.class));
        assertThat(sql.getValue()).contains("sync_status = 'FAILED'")
    ...
    void findByBookingReturnsResults() {
        ...
        List<BookingCalendarSyncJdbcRepository.BookingCalendarSyncRecord> results = repository.findByBooking(BOOKING_ID);
    ...
    void findFailedSyncsFiltersByMaxRetriesAndBefore() {
        ...
        List<BookingCalendarSyncJdbcRepository.BookingCalendarSyncRecord> results =
                repository.findFailedSyncs(5, before);
        assertThat(results).hasSize(1);
    ...
    private BookingCalendarSyncJdbcRepository.BookingCalendarSyncRecord record(
            String status, String action, String externalId, int retryCount) {
        return new BookingCalendarSyncJdbcRepository.BookingCalendarSyncRecord(
                SYNC_ID, BOOKING_ID, BUSINESS_ID, PROVIDER,
                externalId, status, action, null,
                retryCount, OffsetDateTime.now(), null,
