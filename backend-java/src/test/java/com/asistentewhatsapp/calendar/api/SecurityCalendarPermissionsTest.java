...
class SecurityCalendarPermissionsTest {
    ...
    void withCalendarConfigViewCanAccessStatus() throws Exception {
        AuthenticatedUser user = new AuthenticatedUser(
                UUID.randomUUID(), UUID.randomUUID(), "Test", "Test", "User",
                "test@demo.cl", "America/Santiago",
                List.of("OWNER"),
                List.of("CALENDAR_CONFIG_VIEW"));
    ...
    void withoutCalendarConfigManageGets403OnDisconnect() throws Exception {
        mockMvc.perform(delete("/api/v1/calendar-integrations/{id}", ACCOUNT_ID))
                .andExpect(status().isForbidden());
    }
    ...
    void withoutCalendarConfigManageGets403OnSelectCalendar() throws Exception {
        mockMvc.perform(post("/api/v1/calendar-integrations/{id}/select-calendar", ACCOUNT_ID)
                        .param("calendarId", "c1")
                        .param("calendarSummary", "Cal"))
                .andExpect(status().isForbidden());
    }
    ...
    void withoutBookingsUpdateGets403OnSyncStatus() throws Exception {
        mockMvc.perform(get("/api/v1/bookings/{id}/calendar-sync", BOOKING_ID))
                .andExpect(status().isForbidden());
    }
