...
class CalendarIntegrationControllerTest {
	...
	private void authenticateAsOwner() {
		AuthenticatedUser user = new AuthenticatedUser(UUID.randomUUID(), BUSINESS_ID, "Test", "Test", "User",
				"test@demo.cl", "America/Santiago", List.of("OWNER"),
	...
	void getStatusReturnsAccounts() throws Exception {
		authenticateAsOwner();
		when(integrationService.getStatus(any())).thenReturn(List.of(new CalendarAccountResponse(ACCOUNT_ID, "GOOGLE",
				"te***@demo.cl", "primary", "Calendar", true, OffsetDateTime.now(), null, false, null, "CONNECTED")));
	...
	void connectGoogleRedirectsToAuthUrl() throws Exception {
		...
		mockMvc.perform(get("/api/v1/calendar-integrations/google/connect")).andExpect(status().is3xxRedirection())
	...
	void googleCallbackWithAccessDeniedRedirectsToDenied() throws Exception {
		mockMvc.perform(get("/api/v1/calendar-integrations/google/callback").param("state", "s").param("code", "c")
				.param("error", "access_denied")).andExpect(status().is3xxRedirection())
				.andExpect(header().string("Location", "http://localhost:5173/configuration?calendar=denied"));
	}
	...
	void googleCallbackWithValidStateAndCodeRedirectsToConnected() throws Exception {
		when(integrationService.handleOAuthCallback(eq("valid-state"), eq("valid-code")))
				.thenReturn(new CalendarAccountResponse(ACCOUNT_ID, "GOOGLE", "te***@demo.cl", "primary", "Calendar",
						true, OffsetDateTime.now(), null, false, null, "CONNECTED"));

		mockMvc.perform(get("/api/v1/calendar-integrations/google/callback").param("state", "valid-state").param("code",
				"valid-code")).andExpect(status().is3xxRedirection())
				.andExpect(header().string("Location", "http://localhost:5173/configuration?calendar=connected"));
	}
	...
	void googleCallbackWithErrorRedirectsToError() throws Exception {
		...
		mockMvc.perform(get("/api/v1/calendar-integrations/google/callback").param("state", "bad").param("code", "bad"))
				.andExpect(status().is3xxRedirection())
				.andExpect(header().string("Location", "http://localhost:5173/configuration?calendar=error"));
	}
	...
	void disconnectDeletesAccount() throws Exception {
		authenticateAsOwner();
		mockMvc.perform(delete("/api/v1/calendar-integrations/{accountId}", ACCOUNT_ID)).andExpect(status().isOk())
	...
	void selectCalendarSelectsCalendar() throws Exception {
		...
		mockMvc.perform(post("/api/v1/calendar-integrations/{accountId}/select-calendar", ACCOUNT_ID)
				.param("calendarId", "cal-1").param("calendarSummary", "My Calendar")).andExpect(status().isOk())
	...
	void listCalendarsReturnsList() throws Exception {
		...
		mockMvc.perform(get("/api/v1/calendar-integrations/{accountId}/calendars", ACCOUNT_ID))
				.andExpect(status().isOk()).andExpect(jsonPath("$[0].id").value("cal-1"))
	...
	void getSyncStatusReturnsStatus() throws Exception {
		authenticateAsOwner();
		mockMvc.perform(get("/api/v1/bookings/{bookingId}/calendar-sync", BOOKING_ID)).andExpect(status().isOk());
	...
	void retrySyncRetriesSync() throws Exception {
		authenticateAsOwner();
		mockMvc.perform(post("/api/v1/bookings/{bookingId}/calendar-sync/retry", BOOKING_ID)).andExpect(status().isOk())
