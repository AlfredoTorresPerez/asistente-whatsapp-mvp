...
class CalendarSyncServiceTest {
    ...
    void syncCancelledWithNullExternalIdDoesNotCallProvider() {
        mockEventData();
        BookingCalendarSyncRecord synced = new BookingCalendarSyncRecord(
                UUID.randomUUID(), BOOKING_ID, BUSINESS_ID, PROVIDER_NAME,
                null, "SYNCED", "DELETE", null,
    ...
    void processPendingSyncsDeleteNotFoundMarksCancelled() {
        ...
        when(accountRepository.findByBusinessAndProvider(BUSINESS_ID, PROVIDER_NAME))
                .thenReturn(Optional.of(activeAccount()));
        doThrow(new GoogleCalendarApiException(404, "Not found", null))
                .when(provider).deleteEvent(eq(CALENDAR_ID), eq("ext_event_1"), any());
    ...
    void processPendingSyncsAuthErrorRefreshesToken() {
        ...
        when(provider.refreshAccessToken(anyString()))
                .thenReturn(new RefreshResult("new_access", 3600L));
        when(accountRepository.findByIdAndBusiness(any(), any()))
                .thenReturn(Optional.of(account));
    ...
    void multipleProvidersAreBothProcessed() {
        ...
        CalendarIntegrationAccountRecord outlookAccount = new CalendarIntegrationAccountRecord(
                UUID.randomUUID(), BUSINESS_ID, "OUTLOOK", "outlook@demo.cl",
                "enc_access", "enc_refresh",
                OffsetDateTime.now().plusHours(1), "outlook_cal", "Outlook Calendar",
                true, OffsetDateTime.now(), OffsetDateTime.now(), OffsetDateTime.now(),
                null, OffsetDateTime.now(), false);

        BookingCalendarSyncRecord pendingGoogle = pendingRecord("CREATE");
        BookingCalendarSyncRecord pendingOutlook = new BookingCalendarSyncRecord(
                UUID.randomUUID(), BOOKING_ID, BUSINESS_ID, "OUTLOOK",
                null, "PENDING", "CREATE", null, 0, null, null,
                OffsetDateTime.now(), OffsetDateTime.now());
    ...
    void retrySyncRetriesFailedRecords() {
        ...
        when(syncRepository.findByBookingAndBusiness(BOOKING_ID, BUSINESS_ID))
    ...
    private CalendarIntegrationAccountRecord activeAccount() {
        return new CalendarIntegrationAccountRecord(
                ...
                OffsetDateTime.now().plusHours(1), CALENDAR_ID, "Calendar",
                true, OffsetDateTime.now(), OffsetDateTime.now(), OffsetDateTime.now(),
                null, OffsetDateTime.now(), false);
    }
    ...
    private CalendarIntegrationAccountRecord activeAccountWithRefresh() {
        return new CalendarIntegrationAccountRecord(
                UUID.randomUUID(), BUSINESS_ID, PROVIDER_NAME, "test@demo.cl",
                "encrypted_access", "encrypted_refresh",
                OffsetDateTime.now().minusHours(1), CALENDAR_ID, "Calendar",
                true, OffsetDateTime.now(), OffsetDateTime.now(), OffsetDateTime.now(),
                null, OffsetDateTime.now(), false);
    }
    ...
    private BookingCalendarSyncRecord pendingRecord(String action, String externalEventId) {
        return new BookingCalendarSyncRecord(
                UUID.randomUUID(), BOOKING_ID, BUSINESS_ID, PROVIDER_NAME,
                externalEventId, "PENDING", action, null,
                0, null, null,
                OffsetDateTime.now(), OffsetDateTime.now());
    }
