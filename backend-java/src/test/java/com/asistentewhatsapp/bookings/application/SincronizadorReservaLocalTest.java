...
class SincronizadorReservaLocalTest {
    ...
    void sincronizarReservaUpsertsFactAndUpdatesBookingSyncStatus() {
        sync.sincronizarReserva(
                businessId, bookingId, "56950954580", "Cliente Test",
                "Limpieza facial", "Providencia", "María",
                startsAt, 60, "CONFIRMED", conversationId,
                "WHATSAPP_AI", "reservar", "trace-123");

        verify(repository).upsertBookingFact(
                eq(bookingId), eq(businessId), eq("56950954580"), eq("Cliente Test"),
                eq("56950954580"), eq("Limpieza facial"), eq("Providencia"), eq("María"),
    ...
    void sincronizarReservaHandlesNullConversationId() {
        sync.sincronizarReserva(
                ...
                "PUBLIC_LINK", "reservar", "trace-789");

        verify(repository).upsertBookingFact(
                eq(bookingId), eq(businessId), any(), eq("Cliente"),
                any(), eq("Servicio"), eq("Sucursal"), eq("Profesional"),
                any(), any(), eq("CONFIRMED"), eq(null),
                eq("PUBLIC_LINK"), eq("reservar"), eq(startsAt));
    }
}
