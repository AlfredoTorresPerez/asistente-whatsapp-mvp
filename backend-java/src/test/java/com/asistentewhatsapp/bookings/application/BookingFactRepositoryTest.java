...
class BookingFactRepositoryTest {
	...
	void hasActiveBookingReturnsTrueWhenExists() {
		when(jdbcTemplate.queryForObject(anyString(), eq(Boolean.class), any(), any(), any())).thenReturn(true);
	...
	void hasActiveBookingReturnsFalseWhenNotExists() {
		when(jdbcTemplate.queryForObject(anyString(), eq(Boolean.class), any(), any(), any())).thenReturn(false);
	...
	void findActiveByPhoneReturnsEmptyWhenNoResults() {
		...
		List<BookingFactRepository.BookingFact> facts = repository.findActiveByPhone(UUID.randomUUID(), "56950954580");
