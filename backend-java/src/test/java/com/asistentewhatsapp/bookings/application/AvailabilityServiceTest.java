...
class AvailabilityServiceTest {
    ...
    void acceptsNoDuplicateWhenExcludingOwnBooking() {
        ...
        when(repository.countCustomerActiveOverlappingBookingsExcluding(
                eq(BUSINESS_ID), eq(CUSTOMER_ID), eq(PROFESSIONAL_ID),
                eq(startsAt), eq(endsAt), eq(excludeBookingId)))
                .thenReturn(0);

        assertThatCode(() -> availabilityService.checkCustomerDuplicateActiveBooking(
                BUSINESS_ID, CUSTOMER_ID, PROFESSIONAL_ID, startsAt, endsAt, excludeBookingId))
    ...
    void rejectsDuplicateWhenExcludingDifferentBooking() {
        ...
        assertThatThrownBy(() -> availabilityService.checkCustomerDuplicateActiveBooking(
                BUSINESS_ID, CUSTOMER_ID, PROFESSIONAL_ID, startsAt, endsAt, excludeBookingId))
    ...
    void acceptsNewBookingForCustomerWithoutOverlap() {
        ...
        when(repository.countCustomerActiveOverlappingBookingsExcluding(
                eq(BUSINESS_ID), eq(CUSTOMER_ID), eq(PROFESSIONAL_ID),
                eq(startsAt), eq(endsAt), eq(null)))
                .thenReturn(0);

        assertThatCode(() -> availabilityService.checkCustomerDuplicateActiveBooking(
                BUSINESS_ID, CUSTOMER_ID, PROFESSIONAL_ID, startsAt, endsAt, null))
                .doesNotThrowAnyException();
    }
    ...
    void skipsDuplicateCheckWhenCustomerIdIsNull() {
        ...
        assertThatCode(() -> availabilityService.checkCustomerDuplicateActiveBooking(
                BUSINESS_ID, null, PROFESSIONAL_ID, startsAt, endsAt, null))
                .doesNotThrowAnyException();
    }
}
