...
class BookingSyncEventProcessorTest {
	...
	private final BookingSyncEventProcessor processor = new BookingSyncEventProcessor(eventRepository, syncRepository,
			properties);
	...
	private BookingSyncEventRecord event(String status, int attempts, int maxAttempts) {
		return new BookingSyncEventRecord(eventId, bookingId, businessId, "RESERVA_CREADA", 1, "{}", "key-" + bookingId,
				status, attempts, maxAttempts, now, null, null, null, "trace-001", now, now);
	}
	...
	void processDueEventsUpdatesFactStatusAndMarksSynced() {
		BookingSyncEventRecord ev = event("PROCESSING", 1, 5);
		when(eventRepository.claimDueEvents(10, 120000)).thenReturn(List.of(ev));
		when(syncRepository.findEventByIdempotencyKey("key-" + bookingId)).thenReturn(Optional.empty());
	...
	void processDueEventsSkipsUnsupportedEventType() {
		BookingSyncEventRecord ev = new BookingSyncEventRecord(eventId, bookingId, businessId, "UNSUPPORTED_TYPE", 1,
				"{}", "key", "PROCESSING", 1, 5, now, null, null, null, "trace", now, now);
		when(eventRepository.claimDueEvents(10, 120000)).thenReturn(List.of(ev));
	...
	void processDueEventsMarksFailedWhenMaxAttemptsExceeded() {
		...
		verify(eventRepository).markFailedOrRetry(eq(eventId), eq(5), eq(5), eq("RuntimeException"), eq("DB error"),
				any());
	}
	...
	void processDueEventsRetriesWhenAttemptsBelowMax() {
		...
		verify(eventRepository).markFailedOrRetry(eq(eventId), eq(1), eq(5), eq("RuntimeException"), eq("DB error"),
				any());
	}
	...
	void processDueEventsHandlesMultipleEvents() {
		...
		BookingSyncEventRecord ev1 = new BookingSyncEventRecord(eventId1, bookingId, businessId, "RESERVA_CREADA", 1,
				"{}", "k1", "PROCESSING", 1, 5, now, null, null, null, "trace", now, now);
		BookingSyncEventRecord ev2 = new BookingSyncEventRecord(eventId2, bookingId2, businessId, "RESERVA_CREADA", 1,
				"{}", "k2", "PROCESSING", 1, 5, now, null, null, null, "trace", now, now);
