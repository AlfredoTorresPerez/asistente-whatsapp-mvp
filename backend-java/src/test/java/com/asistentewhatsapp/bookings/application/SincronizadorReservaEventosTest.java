...
class SincronizadorReservaEventosTest {
	...
	void sincronizarReservaEnqueuesEventWithCorrectData() {
		sync.sincronizarReserva(businessId, bookingId, "56950954580", "Cliente", "Servicio", "Sucursal", "Profesional",
				startsAt, 30, "CONFIRMED", conversationId, "WHATSAPP_AI", "reservar", "trace-001");

		verify(eventRepository).enqueue(any(), eq(bookingId), eq(businessId), eq("RESERVA_CREADA"), eq(1), anyString(),
				anyString(), eq(properties.getEventMaxAttempts()), any(), eq("trace-001"));
	}

	@Test
	void sincronizarReservaEnqueuesEventWithNullConversationId() {
		sync.sincronizarReserva(businessId, bookingId, "56950954580", "Cliente", "Servicio", "Sucursal", "Profesional",
				startsAt, 30, "CONFIRMED", null, "PUBLIC_LINK", "reservar", "trace-002");

		verify(eventRepository).enqueue(any(), eq(bookingId), eq(businessId), eq("RESERVA_CREADA"), eq(1), anyString(),
				anyString(), anyInt(), any(), eq("trace-002"));
	}
	...
	void sincronizarReservaEnqueuesEventWithMaxAttemptsFromProperties() {
		...
		sync.sincronizarReserva(businessId, bookingId, "56950954580", "Cliente", "Servicio", "Sucursal", "Profesional",
				startsAt, 30, "CONFIRMED", conversationId, "WHATSAPP_AI", "reservar", "trace-003");

		verify(eventRepository).enqueue(any(), eq(bookingId), eq(businessId), eq("RESERVA_CREADA"), eq(1), anyString(),
				anyString(), eq(10), any(), eq("trace-003"));
	}
}
