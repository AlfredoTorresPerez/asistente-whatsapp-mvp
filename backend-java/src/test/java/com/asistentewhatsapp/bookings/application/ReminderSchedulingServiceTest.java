...
class ReminderSchedulingServiceTest {
	...
	void scheduleDefaultRemindersWithFutureStartsAtInsertsFourReminders() {
		...
		verify(agendaRepository, times(4)).insertReminderWithRevision(any(), any(), anyString(), anyString(), any(),
				anyInt());
	}
	...
	void scheduleDefaultRemindersUsesIncrementedRevision() {
		...
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWENTY_FOUR_HOURS_BEFORE",
				"WHATSAPP", startsAt.minusHours(24), 4);
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWENTY_FOUR_HOURS_BEFORE",
				"EMAIL", startsAt.minusHours(24), 4);
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWO_HOURS_BEFORE", "WHATSAPP",
				startsAt.minusHours(2), 4);
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWO_HOURS_BEFORE", "EMAIL",
				startsAt.minusHours(2), 4);
	}
	...
	void scheduleDefaultRemindersWithNullStartsAtSkips() {
		...
		verify(agendaRepository, never()).insertReminderWithRevision(any(), any(), anyString(), anyString(), any(),
				anyInt());
	}
	...
	void scheduleDefaultRemindersWhen24hInPastBut2hFutureInsertsOnly2h() {
		...
		verify(agendaRepository).cancelReminderByBooking(BUSINESS_ID, BOOKING_ID);
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWO_HOURS_BEFORE", "WHATSAPP",
				startsAt.minusHours(2), 0);
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWO_HOURS_BEFORE", "EMAIL",
				startsAt.minusHours(2), 0);
		verify(agendaRepository, never()).insertReminderWithRevision(any(), any(), eq("TWENTY_FOUR_HOURS_BEFORE"),
	...
	void scheduleDefaultRemindersWithFirstRevisionUsesZero() {
		...
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWENTY_FOUR_HOURS_BEFORE",
				"WHATSAPP", startsAt.minusHours(24), 0);
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWENTY_FOUR_HOURS_BEFORE",
				"EMAIL", startsAt.minusHours(24), 0);
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWO_HOURS_BEFORE", "WHATSAPP",
				startsAt.minusHours(2), 0);
		verify(agendaRepository).insertReminderWithRevision(BUSINESS_ID, BOOKING_ID, "TWO_HOURS_BEFORE", "EMAIL",
				startsAt.minusHours(2), 0);
