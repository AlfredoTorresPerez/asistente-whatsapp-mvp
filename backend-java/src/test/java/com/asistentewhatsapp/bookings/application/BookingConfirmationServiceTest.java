...
class BookingConfirmationServiceTest {
	...
	void confirmBlocksBookingThatRequiresDepositWithoutApprovedPayment() {
		...
		ConfirmationLinkRecord link = confirmationLink(businessId, bookingId, true, "PENDING",
		...
		assertThatThrownBy(() -> fixture.service.confirm("token")).isInstanceOf(ApiException.class).extracting("code")
	...
	void confirmAllowsBookingThatRequiresDepositWithApprovedPayment() {
		...
		UUID businessId = UUID.randomUUID();
		UUID bookingId = UUID.randomUUID();
		ConfirmationLinkRecord link = confirmationLink(businessId, bookingId, true, "PAID",
				BookingStateMachine.PENDING_PAYMENT);
		when(fixture.tokenHashService.sha256("token")).thenReturn("hash");
		when(fixture.repository.findByTokenHashForUpdate("hash")).thenReturn(link);
		when(fixture.repository.findByTokenHash("hash")).thenReturn(link);
		when(fixture.bookingPaymentService.hasApprovedRequiredDeposit(businessId, bookingId)).thenReturn(true);
		when(fixture.repository.hasOverlappingActiveBooking(eq(businessId), eq(bookingId), any(), any(), any()))
				.thenReturn(false);
		...
		verify(fixture.completeAgendaJdbcRepository).insertStatusHistory(eq(businessId), eq(bookingId),
				eq(BookingStateMachine.PENDING_PAYMENT), eq(BookingStateMachine.CONFIRMED), any(), eq(null),
				eq("PUBLIC_LINK"));
	}
	...
	void confirmInvokesNotificationsServiceSideEffects() {
		...
		verify(fixture.notificationsService).auditRecord(link.businessId(), link.bookingId(), link.linkId(),
				link.bookingStatus(), link.linkStatus(), link.requiresDeposit(), link.depositAmount(),
				link.paymentStatus(), link.startsAt());
		verify(fixture.notificationsService).syncCalendar(link.bookingId(), link.businessId());
	}

	private static ConfirmationLinkRecord confirmationLink(UUID businessId, UUID bookingId, boolean requiresDeposit,
		...
		return new ConfirmationLinkRecord(UUID.randomUUID(), businessId, bookingId, "OPENED",
				...
				OffsetDateTime.now(ZoneOffset.UTC), null, "Reserva", bookingStatus, "Servicio", "Profesional", "Sala",
				OffsetDateTime.now(ZoneOffset.UTC).plusDays(2), 60, UUID.randomUUID(), "Sucursal", UUID.randomUUID(),
				UUID.randomUUID(), UUID.randomUUID(), "Sucursal", "Cliente", "+56911111111", "cliente@example.com",
				requiresDeposit, BigDecimal.valueOf(10000), paymentStatus, null);
	}
	...
	private static final class Fixture {
		...
		private final CompleteAgendaJdbcRepository completeAgendaJdbcRepository = mock(
				CompleteAgendaJdbcRepository.class);
		...
		private final BookingConfirmationService service = new BookingConfirmationService(repository, properties,
				tokenHashService, auditService, channelDispatchService, completeAgendaJdbcRepository,
				whatsAppWebChannelJdbcRepository, bookingEmailService, bookingPaymentService, calendarSyncService,
				notificationsService, reminderSchedulingService, sincronizadorReservaMotorReglas, transactionManager,
