...
@Import({SecurityConfig.class, GlobalExceptionHandler.class, JwtAccessDeniedHandler.class, JwtAuthenticationEntryPoint.class})
class AdminAuthorizationTest {
    ...
    void agentCannotModifyCompanySettings() throws Exception {
        mockMvc.perform(patch("/api/v1/company")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(validCompanyPayload()))
    ...
    void agentCannotAccessSensitiveAdministrationRoutes() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/admin/security")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
                .andExpect(status().isForbidden());

        mockMvc.perform(get("/api/v1/security/audit-log"))
                .andExpect(status().isForbidden());

        mockMvc.perform(post("/api/v1/whatsapp-web/connect"))
                .andExpect(status().isForbidden());

        mockMvc.perform(post("/api/channels/whatsapp-web/test-message")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
                .andExpect(status().isForbidden());
    }
    ...
    void supervisorCanReadButCannotModifyCompanySettings() throws Exception {
        when(companyService.getCurrent(any()))
    ...
    void adminCanModifyCompanySettings() throws Exception {
        when(companyService.updateCurrent(any(), any()))
                .thenReturn(companyResponse());
