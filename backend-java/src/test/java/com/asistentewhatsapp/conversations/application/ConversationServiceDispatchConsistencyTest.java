...
class ConversationServiceDispatchConsistencyTest {
    ...
    private final AestheticCenterJdbcRepository aestheticCenterJdbcRepository = mock(AestheticCenterJdbcRepository.class);
    ...
    void sendMessagePersistsPendingBeforeDispatchAndMarksSimulated() {
        ...
        when(repository.insertOutboundMessage(
                eq(user.businessId()),
                eq(conversationId),
                eq(user.userId()),
                eq("Hola"),
                ...
                eq(null),
                eq(null),
                eq("retry-1"))).thenReturn(messageId);
        ...
        ConversationMessageResponse response = service.sendMessage(
                user,
                conversationId,
                new SendConversationMessageRequest("Hola", null, "retry-1", null));

        assertThat(response.status()).isEqualTo("SIMULATED");
        inOrder(repository, channelDispatchService).verify(repository).insertOutboundMessage(
                eq(user.businessId()),
                eq(conversationId),
                eq(user.userId()),
                eq("Hola"),
                ...
                eq(null),
                eq("retry-1"));
        verify(repository).insertMessageDeliveryLog(eq(user.businessId()), eq(messageId), eq("PENDING"), eq(null), any());
        verify(repository).updateOutboundMessageDelivery(
                eq(user.businessId()),
                eq(conversationId),
                eq(messageId),
                eq("SIMULATED"),
    ...
    void sendMessageMarksFailedWhenProviderIsDown() {
        ...
        when(repository.findLatestProviderChatId(user.businessId(), conversationId)).thenReturn(Optional.empty());
        when(repository.insertOutboundMessage(any(), any(), any(), any(), any(), any(), any(), any())).thenReturn(messageId);
        when(channelDispatchService.dispatch(any(ChannelDispatchRequest.class)))
        ...
        verify(repository).updateOutboundMessageDelivery(
                ...
                eq(null),
                any());
        verify(repository).insertMessageDeliveryLog(eq(user.businessId()), eq(messageId), eq("FAILED"), eq(null), any());
    }
    ...
    private ConversationJdbcRepository.ConversationContextRecord context(UUID conversationId) {
        return new ConversationJdbcRepository.ConversationContextRecord(
                conversationId,
                "WHATSAPP",
                "OPEN",
                "+56911112222",
    ...
    private ConversationMessageResponse message(UUID messageId, String status) {
        return new ConversationMessageResponse(
                ...
                OffsetDateTime.now(),
                null,
                "FAILED".equals(status) ? OffsetDateTime.now() : null,
                OffsetDateTime.now());
    }
