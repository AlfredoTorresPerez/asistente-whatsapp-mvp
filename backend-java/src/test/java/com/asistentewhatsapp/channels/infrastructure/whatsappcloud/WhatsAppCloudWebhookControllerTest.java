...
class WhatsAppCloudWebhookControllerTest {
    ...
    void setUp() {
        ...
        WhatsAppCloudWebhookController controller = new WhatsAppCloudWebhookController(
                validator, parser, metrics);
        mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
    }

    @Test
    void verificationGetValidReturnsChallenge() throws Exception {
        WhatsAppCloudApiProperties props = new WhatsAppCloudApiProperties(
                true, null, null,
                null, null, "test_verify", null, false, null,
                null, null, null,
                null, false, 5, 15);
        ...
        WhatsAppCloudWebhookController localController = new WhatsAppCloudWebhookController(
                localValidator, localParser, localMetrics);

        MockMvc localMvc = MockMvcBuilders.standaloneSetup(localController).build();

        localMvc.perform(get("/api/v1/integrations/whatsapp-cloud/webhook")
                        .param("hub.mode", "subscribe")
                        .param("hub.verify_token", "test_verify")
                        .param("hub.challenge", "challenge_value_123"))
                .andExpect(status().isOk())
                .andExpect(content().string("challenge_value_123"));
    }
    ...
    void verificationGetInvalidTokenReturns403() throws Exception {
        doThrow(new com.asistentewhatsapp.shared.exception.ApiException(
                ...
                .when(validator).validateAndExtractChallenge("subscribe", "wrong_token", "challenge");

        mockMvc.perform(get("/api/v1/integrations/whatsapp-cloud/webhook")
                        .param("hub.mode", "subscribe")
    ...
    void verificationGetInvalidModeReturns403() throws Exception {
        doThrow(new com.asistentewhatsapp.shared.exception.ApiException(
                        org.springframework.http.HttpStatus.FORBIDDEN,
                        "WEBHOOK_VERIFICATION_FAILED", "Invalid mode"))
                .when(validator).validateAndExtractChallenge("unsubscribe", "test_verify", "challenge");
    ...
    void invalidSignatureReturnsUnauthorized() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/integrations/whatsapp-cloud/webhook")
                ...
                .andExpect(content().json("{\"status\":\"REJECTED\"}"));
    }
    ...
    void missingSignatureReturnsUnauthorizedWhenRequired() throws Exception {
        doThrow(new com.asistentewhatsapp.shared.exception.ApiException(
                        org.springframework.http.HttpStatus.UNAUTHORIZED,
                        "WEBHOOK_SIGNATURE_MISSING", "Missing signature"))
