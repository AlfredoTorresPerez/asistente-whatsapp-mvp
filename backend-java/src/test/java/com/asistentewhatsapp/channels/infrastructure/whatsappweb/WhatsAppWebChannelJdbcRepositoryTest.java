...
class WhatsAppWebChannelJdbcRepositoryTest {

    @Test
    void insertChannelEventLogIsIdempotentWhenConflictDoesNothing() {
        JdbcTemplate jdbcTemplate = mock(JdbcTemplate.class);
        WhatsAppWebChannelJdbcRepository repository = new WhatsAppWebChannelJdbcRepository(jdbcTemplate, new ObjectMapper());
    ...
    void updateMessageStatusKeepsAckProgressionFields() {
        ...
        assertThat(sql.getValue())
                .contains("set status = ?")
                .contains("sent_at = case when ? in ('SENT', 'DELIVERED', 'READ')")
