...
class WhatsAppCloudWebhookParserTest {
	...
	private String loadFixture(String name) throws IOException {
		InputStream is = getClass().getResourceAsStream("/fixtures/whatsappcloud/" + name);
		if (is == null) {
	...
	void statusSentIsProcessed() throws IOException {
		...
		parser.parseAndProcess(body);
		verify(deliveryStatusService, times(1)).processDeliveryStatus(any(), eq(businessId));
	}
	...
	void unknownPhoneNumberIdReturnsEarly() throws IOException {
		when(repository.findChannelAccountByPhoneNumberId("123456789012345")).thenReturn(Optional.empty());
