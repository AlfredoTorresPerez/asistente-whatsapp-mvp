...
public class AuthService {
	...
	private static final String PASSWORD_RECOVERY_ACCEPTED_MESSAGE = "Si el correo existe, enviaremos instrucciones para restablecer la contrasena.";
	...
	public AuthService(UserAccountRepository userAccountRepository, SecurityPolicyRepository securityPolicyRepository,
			BusinessRepository businessRepository, UserRoleJdbcRepository userRoleJdbcRepository,
			UserPermissionJdbcRepository userPermissionJdbcRepository,
			PasswordResetTokenRepository passwordResetTokenRepository, AuditLogJdbcRepository auditLogJdbcRepository,
			UserSessionJdbcRepository userSessionJdbcRepository, PasswordEncoder passwordEncoder, JwtService jwtService,
			PasswordPolicyService passwordPolicyService, SecurityUserMapper securityUserMapper,
	...
	public LoginResponse login(LoginRequest request, String clientIpAddress) {
		...
		if (!passwordEncoder.matches(request.password(), userAccount.getPasswordHash())) {
			...
			auditLogJdbcRepository.insert(userAccount.getBusinessId(), userAccount.getId(), "LOGIN_FAILED",
					"USER_ACCOUNT", userAccount.getId(), "Intento fallido de inicio de sesion.",
		...
		auditLogJdbcRepository.insert(userAccount.getBusinessId(), userAccount.getId(), "LOGIN_SUCCEEDED",
				"USER_ACCOUNT", userAccount.getId(), "Inicio de sesion exitoso.",
				Map.of("email", userAccount.getEmail(), "ipAddress", clientIpAddress),
				OffsetDateTime.now(ZoneOffset.UTC));

		RefreshTokenService.RefreshTokenResult refreshResult = refreshTokenService.createSession(authenticatedUser,
				null, clientIpAddress);

		AuthUserResponse authUserResponse = securityUserMapper.toAuthUserResponse(authenticatedUser);
		return new LoginResponse(jwtService.createToken(authenticatedUser), refreshResult.rawToken(), "Bearer",
				jwtService.getAccessTokenExpiresInSeconds(), jwtProperties.getRefreshTokenExpiresInSeconds(),
				authUserResponse);
	}
	...
	public StatusResponse logout(AuthenticatedUser authenticatedUser, String refreshToken) {
		if (refreshToken != null && !refreshToken.isBlank()) {
			refreshTokenService.revokeSession(authenticatedUser.businessId(), authenticatedUser.userId(), refreshToken);
		} else {
			refreshTokenService.revokeAllSessions(authenticatedUser.businessId(), authenticatedUser.userId(),
					authenticatedUser.userId());
		}
		auditLogJdbcRepository.insert(authenticatedUser.businessId(), authenticatedUser.userId(), "LOGOUT",
				"USER_ACCOUNT", authenticatedUser.userId(), "Cierre de sesion solicitado.",
	...
	public LoginResponse refresh(String refreshToken, String deviceInfo, String clientIpAddress) {
		...
		UserAccountEntity userAccount = userAccountRepository
				.findScopedById(session.getBusinessId(), session.getUserId())
		...
		auditLogJdbcRepository.insert(userAccount.getBusinessId(), userAccount.getId(), "TOKEN_REFRESHED",
				"USER_ACCOUNT", userAccount.getId(), "Token de acceso renovado.", Map.of("ipAddress", clientIpAddress),
				OffsetDateTime.now(ZoneOffset.UTC));

		AuthUserResponse authUserResponse = securityUserMapper.toAuthUserResponse(authenticatedUser);
		return new LoginResponse(jwtService.createToken(authenticatedUser), rotated.rawToken(), "Bearer",
				jwtService.getAccessTokenExpiresInSeconds(), jwtProperties.getRefreshTokenExpiresInSeconds(),
				authUserResponse);
	}
	...
	public ForgotPasswordResponse forgotPassword(ForgotPasswordRequest request) {
		...
		UserAccountEntity userAccount = optionalUser.get();
		if (userAccount.getStatus() != UserAccountStatus.ACTIVE) {
			auditLogJdbcRepository.insert(userAccount.getBusinessId(), userAccount.getId(),
					"PASSWORD_RECOVERY_REQUESTED", "USER_ACCOUNT", userAccount.getId(),
					"Solicitud de recuperacion de contrasena.",
					Map.of("emailMasked", maskEmail(userAccount.getEmail()), "requestedAt", now.toString(), "status",
							"ACCEPTED", "accountStatus", userAccount.getStatus().name(), "delivery",
							"SKIPPED_NON_ACTIVE_USER"),
					now);
			return acceptedForgotPasswordResponse();
		}

		SecurityPolicyEntity securityPolicy = findSecurityPolicy(userAccount.getBusinessId());
		passwordResetTokenRepository.findAllByUserIdAndConsumedAtIsNullAndExpiresAtAfter(userAccount.getId(), now)
		...
		PasswordResetTokenEntity passwordResetToken = PasswordResetTokenEntity.create(UUID.randomUUID(),
				userAccount.getBusinessId(), userAccount.getId(), hashToken(rawToken), expiresAt);
		passwordResetTokenRepository.save(passwordResetToken);

		auditLogJdbcRepository.insert(userAccount.getBusinessId(), userAccount.getId(), "PASSWORD_RECOVERY_REQUESTED",
				"USER_ACCOUNT", userAccount.getId(), "Solicitud de recuperacion de contrasena.",
				Map.of("emailMasked", maskEmail(userAccount.getEmail()), "requestedAt", now.toString(), "status",
						"ACCEPTED", "passwordMinLength", securityPolicy.getPasswordMinLength(), "tokenId",
		...
		try {
			DeliveryStatus deliveryStatus = transactionalEmailService.sendPasswordResetEmail(userAccount.getEmail(),
					displayName(userAccount), resetUrl, expiresAt.toInstant(),
					resolveZoneId(userAccount.getTimezone()));
			boolean sent = DeliveryStatus.SENT.equals(deliveryStatus);
			auditLogJdbcRepository.insert(userAccount.getBusinessId(), userAccount.getId(),
					sent ? "PASSWORD_RECOVERY_EMAIL_SENT" : "PASSWORD_RECOVERY_EMAIL_SIMULATED", "USER_ACCOUNT",
					userAccount.getId(),
					sent
							? "Correo de recuperacion enviado por SMTP."
							: "Correo de recuperacion simulado o no enviado por configuracion local.",
					Map.of("emailMasked", maskEmail(userAccount.getEmail()), "provider", sent ? "smtp" : "simulation",
							"expiresAt", expiresAt.toString()),
					OffsetDateTime.now(ZoneOffset.UTC));
		} catch (Exception exception) {
			LOGGER.warn("PASSWORD_RESET_EMAIL_FAILED userId={} reason={}", userAccount.getId(),
					exception.getClass().getSimpleName());
			auditLogJdbcRepository.insert(userAccount.getBusinessId(), userAccount.getId(),
					"PASSWORD_RECOVERY_EMAIL_FAILED", "USER_ACCOUNT", userAccount.getId(),
					"No se pudo enviar el correo de recuperacion.", Map.of("emailMasked",
							maskEmail(userAccount.getEmail()), "reason", exception.getClass().getSimpleName()),
					OffsetDateTime.now(ZoneOffset.UTC));
		}
	...
	public ResetPasswordValidationResponse validateResetPasswordToken(String token) {
		Optional<PasswordResetTokenEntity> optionalToken = passwordResetTokenRepository
	...
	public StatusResponse resetPassword(ResetPasswordRequest request) {
		...
		PasswordResetTokenEntity passwordResetToken = passwordResetTokenRepository
		...
		UserAccountEntity userAccount = userAccountRepository
				.findScopedById(passwordResetToken.getBusinessId(), passwordResetToken.getUserId())
		...
		auditLogJdbcRepository.insert(userAccount.getBusinessId(), userAccount.getId(), "PASSWORD_RESET_COMPLETED",
				"USER_ACCOUNT", userAccount.getId(), "Contrasena actualizada desde flujo de recuperacion.",
				Map.of("email", userAccount.getEmail()), OffsetDateTime.now(ZoneOffset.UTC));
	...
	private SecurityPolicyEntity findSecurityPolicy(UUID businessId) {
		return securityPolicyRepository.findByBusinessId(businessId).orElseThrow(
	...
	private void recordAnonymousFailedLogin(String email, String clientIpAddress) {
		businessRepository.findByActiveTrueOrderByCreatedAtAsc().stream().findFirst()
				.ifPresent(business -> auditLogJdbcRepository.insert(business.getId(), null, "LOGIN_FAILED",
						"USER_ACCOUNT", null, "Intento fallido de inicio de sesion.",
						Map.of("email", email, "ipAddress", clientIpAddress), OffsetDateTime.now(ZoneOffset.UTC)));
	}
	...
	private void recordAnonymousPasswordRecoveryRequest(String maskedEmail, OffsetDateTime now) {
		businessRepository.findByActiveTrueOrderByCreatedAtAsc().stream().findFirst().ifPresent(
				business -> auditLogJdbcRepository.insert(business.getId(), null, "PASSWORD_RECOVERY_REQUESTED",
						"USER_ACCOUNT", null, "Solicitud de recuperacion de contrasena.", Map.of("emailMasked",
								maskedEmail, "requestedAt", now.toString(), "status", "ACCEPTED", "userMatched", false),
						now));
	}
	...
	private String buildResetUrl(String rawToken) {
		...
		return UriComponentsBuilder.fromUriString(baseUrl).replacePath(normalizedBasePath(baseUrl) + "/reset-password")
