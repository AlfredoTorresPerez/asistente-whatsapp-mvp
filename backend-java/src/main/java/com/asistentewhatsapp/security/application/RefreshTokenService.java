...
public class RefreshTokenService {
	...
	public RefreshTokenResult createSession(AuthenticatedUser user, String deviceInfo, String ipAddress) {
		...
		UserSessionEntity session = new UserSessionEntity(UUID.randomUUID(), user.businessId(), user.userId(),
				tokenHash, deviceInfo, ipAddress, now, now,
	...
	@Transactional
	public RefreshTokenResult rotate(AuthenticatedUser user, String currentRawToken, String deviceInfo,
			String ipAddress) {
		OffsetDateTime now = OffsetDateTime.now(ZoneOffset.UTC);
		...
		UserSessionEntity newSession = new UserSessionEntity(UUID.randomUUID(), user.businessId(), user.userId(),
				newHash, deviceInfo != null ? deviceInfo : session.getDeviceInfo(),
				ipAddress != null ? ipAddress : session.getIpAddress(), now, now,
				now.plusSeconds(jwtProperties.getRefreshTokenExpiresInSeconds()));
