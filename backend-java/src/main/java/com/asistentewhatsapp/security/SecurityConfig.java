...
public class SecurityConfig {
	...
	SecurityFilterChain publicBookingConfirmationSecurityFilterChain(HttpSecurity http) throws Exception {
		return http.securityMatcher(new AntPathRequestMatcher("/api/v1/public/booking-confirmations/**"))
	...
	SecurityFilterChain publicCustomerBookingsSecurityFilterChain(HttpSecurity http) throws Exception {
		return http.securityMatcher(new AntPathRequestMatcher("/api/v1/public/customer-bookings/**"))
				.csrf(AbstractHttpConfigurer::disable).httpBasic(AbstractHttpConfigurer::disable)
				.formLogin(AbstractHttpConfigurer::disable)
				.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
				.authorizeHttpRequests(authorize -> authorize.anyRequest().permitAll()).cors(Customizer.withDefaults())
				.build();
	}
	...
	SecurityFilterChain publicSecurityFilterChain(HttpSecurity http) throws Exception {
		return http.securityMatcher(SecurityPublicPaths.PUBLIC_ENDPOINTS).csrf(AbstractHttpConfigurer::disable)
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		return http.csrf(AbstractHttpConfigurer::disable).httpBasic(AbstractHttpConfigurer::disable)
				...
				.authorizeHttpRequests(authorize -> authorize.requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
						...
						.requestMatchers(HttpMethod.GET, "/api/v1/admin/**").hasAnyRole("OWNER", "ADMIN", "SUPERVISOR")
						...
						.requestMatchers("/api/v1/company", "/api/businesses/current").hasAnyRole("OWNER", "ADMIN")
						...
						.hasAnyRole("OWNER", "ADMIN").requestMatchers("/api/v1/system/status")
						.hasAnyRole("OWNER", "ADMIN")
						.requestMatchers(HttpMethod.GET, "/api/v1/calendar-integrations/status")
						.hasAnyRole("OWNER", "ADMIN", "SUPERVISOR")
						.requestMatchers(HttpMethod.GET, "/api/v1/calendar-integrations/*/calendars")
						.hasAnyRole("OWNER", "ADMIN", "SUPERVISOR").requestMatchers("/api/v1/calendar-integrations/**")
						.hasAnyRole("OWNER", "ADMIN")
						.requestMatchers(HttpMethod.GET, "/api/v1/bookings/*/calendar-sync")
						.hasAnyRole("OWNER", "ADMIN", "SUPERVISOR")
						.requestMatchers("/api/v1/bookings/*/calendar-sync/retry").hasAnyRole("OWNER", "ADMIN")
