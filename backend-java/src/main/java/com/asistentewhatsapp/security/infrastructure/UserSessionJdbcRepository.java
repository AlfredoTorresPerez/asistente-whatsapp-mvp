...
public class UserSessionJdbcRepository {
    ...
    public void insert(UserSessionEntity session) {
        jdbcTemplate.update("""
            ...
            values (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            session.getId(), session.getBusinessId(), session.getUserId(),
            session.getRefreshTokenHash(), session.getDeviceInfo(),
            session.getIpAddress(), session.getCreatedAt(),
            session.getLastUsedAt(), session.getExpiresAt());
    }
    ...
    public void updateLastUsed(UUID id, OffsetDateTime lastUsedAt) {
        jdbcTemplate.update("update user_session set last_used_at = ? where id = ?",
            lastUsedAt, id);
    }

    public void revoke(UUID id, OffsetDateTime revokedAt, UUID revokedBy) {
        jdbcTemplate.update("update user_session set revoked_at = ?, revoked_by = ? where id = ?",
            revokedAt, revokedBy, id);
    }
