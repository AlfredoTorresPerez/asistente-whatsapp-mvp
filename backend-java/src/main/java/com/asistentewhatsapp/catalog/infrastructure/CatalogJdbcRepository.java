...
public class CatalogJdbcRepository {
	...
	public PagedResponse<CatalogProductResponse> findProducts(UUID businessId, int page, int size, String search,
	...
	public PagedResponse<CatalogCategoryResponse> findCategories(UUID businessId, int page, int size, Boolean active) {
		...
		Long total = jdbcTemplate.queryForObject("select count(*) from product_category" + where, parameters,
				Long.class);
		long totalItems = total == null ? 0 : total;
	...
	public CatalogCategoryResponse insertCategory(UUID businessId, UpsertCatalogCategoryRequest request) {
		...
		jdbcTemplate.update("""
				...
				""",
				new MapSqlParameterSource().addValue("categoryId", categoryId).addValue("businessId", businessId)
						.addValue("code", normalizeCode(request.code())).addValue("name", request.name().trim())
	...
	public CatalogCategoryResponse findCategory(UUID businessId, UUID categoryId) {
		List<CatalogCategoryResponse> items = jdbcTemplate.query("""
				...
				where business_id = :businessId
				  and id = :categoryId
				""", new MapSqlParameterSource().addValue("businessId", businessId).addValue("categoryId", categoryId),
				categoryRowMapper());
		if (items.isEmpty()) {
	...
	public boolean categoryExists(UUID businessId, String categoryCode) {
		Boolean exists = jdbcTemplate.queryForObject(
				"""
						...
						""",
				new MapSqlParameterSource().addValue("businessId", businessId).addValue("categoryCode",
	...
	private MapSqlParameterSource productParameters(UUID businessId, UUID productId,
		...
		return new MapSqlParameterSource().addValue("businessId", businessId).addValue("productId", productId)
				...
				.addValue("sku", normalizeSku(request.sku(), request.name())).addValue("name", request.name().trim())
				.addValue("description", clean(request.description())).addValue("price", request.price())
				.addValue("stock", request.stock() == null ? 0 : request.stock())
				.addValue("stockMinimum", request.stockMinimum() == null ? 0 : request.stockMinimum())
				.addValue("supplier", clean(request.supplier())).addValue("expiresAt", request.expiresAt())
				.addValue("active", request.active() == null || request.active());
	}

	private RowMapper<CatalogProductResponse> productRowMapper() {
		return (rs, rowNum) -> new CatalogProductResponse(rs.getObject("id", UUID.class),
				rs.getObject("category_id", UUID.class), rs.getString("category_code"), rs.getString("category_name"),
				rs.getString("sku"), rs.getString("name"), rs.getString("description"), rs.getBigDecimal("price"),
				rs.getInt("stock_quantity"), rs.getInt("stock_minimum"),
				rs.getInt("stock_quantity") <= rs.getInt("stock_minimum"), rs.getString("supplier"),
				getLocalDate(rs, "expires_at"), rs.getBoolean("active"),
				rs.getObject("created_at", OffsetDateTime.class), rs.getObject("updated_at", OffsetDateTime.class));
	}

	private RowMapper<CatalogCategoryResponse> categoryRowMapper() {
		return (rs, rowNum) -> new CatalogCategoryResponse(rs.getObject("id", UUID.class), rs.getString("code"),
	...
	private String normalizeSku(String sku, String name) {
		...
		return name.trim().toUpperCase(Locale.ROOT).replaceAll("[^A-Z0-9]+", "-").replaceAll("(^-|-$)", "");
	}
