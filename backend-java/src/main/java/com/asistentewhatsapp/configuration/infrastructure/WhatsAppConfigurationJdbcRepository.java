...
public class WhatsAppConfigurationJdbcRepository {
    ...
    public Optional<ChannelAccountDetailRecord> findChannelAccount(UUID businessId) {
        return jdbcTemplate.query(
                        """
                                ...
                                limit 1
                                """,
                        new ChannelAccountDetailRowMapper(),
                        businessId)
                .stream()
                .findFirst();
    }
    ...
    public PreferencesRecord updatePreferences(UUID businessId, WhatsAppConfigurationPreferencesRequest request) {
        jdbcTemplate.update(
                """
                        ...
                        """,
                businessId,
                request.newMessageNotifications(),
                request.autoReassignment(),
                request.agentSignature(),
                request.outOfHoursMessage());
    ...
    public record ChannelAccountDetailRecord(
            UUID id,
            UUID businessId,
            String providerName,
            String sessionKey,
    ...
    public record PreferencesRecord(
            UUID businessId,
    ...
    private static class ChannelAccountDetailRowMapper implements RowMapper<ChannelAccountDetailRecord> {
        ...
        public ChannelAccountDetailRecord mapRow(ResultSet resultSet, int rowNum) throws SQLException {
            return new ChannelAccountDetailRecord(
                    resultSet.getObject("id", UUID.class),
                    ...
                    resultSet.getObject("disconnected_at", OffsetDateTime.class),
    ...
    private static class PreferencesRowMapper implements RowMapper<PreferencesRecord> {
        ...
        public PreferencesRecord mapRow(ResultSet resultSet, int rowNum) throws SQLException {
            return new PreferencesRecord(
                    resultSet.getObject("business_id", UUID.class),
                    resultSet.getBoolean("new_message_notifications"),
                    resultSet.getBoolean("auto_reassignment"),
                    resultSet.getBoolean("agent_signature"),
                    resultSet.getBoolean("out_of_hours_message"));
        }
    }
