...
public class WhatsAppChannelService {
    ...
    public WhatsAppChannelService(
            WhatsAppChannelJdbcRepository repository,
    ...
    public WhatsAppChannelResponse updateChannel(
        ...
        auditLogJdbcRepository.insert(
                ...
                "CHANNEL_ACCOUNT",
                channel.id(),
                "Se actualizo la configuracion del canal WhatsApp.",
                Map.of("providerType", channel.providerName()),
    ...
    public WhatsAppChannelValidateResponse validateConfiguration(AuthenticatedUser authenticatedUser) {
        ...
        return new WhatsAppChannelValidateResponse(
                true, channel.providerName(), "Canal local configurado correctamente.", List.of());
    }
    ...
    public WhatsAppChannelTestMessageResponse sendTestMessage(
        ...
        throw new ApiException(
                ...
                "WHATSAPP_TEST_MESSAGE_UNAVAILABLE",
    ...
    public WhatsAppChannelResponse activateChannel(AuthenticatedUser authenticatedUser) {
        ...
        auditLogJdbcRepository.insert(
                businessId,
                authenticatedUser.userId(),
                "WHATSAPP_CHANNEL_ACTIVATED",
                "CHANNEL_ACCOUNT",
                channel.id(),
                "Se activo el canal WhatsApp.",
                Map.of("providerType", channel.providerName()),
                OffsetDateTime.now(ZoneOffset.UTC));
    ...
    public WhatsAppChannelResponse deactivateChannel(AuthenticatedUser authenticatedUser) {
        ...
        auditLogJdbcRepository.insert(
                businessId,
                authenticatedUser.userId(),
                "WHATSAPP_CHANNEL_DEACTIVATED",
                "CHANNEL_ACCOUNT",
                channel.id(),
                "Se desactivo el canal WhatsApp.",
                Map.of("providerType", channel.providerName()),
                OffsetDateTime.now(ZoneOffset.UTC));
    ...
    private WhatsAppChannelValidateResponse validateCloudApi(ChannelRecord channel) {
        if (!WhatsAppChannelStatus.CREDENTIAL_CONFIGURED.equals(channel.credentialStatus())) {
            return new WhatsAppChannelValidateResponse(
                    false,
                    WhatsAppChannelStatus.PROVIDER_META_CLOUD_API,
        ...
        if (channel.phoneNumberId() == null || channel.phoneNumberId().isBlank()) {
            return new WhatsAppChannelValidateResponse(
                    false,
                    WhatsAppChannelStatus.PROVIDER_META_CLOUD_API,
                    "El identificador tecnico del numero no esta configurado.",
                    List.of("Falta phone_number_id"));
        }
        return new WhatsAppChannelValidateResponse(
                true,
                WhatsAppChannelStatus.PROVIDER_META_CLOUD_API,
                "La configuracion de Meta Cloud API es valida.",
                List.of());
    }
    ...
    private WhatsAppChannelResponse toChannelResponse(ChannelRecord record, List<EventRecord> events) {
        ...
        if (isCloud) {
            cloudConfig = new MetaCloudConfig(
                    record.phoneNumberId(),
                    record.providerAccountId(),
                    record.graphApiVersion(),
                    record.webhookCallbackUrl(),
                    record.webhookStatus(),
                    toWebhookLabel(record.webhookStatus()),
                    record.credentialStatus(),
                    toCredentialLabel(record.credentialStatus()),
                    record.tokenExpiresAt(),
                    record.active());
        }

        List<ChannelEventItem> eventItems = events.stream()
                .map(e -> new ChannelEventItem(
                        e.deliveryId(),
                        e.eventType(),
                        toEventTitle(e.eventType()),
                        e.eventType(),
                        resolveActor(e.eventType()),
                        toEventTone(e.eventType()),
                        e.receivedAt()))
                .toList();
        ...
        return new WhatsAppChannelResponse(
                ...
                toRegistrationLabel(record.registrationStatus()),
                record.operationalStatus(),
                toOperationalLabel(record.operationalStatus()),
                record.webhookStatus(),
                toWebhookLabel(record.webhookStatus()),
                record.credentialStatus(),
                toCredentialLabel(record.credentialStatus()),
    ...
    private String toRegistrationLabel(String status) {
        if (status == null) return "No configurado";
    ...
    private String toEventTitle(String eventType) {
        if (eventType == null) return "Evento del canal";
    ...
    private String resolveActor(String eventType) {
        if (eventType == null) return "Sistema";
        return switch (eventType) {
    ...
    private String toEventTone(String eventType) {
        if (eventType == null) return "neutral";
        return switch (eventType) {
