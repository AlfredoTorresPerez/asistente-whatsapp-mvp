...
public class WhatsAppConfigurationService {
    ...
    public WhatsAppConfigurationService(
            WhatsAppConfigurationJdbcRepository repository,
    ...
    public WhatsAppConfigurationResponse getConfiguration(AuthenticatedUser authenticatedUser) {
        WhatsAppConfigurationJdbcRepository.BusinessRecord business = repository.findBusiness(authenticatedUser.businessId())
                .orElseThrow(() -> new ApiException(
                        HttpStatus.NOT_FOUND,
                        "BUSINESS_NOT_FOUND",
                        "No se encontro la empresa autenticada para cargar configuracion."));
        WhatsAppConfigurationJdbcRepository.ChannelAccountDetailRecord channelAccount = repository.findChannelAccount(authenticatedUser.businessId())
                .orElseThrow(() -> new ApiException(
                        HttpStatus.NOT_FOUND,
                        "WHATSAPP_CHANNEL_NOT_FOUND",
                        "No se encontro el canal WhatsApp Web para la empresa actual."));
        WhatsAppConfigurationJdbcRepository.PreferencesRecord preferences = repository.findOrCreatePreferences(authenticatedUser.businessId());
        ...
        OffsetDateTime lastSynchronizationAt = firstNotNull(
                status.lastEventAt(),
                channelAccount.lastEventAt(),
                channelAccount.updatedAt());

        return new WhatsAppConfigurationResponse(
                status.sessionStatus(),
                resolvedPhone,
                firstNotBlank(business.businessName(), business.companyName()),
                ...
                firstNotBlank(status.qrCode(), channelAccount.lastQrCode()),
                status.adapterReachable(),
                firstNotBlank(status.adapterMode(), channelAccount.providerName()),
                status.warningMessage(),
                toPreferencesResponse(preferences),
                toMainChannel(channelAccount, resolvedPhone, preferences.outOfHoursMessage()),
                toLinkedDevices(channelAccount, status, lastSynchronizationAt),
                toSessionHistory(channelAccount, repository.findRecentSessionEvents(authenticatedUser.businessId(), 8)));
    }
    ...
    private WhatsAppConfigurationPreferencesResponse toPreferencesResponse(
        ...
        return new WhatsAppConfigurationPreferencesResponse(
                preferences.newMessageNotifications(),
                preferences.autoReassignment(),
                preferences.agentSignature(),
                preferences.outOfHoursMessage());
    }
    ...
    private WhatsAppConfigurationChannelResponse toMainChannel(
        ...
        return new WhatsAppConfigurationChannelResponse(
                "GroupUp WhatsApp",
                phoneNumber,
                resolveChannelType(channelAccount.providerName()),
                outOfHoursMessage ? "Lun a Vie: 8:00 a. m. - 6:00 p. m. / Sab: 9:00 a. m. - 1:00 p. m." : "Sin respuesta fuera de horario",
                channelAccount.active());
    }

    private List<WhatsAppConfigurationLinkedDeviceResponse> toLinkedDevices(
            WhatsAppConfigurationJdbcRepository.ChannelAccountDetailRecord channelAccount,
            WhatsAppWebStatusResponse status,
            OffsetDateTime lastSynchronizationAt) {
        List<WhatsAppConfigurationLinkedDeviceResponse> devices = new ArrayList<>();
        devices.add(new WhatsAppConfigurationLinkedDeviceResponse(
                channelAccount.id().toString(),
                "Sesion principal",
                "Administrador",
                LOCAL_LOCATION,
                resolveBrowser(status.adapterMode()),
                toDeviceStatus(status.sessionStatus()),
                lastSynchronizationAt));
        return devices;
    }
    ...
    private List<WhatsAppConfigurationSessionHistoryResponse> toSessionHistory(
        ...
        if (events.isEmpty()) {
            OffsetDateTime occurredAt = firstNotNull(
                    channelAccount.lastEventAt(),
                    channelAccount.connectedAt(),
                    channelAccount.disconnectedAt(),
                    channelAccount.updatedAt(),
                    OffsetDateTime.now(ZoneOffset.UTC));
            return List.of(new WhatsAppConfigurationSessionHistoryResponse(
                    channelAccount.id().toString(),
                    toSessionStatusTitle(channelAccount.status()),
                    "Sistema local",
                    toHistoryTone(channelAccount.status()),
                    occurredAt));
        }

        return events.stream()
                .map(event -> new WhatsAppConfigurationSessionHistoryResponse(
                        event.deliveryId(),
                        toEventTitle(event.eventType(), event.processingStatus()),
                        "Adaptador WhatsApp Web",
                        toEventTone(event.eventType(), event.processingStatus()),
                        event.receivedAt()))
                .toList();
    }
