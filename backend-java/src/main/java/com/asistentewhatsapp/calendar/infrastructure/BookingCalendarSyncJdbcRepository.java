...
public class BookingCalendarSyncJdbcRepository {
	...
	public List<BookingCalendarSyncRecord> findByBooking(UUID bookingId) {
		return jdbcTemplate.query("select * from booking_calendar_sync where booking_id = :bookingId order by provider",
				new MapSqlParameterSource("bookingId", bookingId), recordRowMapper());
	}
	...
	public List<BookingCalendarSyncRecord> findByBookingAndBusiness(UUID bookingId, UUID businessId) {
		return jdbcTemplate.query(
				...
				new MapSqlParameterSource().addValue("bookingId", bookingId).addValue("businessId", businessId),
				recordRowMapper());
	}
	...
	public List<BookingCalendarSyncRecord> findByBookingAndProvider(UUID bookingId, String provider) {
		return jdbcTemplate.query(
				...
				new MapSqlParameterSource().addValue("bookingId", bookingId).addValue("provider", provider),
				recordRowMapper());
	}

	public Optional<BookingCalendarSyncRecord> findByBookingAndProviderAndBusiness(UUID bookingId, String provider,
			UUID businessId) {
		return jdbcTemplate.query(
	...
	public List<BookingCalendarSyncRecord> findFailedSyncs(int maxRetries, OffsetDateTime before) {
		return jdbcTemplate.query("""
				...
				limit 50
				""", new MapSqlParameterSource().addValue("maxRetries", maxRetries).addValue("before", before),
				recordRowMapper());
	}
	...
	public List<BookingCalendarSyncRecord> findByBusinessAndStatusAndRetries(UUID businessId, String status,
		...
		return jdbcTemplate.query("""
				...
				limit :limit
				""",
				new MapSqlParameterSource().addValue("businessId", businessId).addValue("status", status)
						.addValue("maxRetries", maxRetries).addValue("before", before).addValue("limit", limit),
				recordRowMapper());
	}
	...
	public List<BookingCalendarSyncRecord> findPendingSyncs(int limit) {
		return jdbcTemplate.query("""
				...
				limit :limit
				""", new MapSqlParameterSource("limit", limit), recordRowMapper());
	}
	...
	public void insert(BookingCalendarSyncRecord record) {
		jdbcTemplate.update("""
				...
				""", new MapSqlParameterSource().addValue("id", record.id()).addValue("bookingId", record.bookingId())
				.addValue("businessId", record.businessId()).addValue("provider", record.provider())
				.addValue("externalEventId", record.externalEventId()).addValue("syncStatus", record.syncStatus())
				.addValue("syncAction", record.syncAction()).addValue("errorMessage", record.errorMessage())
				.addValue("retryCount", record.retryCount()).addValue("lastSyncAttemptAt", record.lastSyncAttemptAt())
				.addValue("lastSuccessfulSyncAt", record.lastSuccessfulSyncAt())
				.addValue("createdAt", record.createdAt()).addValue("updatedAt", record.updatedAt()));
	}
	...
	public void updateStatus(UUID syncId, String syncStatus, String externalEventId, String errorMessage) {
		jdbcTemplate.update("""
				...
				where id = :id
				""",
				new MapSqlParameterSource().addValue("id", syncId).addValue("syncStatus", syncStatus)
						.addValue("externalEventId", externalEventId).addValue("errorMessage", errorMessage)
	...
	public void updateSyncSuccess(UUID syncId, String externalEventId) {
		jdbcTemplate.update("""
				update booking_calendar_sync set
				    ...
				    updated_at = :now
				where id = :id
				""", new MapSqlParameterSource().addValue("id", syncId).addValue("externalEventId", externalEventId)
				.addValue("now", OffsetDateTime.now()));
	}
	...
	public void updateSyncFailed(UUID syncId, String errorMessage) {
		jdbcTemplate.update("""
				update booking_calendar_sync set
				    ...
				    updated_at = :now
				where id = :id
				""", new MapSqlParameterSource().addValue("id", syncId).addValue("errorMessage",
	...
	private RowMapper<BookingCalendarSyncRecord> recordRowMapper() {
		return (rs, rowNum) -> new BookingCalendarSyncRecord(rs.getObject("id", UUID.class),
				rs.getObject("booking_id", UUID.class), rs.getObject("business_id", UUID.class),
				rs.getString("provider"), rs.getString("external_event_id"), rs.getString("sync_status"),
				rs.getString("sync_action"), rs.getString("error_message"), rs.getInt("retry_count"),
	...
	public record BookingCalendarSyncRecord(UUID id, UUID bookingId, UUID businessId, String provider,
			String externalEventId, String syncStatus, String syncAction, String errorMessage, int retryCount,
			OffsetDateTime lastSyncAttemptAt, OffsetDateTime lastSuccessfulSyncAt, OffsetDateTime createdAt,
