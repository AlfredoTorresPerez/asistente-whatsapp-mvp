...
public class TokenEncryptionService {
	...
	public TokenEncryptionService(@Value("${app.calendar.encryption-secret:}") String encryptionSecret,
		...
		if (encryptionSecret == null || encryptionSecret.isBlank()) {
			if (!isDevelopment) {
				throw new IllegalStateException("APP_CALENDAR_ENCRYPTION_SECRET is not configured. "
			...
			this.ephemeral = true;
			LOGGER.warn(
					"CALENDAR_ENCRYPTION_SECRET_NOT_CONFIGURED using ephemeral key. Tokens will not survive restart.");
		} else {
			byte[] keyBytes;
			try {
				keyBytes = Base64.getDecoder().decode(encryptionSecret.trim());
			} catch (IllegalArgumentException e) {
				throw new IllegalStateException("APP_CALENDAR_ENCRYPTION_SECRET is not valid Base64. "
						+ "Provide a Base64-encoded 32-byte key.", e);
			}
			if (keyBytes.length != 32) {
				throw new IllegalStateException("APP_CALENDAR_ENCRYPTION_SECRET decoded length is " + keyBytes.length
