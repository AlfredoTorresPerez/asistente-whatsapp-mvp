...
public class CalendarIntegrationJdbcRepository {
	...
	public List<CalendarIntegrationAccountRecord> findActiveByBusiness(UUID businessId) {
		return jdbcTemplate.query(
				...
				new MapSqlParameterSource("businessId", businessId), recordRowMapper());
	}
	...
	public List<CalendarIntegrationAccountRecord> findActiveByBusinessAndProvider(UUID businessId, String provider) {
		return jdbcTemplate.query(
				...
				new MapSqlParameterSource().addValue("businessId", businessId).addValue("provider", provider),
				recordRowMapper());
	}

	public Optional<CalendarIntegrationAccountRecord> findById(UUID id) {
		return jdbcTemplate.query("select * from calendar_integration_account where id = :id",
				new MapSqlParameterSource("id", id), recordRowMapper()).stream().findFirst();
	}

	public Optional<CalendarIntegrationAccountRecord> findByIdAndBusiness(UUID id, UUID businessId) {
		return jdbcTemplate
				.query("select * from calendar_integration_account where id = :id and business_id = :businessId",
						new MapSqlParameterSource().addValue("id", id).addValue("businessId", businessId),
						recordRowMapper())
				.stream().findFirst();
	}

	public Optional<CalendarIntegrationAccountRecord> findByBusinessAndProviderActive(UUID businessId,
			String provider) {
		return jdbcTemplate.query(
				"select * from calendar_integration_account where business_id = :businessId and provider = :provider and active = true",
				new MapSqlParameterSource().addValue("businessId", businessId).addValue("provider", provider),
	...
	public void save(CalendarIntegrationAccountRecord record) {
		jdbcTemplate.update(
				"""
						...
						""",
				new MapSqlParameterSource().addValue("id", record.id()).addValue("businessId", record.businessId())
						.addValue("provider", record.provider()).addValue("email", record.email())
						.addValue("accessTokenEncrypted", record.accessTokenEncrypted())
						.addValue("refreshTokenEncrypted", record.refreshTokenEncrypted())
						.addValue("tokenExpiresAt", record.tokenExpiresAt()).addValue("calendarId", record.calendarId())
						.addValue("calendarSummary", record.calendarSummary()).addValue("active", record.active())
						.addValue("connectedAt", record.connectedAt()).addValue("createdAt", record.createdAt())
						.addValue("updatedAt", record.updatedAt()).addValue("revokedAt", record.revokedAt())
	...
	public void updateCalendarId(UUID id, UUID businessId, String calendarId, String calendarSummary) {
		jdbcTemplate.update("""
				...
				""",
				new MapSqlParameterSource().addValue("id", id).addValue("businessId", businessId)
						.addValue("calendarId", calendarId).addValue("calendarSummary", calendarSummary)
	...
	public void updateTokens(UUID id, UUID businessId, String accessTokenEncrypted, String refreshTokenEncrypted,
		...
		jdbcTemplate.update("""
				...
				new MapSqlParameterSource().addValue("id", id).addValue("businessId", businessId)
						...
						.addValue("tokenExpiresAt", tokenExpiresAt).addValue("now", OffsetDateTime.now()));
	}
	...
	public void updateLastSyncAt(UUID id, UUID businessId, OffsetDateTime lastSyncAt) {
		jdbcTemplate.update("""
				...
				where id = :id and business_id = :businessId
				""", new MapSqlParameterSource().addValue("id", id).addValue("businessId", businessId)
				.addValue("lastSyncAt", lastSyncAt).addValue("now", OffsetDateTime.now()));
	}
	...
	public void updateRequiresReconnect(UUID id, UUID businessId, boolean requiresReconnect) {
		jdbcTemplate.update("""
				...
				where id = :id and business_id = :businessId
				""", new MapSqlParameterSource().addValue("id", id).addValue("businessId", businessId)
				.addValue("requiresReconnect", requiresReconnect).addValue("now", OffsetDateTime.now()));
	}
	...
	public void revokeAccount(UUID id, UUID businessId) {
		jdbcTemplate.update("""
				update calendar_integration_account set
				    ...
				    updated_at = :now
				where id = :id and business_id = :businessId
				""", new MapSqlParameterSource().addValue("id", id).addValue("businessId", businessId).addValue("now",
				OffsetDateTime.now()));
	}
	...
	public List<CalendarIntegrationAccountRecord> findAccountsNeedingSync(OffsetDateTime before, int limit) {
		return jdbcTemplate.query("""
				...
				limit :limit
				""", new MapSqlParameterSource().addValue("before", before).addValue("limit", limit),
				recordRowMapper());
	}
	...
	private RowMapper<CalendarIntegrationAccountRecord> recordRowMapper() {
		return (rs, rowNum) -> {
			...
			return new CalendarIntegrationAccountRecord(rs.getObject("id", UUID.class),
					rs.getObject("business_id", UUID.class), rs.getString("provider"), rs.getString("email"),
					rs.getString("access_token_encrypted"), rs.getString("refresh_token_encrypted"),
					rs.getObject("token_expires_at", OffsetDateTime.class), rs.getString("calendar_id"),
					rs.getString("calendar_summary"), rs.getBoolean("active"),
					rs.getObject("connected_at", OffsetDateTime.class),
					rs.getObject("created_at", OffsetDateTime.class), rs.getObject("updated_at", OffsetDateTime.class),
					rs.getObject("revoked_at", OffsetDateTime.class),
					rs.getObject("last_sync_at", OffsetDateTime.class), requiresReconnect);
		};
	}

	public record CalendarIntegrationAccountRecord(UUID id, UUID businessId, String provider, String email,
			String accessTokenEncrypted, String refreshTokenEncrypted, OffsetDateTime tokenExpiresAt, String calendarId,
			String calendarSummary, boolean active, OffsetDateTime connectedAt, OffsetDateTime createdAt,
			OffsetDateTime updatedAt, OffsetDateTime revokedAt, OffsetDateTime lastSyncAt, boolean requiresReconnect) {
	}
}
