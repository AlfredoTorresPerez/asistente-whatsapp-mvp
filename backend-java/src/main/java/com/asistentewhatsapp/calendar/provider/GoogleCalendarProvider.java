...
public class GoogleCalendarProvider implements CalendarProvider {
    ...
    public String getAuthUrl(String state, String redirectUri) {
        ...
        return AUTH_URL
                ...
                + "&redirect_uri=" + urlEncode(redirectUri != null ? redirectUri : this.redirectUri)
                + "&response_type=code"
    ...
    public TokenExchangeResult exchangeCode(String code, String effectiveRedirectUri) {
        ...
        GoogleCalendarHttpClient.TokenResponse tokenResponse = httpClient.exchangeAuthorizationCode(
                code, usedRedirectUri, clientId, clientSecret);
    ...
    public RefreshResult refreshAccessToken(String refreshToken) {
        ...
        GoogleCalendarHttpClient.RefreshTokenResponse response = httpClient.refreshAccessToken(
    ...
    public UserInfoResult getUserInfo(String accessToken) {
        ...
        return new UserInfoResult(
                userInfo.id(),
                userInfo.email(),
                userInfo.verifiedEmail() != null && userInfo.verifiedEmail(),
                userInfo.name());
    }
    ...
    public String createEvent(String calendarId, CalendarEventData eventData, String accessToken) {
        ...
        try {
            ...
            throw new GoogleCalendarApiException(0, "Failed to parse createEvent response: " + e.getMessage(), response);
        }
    }
    ...
    public CalendarEventData getEvent(String calendarId, String externalEventId, String accessToken) {
        ...
        try {
            ...
            String timezone = root.has("start") && root.get("start").has("timeZone")
                    ? root.get("start").get("timeZone").asText() : null;
            String attendeeEmail = null;
            String attendeeName = null;
            if (root.has("attendees") && root.get("attendees").isArray()
                    && root.get("attendees").size() > 0) {
            ...
            return new CalendarEventData(
                    summary, description, location, startAt, endAt, timezone,
                    attendeeEmail, attendeeName, businessId, bookingId, externalEventId);
        } catch (GoogleCalendarApiException e) {
            throw e;
        } catch (Exception e) {
    ...
    private OffsetDateTime parseDateTime(JsonNode parent, String field) {
        ...
        if (!node.has("dateTime")) return null;
        try {
