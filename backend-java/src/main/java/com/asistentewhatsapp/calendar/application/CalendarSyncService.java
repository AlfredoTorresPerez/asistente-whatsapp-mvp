...
public class CalendarSyncService {
            ...
            if (existing.isPresent()) {
                ...
                if ("SYNCED".equals(currentStatus) && "DELETE".equals(syncAction)) {
                    ...
                    if (currentEventId != null) {
                        BookingCalendarSyncRecord updated = new BookingCalendarSyncRecord(
                                rec.id(), rec.bookingId(), rec.businessId(), rec.provider(),
                                currentEventId, "PENDING", "UPDATE", null,
                                0, null, rec.lastSuccessfulSyncAt(),
                    ...
                    LOGGER.debug("CALENDAR_SYNC_ALREADY_SYNCED bookingId={} provider={} eventId={}",
                            bookingId, account.provider(), currentEventId);
                } else if ("FAILED".equals(currentStatus)) {
                    BookingCalendarSyncRecord updated = new BookingCalendarSyncRecord(
                            rec.id(), rec.bookingId(), rec.businessId(), rec.provider(),
                            currentEventId, "PENDING", syncAction, null,
                            rec.retryCount(), rec.lastSyncAttemptAt(), rec.lastSuccessfulSyncAt(),
                            rec.createdAt(), OffsetDateTime.now());
                    syncRepository.insert(updated);
                } else if ("PENDING".equals(currentStatus)) {
                    LOGGER.debug("CALENDAR_SYNC_ALREADY_PENDING bookingId={} provider={}",
                            bookingId, account.provider());
                    ...
                    LOGGER.debug("CALENDAR_SYNC_SKIP bookingId={} provider={} status={}",
                            bookingId, account.provider(), currentStatus);
                }
            } else {
        ...
        try {
                ...
                if (refreshToken != null) {
                    try {
                        ...
                        OffsetDateTime newExpiry = refreshResult.expiresInSeconds() != null
                                ...
                                : null;
                        accountRepository.updateTokens(account.id(), account.businessId(),
                                newEncryptedAccess, account.refreshTokenEncrypted(), newExpiry);
                        accessToken = refreshResult.accessToken();
                        tokenRefreshed = true;
                        LOGGER.info("CALENDAR_TOKEN_REFRESHED accountId={}", account.id());
                    } catch (GoogleCalendarApiException e) {
                        LOGGER.warn("CALENDAR_TOKEN_REFRESH_FAILED accountId={} reason={}", account.id(), e.getMessage());
            ...
            try {
                if ("DELETE".equals(action)) {
                    ...
                    LOGGER.info("CALENDAR_SYNC_CANCELLED syncId={} provider={} eventId={}",
                    ...
                    if (externalEventId != null) {
                        try {
                            ...
                            if (e.isNotFound()) {
                                ...
                                String newEventId = computeIdempotentId(
                                        eventData.businessId(), eventData.bookingId());
                                CalendarEventData updatedEvent = new CalendarEventData(
                                        eventData.summary(), eventData.description(), eventData.location(),
                                        eventData.startAt(), eventData.endAt(), eventData.timezone(),
                                        eventData.attendeeEmail(), eventData.attendeeName(),
                                        eventData.businessId(), eventData.bookingId(), newEventId);
                    ...
                    CalendarEventData newEvent = new CalendarEventData(
                            eventData.summary(), eventData.description(), eventData.location(),
                            eventData.startAt(), eventData.endAt(), eventData.timezone(),
                            eventData.attendeeEmail(), eventData.attendeeName(),
                            eventData.businessId(), eventData.bookingId(), idempotentId);
                    try {
    ...
    private void handleSyncError(BookingCalendarSyncRecord record,
            CalendarIntegrationAccountRecord account, GoogleCalendarApiException e) {
        String msg = e.getMessage();
        LOGGER.warn("CALENDAR_SYNC_ERROR syncId={} provider={} statusCode={} reason={}",
                record.id(), record.provider(), e.getStatusCode(), msg);
        ...
        if (e.isAuthError()) {
            if (hasRefreshToken(account)) {
                try {
                    ...
                    account = accountRepository.findByIdAndBusiness(account.id(), account.businessId())
                            .orElse(account);
                    retryWithNewToken(record, provider, account, newAccessToken, e);
                    return;
                } catch (Exception refreshError) {
                    LOGGER.warn("CALENDAR_TOKEN_REFRESH_FAILED accountId={} reason={}",
                            account.id(), refreshError.getMessage());
                }
            }
    ...
    private void retryWithNewToken(BookingCalendarSyncRecord record,
            CalendarProvider provider, CalendarIntegrationAccountRecord account,
            String newAccessToken, GoogleCalendarApiException originalError) {
        try {
            String externalEventId = record.externalEventId();
            String action = record.syncAction();
            String calendarId = account.calendarId();
            ...
            if ("DELETE".equals(action) && externalEventId != null) {
                ...
                provider.updateEvent(calendarId, externalEventId, loadBookingEventData(
                        record.bookingId(), record.businessId()), newAccessToken);
                syncRepository.updateSyncSuccess(record.id(), externalEventId);
            ...
            String retryMsg = retryError.getMessage() != null ? retryError.getMessage() : retryError.getClass().getSimpleName();
    ...
    private void markRequiresReconnect(CalendarIntegrationAccountRecord account) {
        try {
            ...
            LOGGER.warn("CALENDAR_ACCOUNT_REQUIRES_RECONNECT accountId={} provider={}",
    ...
    private String computeIdempotentId(UUID businessId, UUID bookingId) {
        if (businessId == null || bookingId == null) return null;
