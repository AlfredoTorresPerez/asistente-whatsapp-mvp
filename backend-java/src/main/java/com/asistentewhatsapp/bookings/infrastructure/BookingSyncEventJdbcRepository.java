...
public class BookingSyncEventJdbcRepository {
	...
	public boolean enqueue(UUID eventId, UUID bookingId, UUID businessId, String eventType, int eventVersion,
			String eventBody, String idempotencyKey, int maxAttempts, OffsetDateTime nextAttemptAt, String traceId) {
		try {
			jdbcTemplate.update(
					"""
							...
							""",
					eventId, bookingId, businessId, eventType, eventVersion, eventBody, idempotencyKey, maxAttempts,
	...
	public List<BookingSyncEventRecord> claimDueEvents(int limit, long processingTimeoutMs) {
		return jdbcTemplate.query("""
				...
				""", (rs, rowNum) -> new BookingSyncEventRecord(rs.getObject("id", UUID.class),
				rs.getObject("booking_id", UUID.class), rs.getObject("business_id", UUID.class),
				rs.getString("event_type"), rs.getInt("event_version"), rs.getString("event_body"),
				rs.getString("idempotency_key"), rs.getString("status"), rs.getInt("attempts"),
				rs.getInt("max_attempts"), rs.getObject("next_attempt_at", OffsetDateTime.class),
				rs.getObject("locked_at", OffsetDateTime.class), rs.getString("last_error_code"),
				rs.getString("last_error_message"), rs.getString("trace_id"),
				rs.getObject("created_at", OffsetDateTime.class), rs.getObject("updated_at", OffsetDateTime.class)),
				processingTimeoutMs, limit);
	}
	...
	public void markFailedOrRetry(UUID eventId, int attempts, int maxAttempts, String errorCode, String errorMessage,
