...
public class BookingPaymentJdbcRepository {
    ...
    public BookingPaymentBookingRecord findBookingForUpdate(UUID businessId, UUID bookingId) {
        List<BookingPaymentBookingRecord> items = jdbcTemplate.query(
                ...
                bookingRowMapper());
        if (items.isEmpty()) throw new ResourceNotFoundException("No se encontro la reserva asociada al pago.");
    ...
    public BookingPaymentNotificationRecord findNotificationContext(UUID businessId, UUID bookingId) {
        List<BookingPaymentNotificationRecord> items = jdbcTemplate.query(
                ...
                new MapSqlParameterSource()
                        .addValue("businessId", businessId)
                        .addValue("bookingId", bookingId),
                (rs, rowNum) -> new BookingPaymentNotificationRecord(
    ...
    public Optional<BookingPaymentRecord> findExisting(
            UUID businessId, String provider,
            String providerPaymentId, String providerExternalReference,
        ...
        if (!hasCondition) return Optional.empty();
    ...
    public BookingPaymentRecord findByProviderPreferenceId(String provider, String preferenceId) {
        List<BookingPaymentRecord> items = jdbcTemplate.query(
                ...
                new MapSqlParameterSource()
                        .addValue("provider", provider)
                        .addValue("preferenceId", preferenceId),
                paymentRowMapper());
        return items.isEmpty() ? null : items.getFirst();
    }
    ...
    public BookingPaymentRecord insertPayment(
        ...
        try {
            jdbcTemplate.update(
                    ...
                    """,
                    paymentParams(paymentId, businessId, bookingId, provider,
                            providerPaymentId, providerPreferenceId, providerExternalReference,
                            idempotencyKey, amount, currency, status, rawPayload, metadata,
                            occurredAt, statusDetail, rawStatus, paymentMethodId, installments, payerEmail));
        } catch (DuplicateKeyException e) {
            BookingPaymentRecord existing = findExisting(businessId, provider,
                    providerPaymentId, providerExternalReference, idempotencyKey)
                    .orElseThrow(() -> e);
            return existing;
        }
    ...
    public BookingPaymentRecord insertManualPayment(
            UUID businessId, UUID bookingId, String provider,
            String providerPaymentId, String idempotencyKey,
            BigDecimal amount, String currency, String status,
            String rawPayload, String metadata, OffsetDateTime occurredAt) {
        UUID paymentId = UUID.randomUUID();
        try {
            jdbcTemplate.update(
                    ...
                    """,
                    paymentParams(paymentId, businessId, bookingId, provider,
                            providerPaymentId, null, null,
                            idempotencyKey, amount, currency, status, rawPayload, metadata,
                            occurredAt, null, null, null, null, null));
        } catch (DuplicateKeyException e) {
            return findExisting(businessId, provider, providerPaymentId, null, idempotencyKey)
                    .orElseThrow(() -> e);
        }
        return findById(paymentId);
    }
    ...
    public BookingPaymentRecord insertPaymentFromCheckout(
            ...
            String providerPaymentId, String providerPreferenceId,
            String providerExternalReference, String idempotencyKey,
            BigDecimal amount, String currency, String paymentPurpose,
            String checkoutUrl, OffsetDateTime checkoutExpiresAt,
            String metadata, UUID paymentId) {
        String resolvedCheckoutUrl = checkoutUrl.contains("{paymentId}")
                ? checkoutUrl.replace("{paymentId}", paymentId.toString())
    ...
    public BookingPaymentRecord updatePaymentProviderStatus(
            UUID paymentId, String status, String rawPayload, String metadata,
    ...
    public PublicBookingPaymentDetailRecord findPaymentDetail(UUID paymentId) {
        List<PublicBookingPaymentDetailRecord> items = jdbcTemplate.query(
                ...
                (rs, rowNum) -> new PublicBookingPaymentDetailRecord(
                        rs.getObject("id", UUID.class), rs.getObject("booking_id", UUID.class),
                        rs.getString("provider"), rs.getString("provider_payment_id"),
                        rs.getBigDecimal("amount"), rs.getString("currency"), rs.getString("status"),
                        rs.getString("checkout_url"),
                        rs.getObject("checkout_expires_at", OffsetDateTime.class),
                        ...
                        rs.getString("booking_status"),
                        rs.getString("booking_payment_status"),
                        rs.getString("subject"), rs.getString("service_name"),
                        rs.getString("professional_name"), rs.getString("room_name"),
                        rs.getObject("starts_at", OffsetDateTime.class),
                        rs.getInt("duration_minutes"),
                        rs.getString("location_name"),
                        rs.getString("customer_name")));
        if (items.isEmpty()) throw new ResourceNotFoundException("No se encontro el pago de reserva.");
        return items.getFirst();
    }
    ...
    public Optional<BookingPaymentRecord> findByProviderExternalReferenceForUpdate(String provider, String externalRef) {
        List<BookingPaymentRecord> items = jdbcTemplate.query(
                "select id, business_id, booking_id, provider, provider_payment_id, "
                + "provider_preference_id, provider_external_reference, "
                + "idempotency_key, amount, currency, status, "
    ...
    private MapSqlParameterSource paymentParams(
            UUID paymentId, UUID businessId, UUID bookingId, String provider,
            String providerPaymentId, String providerPreferenceId,
            String providerExternalReference, String idempotencyKey,
            BigDecimal amount, String currency, String status,
            String rawPayload, String metadata, OffsetDateTime occurredAt,
    ...
    private RowMapper<BookingPaymentBookingRecord> bookingRowMapper() {
        return (rs, rowNum) -> new BookingPaymentBookingRecord(
                rs.getObject("booking_id", UUID.class),
                rs.getObject("business_id", UUID.class),
                rs.getString("booking_status"),
    ...
    private RowMapper<BookingPaymentRecord> paymentRowMapper() {
        return (rs, rowNum) -> new BookingPaymentRecord(
                rs.getObject("id", UUID.class),
                rs.getObject("business_id", UUID.class),
                rs.getObject("booking_id", UUID.class),
                rs.getString("provider"),
                rs.getString("provider_payment_id"),
                rs.getString("provider_preference_id"),
                rs.getString("provider_external_reference"),
    ...
    public record BookingPaymentBookingRecord(
            UUID bookingId, UUID businessId, String bookingStatus,
            boolean requiresDeposit, BigDecimal depositAmount, String paymentStatus) {}

    public record BookingPaymentNotificationRecord(
            UUID bookingId, UUID businessId, String subject, String bookingStatus,
            OffsetDateTime startsAt, int durationMinutes, String location,
    ...
    public record BookingPaymentRecord(
            UUID id, UUID businessId, UUID bookingId, String provider,
            String providerPaymentId, String providerPreferenceId,
            String providerExternalReference, String idempotencyKey,
            BigDecimal amount, String currency, String status,
            String paymentPurpose,
            OffsetDateTime approvedAt, OffsetDateTime rejectedAt,
            OffsetDateTime expiredAt, OffsetDateTime refundedAt,
            String checkoutUrl, OffsetDateTime checkoutExpiresAt,
            boolean manual, OffsetDateTime createdAt) {}

    public record PublicBookingPaymentDetailRecord(
            UUID id, UUID bookingId, String provider, String providerPaymentId,
            BigDecimal amount, String currency, String status,
            String checkoutUrl, OffsetDateTime checkoutExpiresAt, boolean manual,
            OffsetDateTime approvedAt, OffsetDateTime rejectedAt,
            OffsetDateTime expiredAt, OffsetDateTime refundedAt, OffsetDateTime createdAt,
            String bookingStatus, String bookingPaymentStatus, String subject,
            String serviceName, String professionalName, String roomName,
            OffsetDateTime startsAt, int durationMinutes, String locationName,
            String customerName) {}
}