...
public class BookingActionLinkJdbcRepository {
	...
	public ActionBookingRecord findBooking(UUID businessId, UUID bookingId) {
		List<ActionBookingRecord> items = queryBooking("where b.business_id = :businessId and b.id = :bookingId",
	...
	public UUID insertRescheduleLink(UUID businessId, UUID bookingId, String tokenHash, String publicUrl,
		...
		jdbcTemplate.update("""
				...
				""",
				params(businessId, bookingId).addValue("id", id).addValue("tokenHash", tokenHash)
						.addValue("publicUrl", publicUrl).addValue("proposedStartsAt", proposedStartsAt)
						.addValue("proposedEndsAt", proposedEndsAt).addValue("locationId", locationId)
						.addValue("serviceId", serviceId).addValue("professionalId", professionalId)
						.addValue("roomId", roomId).addValue("expiresAt", expiresAt).addValue("reason", reason)
						.addValue("actorUserId", actorUserId).addValue("createdByChannel", createdByChannel));
		return id;
	}
	...
	public UUID insertCancellationLink(UUID businessId, UUID bookingId, String tokenHash, String publicUrl,
		...
		jdbcTemplate.update("""
				...
				""",
				params(businessId, bookingId).addValue("id", id).addValue("tokenHash", tokenHash)
						.addValue("publicUrl", publicUrl).addValue("expiresAt", expiresAt).addValue("reason", reason)
						.addValue("actorUserId", actorUserId).addValue("createdByChannel", createdByChannel));
		jdbcTemplate.update("""
				update booking
	...
	public RescheduleLinkRecord findRescheduleByTokenHash(String tokenHash, boolean forUpdate) {
		...
		List<RescheduleLinkRecord> items = jdbcTemplate.query(rescheduleSql("where l.token_hash = :tokenHash" + lock),
				new MapSqlParameterSource().addValue("tokenHash", tokenHash), rescheduleMapper());
		if (items.isEmpty()) {
	...
	public CancellationLinkRecord findCancellationByTokenHash(String tokenHash, boolean forUpdate) {
		String lock = forUpdate ? " for update of l, b" : "";
		List<CancellationLinkRecord> items = jdbcTemplate.query(
				cancellationSql("where l.token_hash = :tokenHash" + lock),
				new MapSqlParameterSource().addValue("tokenHash", tokenHash), cancellationMapper());
		if (items.isEmpty()) {
	...
	private List<ActionBookingRecord> queryBooking(String whereClause, MapSqlParameterSource params) {
		return jdbcTemplate
				.query("""
						...
						"""
						+ whereClause, params, bookingMapper());
	}
	...
	private RowMapper<ActionBookingRecord> bookingMapper() {
		return (rs, rowNum) -> new ActionBookingRecord(rs.getObject("booking_id", UUID.class),
				rs.getObject("business_id", UUID.class), rs.getString("subject"), rs.getString("booking_status"),
				...
				rs.getObject("room_id", UUID.class), rs.getString("room_name"),
				rs.getObject("conversation_id", UUID.class), rs.getInt("reschedule_count"),
	...
	private RowMapper<RescheduleLinkRecord> rescheduleMapper() {
		return (rs, rowNum) -> new RescheduleLinkRecord(rs.getObject("link_id", UUID.class),
				rs.getObject("business_id", UUID.class), rs.getObject("booking_id", UUID.class),
				rs.getString("link_status"), rs.getString("reschedule_url"),
				rs.getObject("proposed_starts_at", OffsetDateTime.class),
				rs.getObject("proposed_ends_at", OffsetDateTime.class),
				rs.getObject("proposed_location_id", UUID.class), rs.getObject("proposed_service_id", UUID.class),
				rs.getObject("proposed_professional_id", UUID.class), rs.getObject("proposed_room_id", UUID.class),
				rs.getObject("expires_at", OffsetDateTime.class), rs.getObject("used_at", OffsetDateTime.class),
				rs.getString("reason"), rs.getString("subject"), rs.getString("booking_status"),
				rs.getObject("current_starts_at", OffsetDateTime.class),
				rs.getObject("current_ends_at", OffsetDateTime.class), rs.getString("current_location_name"),
				rs.getString("current_service_name"), rs.getString("proposed_service_name"),
				rs.getString("current_professional_name"), rs.getString("proposed_professional_name"),
				rs.getString("current_room_name"), rs.getString("proposed_room_name"),
				rs.getString("proposed_location_name"), rs.getString("customer_name"), rs.getString("customer_phone"),
				rs.getString("customer_email"));
	}

	private RowMapper<CancellationLinkRecord> cancellationMapper() {
		return (rs, rowNum) -> new CancellationLinkRecord(rs.getObject("link_id", UUID.class),
				rs.getObject("business_id", UUID.class), rs.getObject("booking_id", UUID.class),
				rs.getString("link_status"), rs.getString("cancellation_url"),
				rs.getObject("expires_at", OffsetDateTime.class), rs.getObject("used_at", OffsetDateTime.class),
				rs.getString("cancellation_reason"), rs.getString("subject"), rs.getString("booking_status"),
				rs.getObject("starts_at", OffsetDateTime.class), rs.getObject("ends_at", OffsetDateTime.class),
				rs.getString("location_name"), rs.getString("service_name"), rs.getString("professional_name"),
				rs.getString("room_name"), rs.getString("customer_name"), rs.getString("customer_phone"),
				rs.getString("customer_email"));
	}
	...
	public record ActionBookingRecord(UUID bookingId, UUID businessId, String subject, String bookingStatus,
			...
			UUID serviceId, String serviceName, UUID professionalId, String professionalName, UUID roomId,
			String roomName, UUID conversationId, int rescheduleCount, String customerName, String customerPhone,
			String customerEmail) {
	}

	public record RescheduleLinkRecord(UUID linkId, UUID businessId, UUID bookingId, String linkStatus,
			String publicUrl, OffsetDateTime proposedStartsAt, OffsetDateTime proposedEndsAt, UUID proposedLocationId,
			UUID proposedServiceId, UUID proposedProfessionalId, UUID proposedRoomId, OffsetDateTime expiresAt,
			OffsetDateTime usedAt, String reason, String subject, String bookingStatus, OffsetDateTime currentStartsAt,
			OffsetDateTime currentEndsAt, String currentLocationName, String currentServiceName,
			String proposedServiceName, String currentProfessionalName, String proposedProfessionalName,
			String currentRoomName, String proposedRoomName, String proposedLocationName, String customerName,
			String customerPhone, String customerEmail) {
	}

	public record CancellationLinkRecord(UUID linkId, UUID businessId, UUID bookingId, String linkStatus,
			String publicUrl, OffsetDateTime expiresAt, OffsetDateTime usedAt, String cancellationReason,
			String subject, String bookingStatus, OffsetDateTime startsAt, OffsetDateTime endsAt, String locationName,
