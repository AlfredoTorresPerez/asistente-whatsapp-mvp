...
public class MercadoPagoPaymentProvider implements BookingPaymentProvider {
    ...
    public CreateCheckoutResult createCheckout(
        ...
        return new CreateCheckoutResult(
                checkoutUrl,
                null, // providerPaymentId (se obtiene del webhook cuando se aprueba)
                preferenceId,
                externalRef,
                metadata,
                expiresAt);
    ...
    public Optional<PaymentNotification> parseWebhook(String rawBody, Map<String, String> headers) {
        ...
        try {
            ...
            // Notification type "payment" with action "payment.created" or "payment.updated"
    ...
    private PaymentNotification fetchPaymentInfo(long mpPaymentId) {
        ResponseEntity<JsonNode> response = restClient.get()
                .uri(API_BASE + "/v1/payments/" + mpPaymentId)
                .header("Authorization", "Bearer " + accessToken)
                .retrieve()
                .toEntity(JsonNode.class);
        if (!response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
    ...
    private PaymentNotification normalizePaymentNotification(JsonNode payment) {
        ...
        String payerEmail = payment.has("payer") && payment.get("payer").has("email")
                ? payment.get("payer").get("email").asText() : null;
    ...
    public RefundResult refund(UUID businessId, UUID bookingId, UUID paymentId,
        ...
        Map<String, Object> payload = new LinkedHashMap<>();
        if (amount != null) payload.put("amount", amount.doubleValue());
