...
public class BookingReminderService {
	...
	public BookingReminderService(CompleteAgendaJdbcRepository agendaRepository,
			ChannelDispatchService channelDispatchService, BookingEmailService bookingEmailService,
			AuditService auditService, EmailTemplateRenderer templateRenderer, BookingReminderProperties properties,
	...
	public void processDueReminders() {
		...
		List<DueReminderRecord> claimed = agendaRepository.claimDueReminders(OffsetDateTime.now(ZoneOffset.UTC),
				properties.getBatchSize(), INSTANCE_ID);
	...
	private void processReminder(DueReminderRecord reminder) {
		...
		if (!BookingStateMachine.CONFIRMED.equals(bookingStatus)
			...
			auditService.record(reminder.businessId(), null, "BOOKING_REMINDER_SKIPPED", "BOOKING",
					reminder.bookingId(), "Recordatorio saltado: cita " + reminder.bookingStatus());
			return;
		}
		try {
	...
	private void handleRetry(DueReminderRecord reminder, RuntimeException exception) {
		...
		if (currentAttempt >= maxAttempts) {
			...
			LOG.warn("BOOKING_REMINDER_MAX_RETRIES reminderId={} bookingId={} attempts={} errorCode={}", reminder.id(),
					reminder.bookingId(), currentAttempt, errorCode);
			return;
		}
		...
		auditService.record(reminder.businessId(), null, "BOOKING_REMINDER_RETRY", "BOOKING", reminder.bookingId(),
				"Reintento " + (currentAttempt + 1) + "/" + maxAttempts + " en " + delayMinutes + "min: " + message);
	...
	private String classifyError(Throwable exception) {
		...
		if (msgLower.contains("timeout") || msgLower.contains("timed out"))
			return "TIMEOUT";
		if (msgLower.contains("connection refused") || msgLower.contains("connect refused"))
			return "CONNECTION_REFUSED";
		if (msgLower.contains("mail server") || msgLower.contains("smtp"))
			return "SMTP_ERROR";
		if (msgLower.contains("quota") || msgLower.contains("rate limit"))
			return "RATE_LIMIT";
		if (msgLower.contains("unreachable") || msgLower.contains("network"))
			return "NETWORK_ERROR";
		return "TEMPORARY_ERROR";
	}

	private void sendWhatsAppReminder(DueReminderRecord reminder) {
		channelDispatchService.dispatch(new ChannelDispatchRequest(reminder.businessId(), MessageChannelType.WHATSAPP,
				reminder.customerPhone(), buildWhatsAppBody(reminder)));
	}
	...
	private void sendEmailReminder(DueReminderRecord reminder) {
		...
		try {
			transactionalEmailService.sendReminderEmail(reminder.customerEmail(), buildSubject(reminder), textBody,
					htmlBody);
		} catch (Exception e) {
			LOG.warn("BOOKING_REMINDER_TRANSACTIONAL_EMAIL_FAILED bookingId={} reason={}", reminder.bookingId(),
					e.getMessage());
		}
	}

	private String buildSubject(DueReminderRecord reminder) {
		String businessName = reminder.locationName() != null ? reminder.locationName() : "nuestra clinica";
	...
	private String build24hReminderHtml(DueReminderRecord reminder) {
		try {
			...
			LOG.warn("BOOKING_REMINDER_24H_HTML_RENDER_FAILED bookingId={} reason={}", reminder.bookingId(),
					e.getMessage());
	...
	private String buildWhatsAppBody(DueReminderRecord reminder) {
		return "Recordatorio de cita\n\n" + "Hola " + valueOrFallback(reminder.customerName(), "cliente") + ".\n"
				...
				+ valueOrFallback(reminder.locationName(), "Por confirmar") + "\n" + "Profesional: "
				+ valueOrFallback(reminder.professionalName(), "Por asignar");
	}
	...
	private String buildFallbackText(DueReminderRecord reminder) {
		return "Recordatorio: tu cita es manana\n\n" + "Hola " + valueOrFallback(reminder.customerName(), "cliente")
				...
				+ valueOrFallback(reminder.serviceName(), reminder.subject()) + "\n" + "Fecha: "
				+ formatDateTime(reminder.startsAt()) + "\n" + "Sucursal: "
				+ valueOrFallback(reminder.locationName(), "Por confirmar") + "\n" + "Profesional: "
				+ valueOrFallback(reminder.professionalName(), "Por asignar") + "\n\n"
