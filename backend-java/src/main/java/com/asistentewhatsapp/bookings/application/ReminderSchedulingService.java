...
public class ReminderSchedulingService {
	...
	private int nextRevision(UUID businessId, UUID bookingId) {
		Integer max = agendaRepository.findMaxReminderRevision(businessId, bookingId, "TWENTY_FOUR_HOURS_BEFORE",
				"EMAIL");
	...
	private void schedule24h(UUID businessId, UUID bookingId, OffsetDateTime startsAt, int revision) {
		...
		agendaRepository.insertReminderWithRevision(businessId, bookingId, "TWENTY_FOUR_HOURS_BEFORE", "WHATSAPP",
				scheduledAt, revision);
		agendaRepository.insertReminderWithRevision(businessId, bookingId, "TWENTY_FOUR_HOURS_BEFORE", "EMAIL",
				scheduledAt, revision);
	}
	...
	private void schedule2h(UUID businessId, UUID bookingId, OffsetDateTime startsAt, int revision) {
		...
		agendaRepository.insertReminderWithRevision(businessId, bookingId, "TWO_HOURS_BEFORE", "WHATSAPP", scheduledAt,
				revision);
		agendaRepository.insertReminderWithRevision(businessId, bookingId, "TWO_HOURS_BEFORE", "EMAIL", scheduledAt,
				revision);
	}
}
