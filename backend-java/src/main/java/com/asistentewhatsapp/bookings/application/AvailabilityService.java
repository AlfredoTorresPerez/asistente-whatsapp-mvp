...
public class AvailabilityService {
    ...
    public void checkProfessionalAbsence(UUID businessId, UUID professionalId, OffsetDateTime startsAt, OffsetDateTime endsAt) {
        if (repository.hasActiveAbsence(businessId, professionalId, startsAt, endsAt)) {
            LOGGER.warn("[diagnostico] Ausencia activa: businessId={} professionalId={} startsAt={}", businessId, professionalId, startsAt);
    ...
    public void checkCustomerDuplicateActiveBooking(UUID businessId, UUID customerId, UUID professionalId,
        ...
        int count = repository.countCustomerActiveOverlappingBookingsExcluding(
                businessId, customerId, professionalId, startsAt, endsAt, excludeBookingId);
        if (count > 0) {
            LOGGER.warn("[diagnostico] Duplicado de cliente: customerId={} professionalId={} startsAt={}", customerId, professionalId, startsAt);
            throw conflict("CUSTOMER_DUPLICATE_BOOKING",
                    "El cliente ya tiene una reserva activa en el mismo horario.",
                    Map.of("startsAt", "El cliente ya tiene una reserva en este horario."));
        }
    }

    public void checkMinAdvanceNotice(UUID businessId, UUID locationId, UUID professionalId, OffsetDateTime startsAt, ZoneId zone) {
        OffsetDateTime now = OffsetDateTime.now(zone);
        long minutesAhead = Duration.between(now, startsAt).toMinutes();
        if (minutesAhead < 0) {
            throw conflict("BOOKING_IN_PAST",
    ...
    private ApiException conflict(String code, String message, Map<String, String> fieldErrors) {
        LOGGER.warn("[diagnostico] Conflicto de disponibilidad: code={} message={} fieldErrors={}", code, message, fieldErrors);
