...
public class BookingFactRepository {
    ...
    public List<BookingFact> findActiveByPhone(UUID businessId, String phone) {
        return jdbcTemplate.query("""
                ...
                limit 20
                """,
                (rs, rowNum) -> new BookingFact(
                        rs.getObject("booking_id", UUID.class),
                        rs.getObject("business_id", UUID.class),
                        rs.getString("customer_phone"),
                        rs.getString("customer_name"),
                        rs.getString("customer_management_id"),
                        rs.getString("service_name"),
                        rs.getString("location_name"),
                        rs.getString("professional_name"),
                        Optional.ofNullable(rs.getObject("booking_date", java.sql.Date.class))
                                .map(java.sql.Date::toLocalDate).orElse(null),
                        Optional.ofNullable(rs.getObject("booking_time", java.sql.Time.class))
                                .map(java.sql.Time::toLocalTime).orElse(null),
                        rs.getString("booking_status"),
                        Optional.ofNullable(rs.getObject("conversation_id", UUID.class)).orElse(null),
                        rs.getString("channel_origin"),
                        rs.getString("origin_intent"),
                        rs.getBoolean("tiene_reserva_activa"),
                        rs.getObject("booking_created_at", OffsetDateTime.class)),
                businessId, phone, phone);
    }
    ...
    public record BookingFact(
            ...
            OffsetDateTime bookingCreatedAt) {
    }
}
