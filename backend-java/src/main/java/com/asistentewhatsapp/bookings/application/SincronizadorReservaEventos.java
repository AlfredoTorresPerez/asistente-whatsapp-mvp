...
public class SincronizadorReservaEventos implements SincronizadorReservaMotorReglas {
	...
	public SincronizadorReservaEventos(BookingSyncEventJdbcRepository eventRepository,
	...
	public void sincronizarReserva(UUID businessId, UUID bookingId, String customerPhone, String customerName,
			...
			String traceId) {
		String eventBody = buildEventBody(bookingId, businessId, customerPhone, customerName, serviceName, locationName,
				professionalName, startsAt, durationMinutes, bookingStatus, conversationId, channelOrigin,
				originIntent);
		String idempotencyKey = "RESERVA_CREADA_" + bookingId + "_"
				+ (startsAt != null ? startsAt.toEpochSecond() : "");
		OffsetDateTime nextAttemptAt = OffsetDateTime.now(ZoneOffset.UTC);

		boolean enqueued = eventRepository.enqueue(UUID.randomUUID(), bookingId, businessId, "RESERVA_CREADA", 1,
				eventBody, idempotencyKey, properties.getEventMaxAttempts(), nextAttemptAt, traceId);

		if (enqueued) {
			log.info("BOOKING_SYNC_EVENT_ENQUEUED bookingId={} eventType=RESERVA_CREADA idempotencyKey={}", bookingId,
					idempotencyKey);
		} else {
	...
	private String buildEventBody(UUID bookingId, UUID businessId, String customerPhone, String customerName,
			String serviceName, String locationName, String professionalName, OffsetDateTime startsAt,
			int durationMinutes, String bookingStatus, UUID conversationId, String channelOrigin, String originIntent) {
		return String.format(
				"""
						...
						""",
				safeJson(bookingId.toString()), safeJson(businessId.toString()), safeJson(customerName),
				safeJson(serviceName), safeJson(locationName), safeJson(professionalName),
				startsAt != null ? startsAt.toString() : "", durationMinutes, safeJson(bookingStatus),
				conversationId != null ? conversationId.toString() : "", safeJson(channelOrigin),
