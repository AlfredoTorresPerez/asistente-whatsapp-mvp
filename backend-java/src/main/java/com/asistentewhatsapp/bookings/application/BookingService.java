...
public class BookingService {
	...
	public PagedResponse<BookingSummaryResponse> list(AuthenticatedUser authenticatedUser, int page, int size,
			OffsetDateTime from, OffsetDateTime to, String search, String status, UUID responsibleUserId) {
		int resolvedPage = Math.max(page, 0);
		int resolvedSize = Math.min(Math.max(size, 1), 500);
		DateRange dateRange = resolveDateRange(from, to);

		return bookingJdbcRepository.findBookings(authenticatedUser.businessId(), resolvedPage, resolvedSize,
				dateRange.from(), dateRange.to(), normalizeSearch(search), normalizeOptionalStatus(status),
	...
	public BookingDetailResponse create(AuthenticatedUser authenticatedUser, CreateBookingRequest request) {
		...
		LocationResolution location = resolveLocation(authenticatedUser.businessId(), request.locationId(),
				normalizeOptionalText(request.location(), "location", 160), null, true);
		...
		availabilityService.checkMinAdvanceNotice(authenticatedUser.businessId(), location.locationId(), professionalId,
				startsAt, zone);
		if (location.locationId() != null && professionalId != null) {
			OffsetDateTime endsAt = startsAt.plusMinutes(durationMinutes);
			availabilityService.checkProfessionalAbsence(authenticatedUser.businessId(), professionalId, startsAt,
					endsAt);
			availabilityService.checkProfessionalDailyCapacity(authenticatedUser.businessId(), professionalId,
					startsAt);
		}
		if (customerId != null) {
			OffsetDateTime endsAt = startsAt.plusMinutes(durationMinutes);
			availabilityService.checkCustomerDuplicateActiveBooking(authenticatedUser.businessId(), customerId,
					professionalId, startsAt, endsAt, null);
		}
		ensureSlotAvailable(authenticatedUser.businessId(), null, location.locationId(), startsAt, durationMinutes);
		UUID bookingId = bookingJdbcRepository.insertBooking(authenticatedUser.businessId(), customer.id(), null, null,
				assignedUserId, subject, status, startsAt, durationMinutes, location.locationId(),
				location.locationText(), notes, resolveCompletedAt(status, startsAt));
		auditService.record(authenticatedUser.businessId(), authenticatedUser.userId(), "BOOKING_CREATED", "BOOKING",
				bookingId, "Reserva creada manualmente.",
	...
	public BookingDetailResponse createFromConversation(AuthenticatedUser authenticatedUser, UUID conversationId,
		...
		UUID leadId = request.leadId() != null
				...
				: bookingJdbcRepository.findLeadIdByConversation(authenticatedUser.businessId(), conversationId)
						.orElse(null);
		...
		OffsetDateTime startsAt = normalizeStartsAt(request.startsAt());
		LocationResolution location = resolveLocation(authenticatedUser.businessId(), request.locationId(),
				normalizeOptionalText(request.location(), "location", 160), conversation.locationId(), true);
		...
		UUID bookingId = bookingJdbcRepository.insertBooking(authenticatedUser.businessId(), conversation.customerId(),
				leadId, conversationId, assignedUserId, normalizeRequiredValue(request.subject(), "subject", 160),
				status, startsAt, durationMinutes, location.locationId(), location.locationText(),
				normalizeOptionalText(request.notes(), "notes", 2000), resolveCompletedAt(status, startsAt));
		auditService.record(authenticatedUser.businessId(), authenticatedUser.userId(),
				"BOOKING_CREATED_FROM_CONVERSATION", "BOOKING", bookingId, "Reserva creada desde conversacion.",
	...
	public BookingDetailResponse createFromLead(AuthenticatedUser authenticatedUser, UUID leadId,
		...
		UUID bookingId = bookingJdbcRepository.insertBooking(authenticatedUser.businessId(), lead.customerId(), leadId,
				lead.conversationId(), assignedUserId, normalizeRequiredValue(request.subject(), "subject", 160),
				status, startsAt, durationMinutes, location.locationId(), location.locationText(),
				normalizeOptionalText(request.notes(), "notes", 2000), resolveCompletedAt(status, startsAt));
		auditService.record(authenticatedUser.businessId(), authenticatedUser.userId(), "BOOKING_CREATED_FROM_LEAD",
				"BOOKING", bookingId, "Reserva creada desde prospecto.",
	...
	@Transactional
	public BookingDetailResponse update(AuthenticatedUser authenticatedUser, UUID bookingId,
		...
		OffsetDateTime startsAt = normalizeStartsAt(request.startsAt());
		LocationResolution location = resolveLocation(authenticatedUser.businessId(), request.locationId(),
				normalizeOptionalText(request.location(), "location", 160), current.locationId(), true);
		...
		bookingJdbcRepository.updateBooking(authenticatedUser.businessId(), bookingId,
				resolveResponsibleUserId(authenticatedUser, request.assignedUserId()),
				normalizeRequiredValue(request.subject(), "subject", 160), status, startsAt,
				normalizeDuration(request.durationMinutes()), location.locationId(), location.locationText(),
				normalizeOptionalText(request.notes(), "notes", 2000), resolveCompletedAt(status, startsAt));
		auditService.record(authenticatedUser.businessId(), authenticatedUser.userId(), "BOOKING_UPDATED", "BOOKING",
				bookingId, "Reserva actualizada.",
				AuditMetadata.of("previousStatus", current.status(), "newStatus", status, "subject", request.subject(),
						"startsAt", startsAt, "durationMinutes", request.durationMinutes(), "locationId",
						location.locationId()));
		return bookingJdbcRepository.findBookingDetail(authenticatedUser.businessId(), bookingId);
	}
	...
	private LocationResolution resolveLocation(UUID businessId, UUID requestedLocationId, String requestedLocationText,
			UUID fallbackLocationId, boolean requiredWhenMultiple) {
		UUID candidateLocationId = requestedLocationId != null ? requestedLocationId : fallbackLocationId;
		if (candidateLocationId != null) {
			BusinessLocationRecord location = businessLocationJdbcRepository.findActiveById(businessId,
					candidateLocationId);
			return new LocationResolution(location.id(),
					requestedLocationText != null ? requestedLocationText : location.name());
		}
	...
	private BookingJdbcRepository.CustomerRecord resolveCustomer(AuthenticatedUser authenticatedUser,
		...
		BookingJdbcRepository.CustomerRecord existing = bookingJdbcRepository
				.findCustomerByNormalizedPhone(authenticatedUser.businessId(), phone).orElse(null);
		if (existing != null) {
			bookingJdbcRepository.updateCustomer(authenticatedUser.businessId(), existing.id(), nameParts.firstName(),
					nameParts.lastName(), customerName, phone, email);
			return bookingJdbcRepository.findCustomerById(authenticatedUser.businessId(), existing.id());
		}

		UUID customerId = bookingJdbcRepository.insertCustomer(authenticatedUser.businessId(), nameParts.firstName(),
				nameParts.lastName(), customerName, phone, email);
		return bookingJdbcRepository.findCustomerById(authenticatedUser.businessId(), customerId);
	}

	private BookingJdbcRepository.LeadContextRecord ensureLeadBelongsToBusiness(AuthenticatedUser authenticatedUser,
			UUID leadId) {
	...
	private DateRange resolveDateRange(OffsetDateTime from, OffsetDateTime to) {
		...
		OffsetDateTime resolvedTo = to != null ? to : resolvedFrom.plusMonths(1).minusNanos(1);
	...
	private NameParts splitName(String displayName, String firstName, String lastName) {
		...
		return new NameParts(firstName != null ? firstName : parts[0], lastName != null ? lastName : parts[1]);
	}
	...
	private void ensureSlotAvailable(UUID businessId, UUID bookingId, UUID locationId, OffsetDateTime startsAt,
			int durationMinutes) {
		if (locationId == null) {
			return;
		}
