...
public class SincronizadorReservaLocal implements SincronizadorReservaMotorReglas {
    ...
    public void sincronizarReserva(UUID businessId, UUID bookingId,
        ...
        OffsetDateTime bookingDate = startsAt != null ? startsAt.toLocalDate().atStartOfDay(startsAt.getOffset()).toOffsetDateTime() : null;
        OffsetDateTime bookingTime = startsAt != null ? startsAt.toLocalTime().atDate(startsAt.toLocalDate()).atOffset(startsAt.getOffset()) : null;

        repository.upsertBookingFact(
                bookingId, businessId, obfuscatedPhone, customerName, managementId,
                serviceName, locationName, professionalName,
                bookingDate, bookingTime, bookingStatus, conversationId,
                channelOrigin, originIntent, startsAt);

        repository.updateBookingSyncStatus(bookingId, businessId, "SYNCED");

        log.info("BOOKING_FACT_SYNCED bookingId={} businessId={} phoneObfuscated={} service={} status={}",
                bookingId, businessId, !obfuscatedPhone.equals(customerPhone), serviceName, bookingStatus);
    }
}
