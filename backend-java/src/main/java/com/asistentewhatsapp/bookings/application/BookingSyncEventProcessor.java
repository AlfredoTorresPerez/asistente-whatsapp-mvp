...
public class BookingSyncEventProcessor {
    ...
    public BookingSyncEventProcessor(
            BookingSyncEventJdbcRepository eventRepository,
            BookingSyncJdbcRepository syncRepository,
    ...
    public void processDueEvents() {
        List<BookingSyncEventRecord> events = eventRepository.claimDueEvents(
                properties.getEventBatchSize(), properties.getEventProcessingTimeoutMs());
    ...
    private void processEventSafely(BookingSyncEventRecord event) {
        try {
            ...
            eventRepository.markFailedOrRetry(
                    event.id(), event.attempts(), event.maxAttempts(),
            ...
            log.error("BOOKING_SYNC_EVENT_FAILED eventId={} bookingId={} attempt={}/{} nextAttemptAt={} error={}",
                    event.id(), event.bookingId(), event.attempts(), event.maxAttempts(),
                    nextAttemptAt, e.getMessage(), e);
        }
    }
    ...
    private void processEvent(BookingSyncEventRecord event) {
        ...
        log.info("BOOKING_SYNC_EVENT_PROCESSED eventId={} bookingId={} type={}", event.id(), event.bookingId(), event.eventType());
    }
