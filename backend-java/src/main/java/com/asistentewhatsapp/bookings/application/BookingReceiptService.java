...
public class BookingReceiptService {
	...
	public UUID generateReceipt(UUID businessId, UUID bookingId) {
		...
		String receiptNumber = "RCP-" + bookingId.toString().substring(0, 8).toUpperCase() + "-" + now.toEpochSecond();
		jdbcTemplate.update("""
				...
				""",
				new MapSqlParameterSource().addValue("id", receiptId).addValue("businessId", businessId)
						.addValue("bookingId", bookingId).addValue("receiptNumber", receiptNumber)
						.addValue("status", RECEIPT_STATUS_GENERATED).addValue("generatedAt", now));
		return receiptId;
	}

	public String formatReceiptSummary(UUID businessId, UUID bookingId, UUID receiptId) {
		MapSqlParameterSource params = new MapSqlParameterSource().addValue("businessId", businessId)
				.addValue("bookingId", bookingId).addValue("receiptId", receiptId);
		var result = jdbcTemplate.query("""
				...
				""", params,
				(rs, rowNum) -> new ReceiptSummary(rs.getString("receipt_number"),
						rs.getObject("generated_at", OffsetDateTime.class), rs.getString("subject"),
						rs.getString("customer_name"), rs.getString("location_name"), rs.getString("service_name")));
		if (result.isEmpty()) {
			return "Comprobante no disponible.";
		}
		ReceiptSummary r = result.getFirst();
		return "Comprobante " + r.receiptNumber() + " - " + r.serviceName() + " en " + r.locationName() + " para "
				+ r.customerName() + " (generado " + r.generatedAt() + ")";
	}

	private record ReceiptSummary(String receiptNumber, OffsetDateTime generatedAt, String subject, String customerName,
