...
public final class BookingStateMachine {
    ...
    public static void assertTransition(String currentStatus, String targetStatus, String action) {
        ...
        throw new ApiException(HttpStatus.CONFLICT,
                ...
                Map.of("currentStatus", current == null ? "SIN_ESTADO" : current, "targetStatus", target == null ? "SIN_ESTADO" : target));
    }
    ...
    public static void assertCanChange(String currentStatus, String action) {
        String current = canonical(currentStatus);
        if (isOpen(current) || CONFIRMED.equals(current) || RESCHEDULED.equals(current) || PENDING_PAYMENT.equals(current)) {
            return;
        }
        throw new ApiException(HttpStatus.CONFLICT,
    ...
    private static boolean canTransition(String current, String target) {
        ...
        return switch (target) {
            ...
            case CONFIRMED -> REQUESTED.equals(current) || PENDING_CONFIRMATION.equals(current) || PENDING_PAYMENT.equals(current);
