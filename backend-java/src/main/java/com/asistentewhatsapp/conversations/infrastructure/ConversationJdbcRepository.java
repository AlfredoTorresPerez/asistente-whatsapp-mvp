...
public class ConversationJdbcRepository {
	...
	private static final Pattern SERIALIZED_WHATSAPP_CHAT_ID = Pattern
			.compile("(?:true|false)_([^_]+@(?:c\\.us|s\\.whatsapp\\.net|lid|g\\.us))_.*", Pattern.CASE_INSENSITIVE);
	...
	public ConversationMetricsResponse findConversationMetrics(UUID businessId) {
		...
		return jdbcTemplate.queryForObject(
				"""
						...
						""",
				parameters,
				(resultSet, rowNum) -> new ConversationMetricsResponse(resultSet.getLong("active_conversations"),
						resultSet.getLong("unattended_conversations"), resultSet.getLong("new_prospects"),
						resultSet.getLong("active_orders")));
	}

	public PagedResponse<ConversationSummaryResponse> findConversations(UUID businessId, int page, int size,
		...
		List<ConversationSummaryResponse> items = jdbcTemplate.query("""
				...
				limit :limit
				offset :offset
				""", parameters, conversationSummaryRowMapper());
	...
	public ConversationDetailResponse findConversationDetail(UUID businessId, UUID conversationId) {
		...
		List<ConversationDetailRow> rows = jdbcTemplate.query("""
				...
				""", parameters, conversationDetailRowMapper());
		...
		return new ConversationDetailResponse(conversation.id(), conversation.status(), conversation.channelType(),
				conversation.unreadCount(), conversation.lastMessagePreview(), conversation.lastMessageAt(),
				conversation.openedAt(), conversation.closedAt(), conversation.assignedUserId(),
				conversation.assignedUserName(), conversation.prospectId(), conversation.locationId(),
				conversation.locationName(),
				new ConversationCustomerResponse(conversation.customerId(), conversation.customerFirstName(),
						conversation.customerLastName(), conversation.customerDisplayName(),
	...
	public ConversationContextRecord findConversationContext(UUID businessId, UUID conversationId) {
		...
		List<ConversationContextRecord> rows = jdbcTemplate.query("""
				...
				(resultSet, rowNum) -> new ConversationContextRecord(resultSet.getObject("id", UUID.class),
						resultSet.getString("channel_type"), resultSet.getString("status"),
						resultSet.getString("customer_phone"), resultSet.getString("customer_display_name")));
	...
	public boolean assignConversationLocationFromMessageIfBlank(UUID businessId, UUID conversationId,
			String messageBody) {
		...
		int updated = jdbcTemplate.update(
				...
				new MapSqlParameterSource().addValue("businessId", businessId)
						.addValue("conversationId", conversationId).addValue("messageBody", messageBody));
		return updated > 0;
	}

	public Optional<String> findLatestProviderChatId(UUID businessId, UUID conversationId) {
		MapSqlParameterSource parameters = conversationParameters(businessId, conversationId).addValue("limit", 20);
		List<ProviderAddressRow> rows = jdbcTemplate.query("""
				...
				limit :limit
				""", parameters,
				(resultSet, rowNum) -> new ProviderAddressRow(resultSet.getString("external_message_id"),
	...
	public List<ConversationMessageResponse> findConversationMessages(UUID businessId, UUID conversationId) {
		return jdbcTemplate.query("""
				...
				""", conversationParameters(businessId, conversationId), conversationMessageRowMapper());
	}
	...
	public Optional<ConversationMessageResponse> findOutboundMessageByIdempotencyKey(UUID businessId,
	...
	public Optional<ChannelAccountRecord> findPrimaryActiveChannelAccount(UUID businessId) {
		List<ChannelAccountRecord> items = jdbcTemplate.query("""
				...
				limit 1
				""", new MapSqlParameterSource().addValue("businessId", businessId),
				(resultSet, rowNum) -> new ChannelAccountRecord(resultSet.getObject("id", UUID.class),
	...
	public UUID insertConversation(UUID businessId, UUID channelAccountId, UUID customerId, UUID assignedUserId,
			String customerName, String customerPhone, OffsetDateTime openedAt) {
		...
		jdbcTemplate.update("""
				...
				""",
				new MapSqlParameterSource().addValue("id", conversationId).addValue("businessId", businessId)
						.addValue("channelAccountId", channelAccountId).addValue("customerId", customerId)
						.addValue("assignedUserId", assignedUserId).addValue("customerName", customerName)
						.addValue("customerPhone", customerPhone).addValue("openedAt", openedAt));
		return conversationId;
	}

	public void updateConversationAssignment(UUID businessId, UUID conversationId, UUID assignedUserId,
		...
		int updated = jdbcTemplate.update("""
				...
				""", conversationParameters(businessId, conversationId).addValue("assignedUserId", assignedUserId)
	...
	public void markConversationRead(UUID businessId, UUID conversationId, OffsetDateTime updatedAt) {
		int updated = jdbcTemplate.update("""
				update conversation
				set unread_count = 0,
				    updated_at = :updatedAt
				where business_id = :businessId
				  and id = :conversationId
				  and unread_count > 0
				""", conversationParameters(businessId, conversationId).addValue("updatedAt", updatedAt));
		if (updated == 0) {
			throw new ResourceNotFoundException("No se encontro la conversacion solicitada.");
		}
	}

	public void updateConversationStatus(UUID businessId, UUID conversationId, String status,
			OffsetDateTime updatedAt) {
		int updated = jdbcTemplate.update("""
				update conversation
				set status = :status,
				...
				where business_id = :businessId
				  and id = :conversationId
				""", conversationParameters(businessId, conversationId).addValue("status", status).addValue("updatedAt",
				updatedAt));
		if (updated == 0) {
			throw new ResourceNotFoundException("No se encontro la conversacion solicitada.");
		}
	...
	public UUID insertOutboundMessage(UUID businessId, UUID conversationId, UUID sentByUserId, String body,
			String status, String externalMessageId, OffsetDateTime sentAt, String idempotencyKey) {
		...
		jdbcTemplate.update("""
				...
				""",
				conversationParameters(businessId, conversationId).addValue("id", messageId)
						.addValue("sentByUserId", sentByUserId).addValue("body", body).addValue("status", status)
						.addValue("externalMessageId", externalMessageId).addValue("idempotencyKey", idempotencyKey)
						.addValue("sentAt", sentAt));
		return messageId;
	}

	public void updateOutboundMessageDelivery(UUID businessId, UUID conversationId, UUID messageId, String status,
			String externalMessageId, OffsetDateTime occurredAt) {
		int updated = jdbcTemplate.update("""
				update message
				set status = :status,
				...
				""",
				conversationParameters(businessId, conversationId).addValue("messageId", messageId)
						.addValue("status", status).addValue("externalMessageId", externalMessageId)
	...
	public void updateConversationOutboundActivity(UUID businessId, UUID conversationId, String preview,
		...
		int updated = jdbcTemplate.update("""
				        ...
				        where business_id = :businessId
				          and id = :conversationId
				""", conversationParameters(businessId, conversationId).addValue("preview", truncatePreview(preview))
				.addValue("occurredAt", occurredAt));
		if (updated == 0) {
			throw new ResourceNotFoundException("No se encontro la conversacion solicitada.");
		}
	...
	public void insertMessageDeliveryLog(UUID businessId, UUID messageId, String deliveryStatus, String providerEventId,
			OffsetDateTime occurredAt) {
		jdbcTemplate.update("""
				...
				new MapSqlParameterSource().addValue("id", UUID.randomUUID()).addValue("businessId", businessId)
						.addValue("messageId", messageId).addValue("deliveryStatus", deliveryStatus)
						.addValue("providerEventId", providerEventId).addValue("occurredAt", occurredAt));
	}
	...
	public ResponseTemplateResponse findTemplateById(UUID businessId, UUID templateId) {
		List<ResponseTemplateResponse> items = jdbcTemplate.query("""
				...
				where business_id = :businessId
				  and id = :templateId
				""", new MapSqlParameterSource().addValue("businessId", businessId).addValue("templateId", templateId),
				templateRowMapper());
		if (items.isEmpty()) {
	...
	public TemplateRecord findTemplateRecordById(UUID businessId, UUID templateId) {
		List<TemplateRecord> items = jdbcTemplate.query("""
				...
				where business_id = :businessId
				  and id = :templateId
				""", new MapSqlParameterSource().addValue("businessId", businessId).addValue("templateId", templateId),
				(resultSet, rowNum) -> new TemplateRecord(resultSet.getObject("id", UUID.class),
						resultSet.getString("name"), resultSet.getString("category"), resultSet.getString("body"),
	...
	public UUID insertTemplate(UUID businessId, String name, String category, String body, boolean active) {
		...
		jdbcTemplate.update("""
				...
				""",
				new MapSqlParameterSource().addValue("id", templateId).addValue("businessId", businessId)
						.addValue("name", name).addValue("category", category).addValue("body", body)
	...
	private QueryParts buildConversationListQuery(UUID businessId, String search, String status, UUID ownerUserId) {
		StringBuilder sql = new StringBuilder("""
				from conversation c
				left join user_account ua on ua.id = c.assigned_user_id
				left join lead l on l.conversation_id = c.id
	...
	private MapSqlParameterSource conversationParameters(UUID businessId, UUID conversationId) {
		return new MapSqlParameterSource().addValue("businessId", businessId).addValue("conversationId",
				conversationId);
	}
	...
	private RowMapper<ConversationSummaryResponse> conversationSummaryRowMapper() {
		return (resultSet, rowNum) -> new ConversationSummaryResponse(resultSet.getObject("id", UUID.class),
				resultSet.getString("customer_name"), resultSet.getString("customer_phone"),
				resultSet.getString("status"), resultSet.getInt("unread_count"),
				resultSet.getString("last_message_preview"),
				resultSet.getObject("last_message_at", OffsetDateTime.class), resultSet.getString("channel_type"),
				resultSet.getObject("assigned_user_id", UUID.class), resultSet.getString("assigned_user_name"),
				resultSet.getObject("prospect_id", UUID.class), resultSet.getObject("location_id", UUID.class),
				resultSet.getString("location_name"));
	}

	private RowMapper<ConversationDetailRow> conversationDetailRowMapper() {
		return (resultSet, rowNum) -> new ConversationDetailRow(resultSet.getObject("id", UUID.class),
				resultSet.getString("status"), resultSet.getString("channel_type"), resultSet.getInt("unread_count"),
				resultSet.getString("last_message_preview"),
				...
				resultSet.getString("customer_display_name"), resultSet.getString("customer_phone"),
				resultSet.getString("customer_email"));
	}

	private RowMapper<ConversationMessageResponse> conversationMessageRowMapper() {
		return (resultSet, rowNum) -> new ConversationMessageResponse(resultSet.getObject("id", UUID.class),
				resultSet.getString("direction"), resultSet.getString("message_type"), resultSet.getString("body"),
				resultSet.getString("status"), resultSet.getString("external_message_id"),
				resultSet.getObject("sent_by_user_id", UUID.class), resultSet.getString("sent_by_user_name"),
	...
	private RowMapper<ResponseTemplateResponse> templateRowMapper() {
		return (resultSet, rowNum) -> new ResponseTemplateResponse(resultSet.getObject("id", UUID.class),
				resultSet.getString("name"), resultSet.getString("category"), resultSet.getString("body"),
	...
	private record ConversationDetailRow(UUID id, String status, String channelType, int unreadCount,
			...
			UUID customerId, String customerFirstName, String customerLastName, String customerDisplayName,
	...
	public record ConversationContextRecord(UUID id, String channelType, String status, String customerPhone,
	...
	public record TemplateRecord(UUID id, String name, String category, String body, boolean active) {
	}
}
