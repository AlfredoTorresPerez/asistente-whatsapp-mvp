...
public class PaymentsAgent extends AbstractAgentHandler {
    ...
    public AgentRoutingResult handle(
        ...
        if (containsAny(normalizedMessage, "senal", "señal", "abono", "abonar", "pagar reserva", "link de pago")) {
            missing.clear();
            ...
            response = knowledgeService.paymentWithRequestAndAmountResponse(
                    request.businessId(),
                    entities.get("numero_solicitud"),
                    entities.get("monto"));
            missing.clear();
            missing.add("metodo_pago");
        } else if (hasRequest) {
            response = knowledgeService.renderRule(
                    request.businessId(),
                    "AI_PAYMENT_MISSING_AMOUNT_RESPONSE",
                    Map.of("requestNumber", entities.get("numero_solicitud")));
        } else {
            response = knowledgeService.renderRule(request.businessId(), "AI_PAYMENT_MISSING_REQUEST_RESPONSE", Map.of());
        }
