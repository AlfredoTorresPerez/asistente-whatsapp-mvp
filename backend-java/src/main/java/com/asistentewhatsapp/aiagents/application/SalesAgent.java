...
public class SalesAgent extends AbstractAgentHandler {
	...
	public AgentRoutingResult handle(AgentConversationRequest request, IntentDetectionResult intent,
		...
		if (primaryIntent == AgentIntent.SERVICE_RECOMMENDATION) {
			String response = knowledgeService.serviceRecommendationResponse(request.businessId(),
					request.messageBody());
			return result(request, intent, type(), entities, missing("servicio_si_desea_agendar"), response, false,
					null);
		}

		if (primaryIntent == AgentIntent.SERVICE_INFORMATION) {
			String response = knowledgeService.serviceInformationResponse(request.businessId(), service,
					request.messageBody());
			List<String> missing = has(entities, "servicio_o_producto") ? List.of() : missing("servicio_o_producto");
		...
		if (primaryIntent == AgentIntent.COMMERCIAL_INQUIRY
			...
			return result(request, intent, type(), entities, missing("servicio_o_producto_especifico"),
					knowledgeService.depilationCatalogResponse(request.businessId()), false, null);
		}
		...
		if (primaryIntent == AgentIntent.PRICE_REQUEST) {
			...
			return knowledgeService.findService(request.businessId(), service)
					...
					.orElseGet(() -> result(request, intent, type(), entities,
							missing("servicio_o_producto_especifico"), knowledgeService.renderRule(request.businessId(),
									"AI_PRICE_UNKNOWN_SERVICE_RESPONSE", Map.of()),
							false, null));
		}

		if (primaryIntent == AgentIntent.QUOTE_REQUEST) {
			String category = firstNonBlank(value(entities, "categoria_servicio"),
					genericDepilationCategory(service, request.messageBody()));
			if (!category.isBlank()) {
				entities.putIfAbsent("categoria_servicio", category);
				return result(request, intent, type(), entities, missing("zona_corporal"),
						knowledgeService.quoteMissingDetailResponse(request.businessId(), category), false, null);
			}
		}

		if (!has(entities, "servicio_o_producto")) {
			String response = knowledgeService.serviceInformationResponse(request.businessId(), "",
					request.messageBody());
			return result(request, intent, type(), entities, missing("servicio_o_producto"), response, false, null);
		}

		String response = knowledgeService.renderRule(request.businessId(), "AI_SALES_NEXT_STEP_RESPONSE",
