...
public class AgentRegistry {
    ...
    private AgentType resolveAgentType(IntentDetectionResult intent) {
        ...
        return switch (primaryIntent) {
            case GREETING, AMBIGUOUS -> AgentType.RECEPTION;
            case COMMERCIAL_INQUIRY, PRICE_REQUEST, QUOTE_REQUEST -> AgentType.SALES;
            case COMMERCIAL_AND_BOOKING -> AgentType.BOOKING;
            case BOOKING_REQUEST, BOOKING_CHANGE, BOOKING_CANCEL, BOOKING_STATUS -> AgentType.BOOKING;
            case PAYMENT_INQUIRY, PAYMENT_PROBLEM -> AgentType.PAYMENTS;
            case SUPPORT_GENERAL, TECHNICAL_MESSAGE -> AgentType.SUPPORT;
