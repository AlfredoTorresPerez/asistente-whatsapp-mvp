    ...
    public AgentRoutingResult handle(
        ...
        if (intent.primaryIntent() == AgentIntent.TECHNICAL_MESSAGE) {
            return result(
                    ...
                    type(),
                    entities,
                    missing("solicitud_valida_centro_estetico"),
    ...
    private LocationAnswer locationResponse(AgentConversationRequest request, String normalizedMessage) {
        ...
        if (mentioned.isPresent()) {
            ...
            if (location.address() == null || location.address().isBlank()) {
                return new LocationAnswer("Tengo registrada la sucursal " + location.name() + ", pero falta configurar su dirección o enlace de mapa. Te derivaré con una persona del equipo para confirmarlo.", true);
            }
            return new LocationAnswer("La sucursal " + location.name() + " está ubicada en:\n" + location.address(), true);
        }
        if (locations.isEmpty()) {
            return new LocationAnswer("No encontré sucursales activas configuradas. Te derivaré con una persona del equipo para confirmar la ubicación.", false);
        }
        return new LocationAnswer("Tenemos estas sucursales activas: " + locations.stream().map(BusinessLocationRecord::name).reduce((l, r) -> l + ", " + r).orElse("sede principal") + ". ¿Sobre cuál necesitas la dirección?", false);
    }

    private record LocationAnswer(String response, boolean locationWasIdentified) {}

    private Optional<BusinessLocationRecord> findMentionedLocation(List<BusinessLocationRecord> locations, String normalizedMessage) {
        Optional<BusinessLocationRecord> byNameOrCode = locations.stream()
                .filter(location -> containsNormalized(normalizedMessage, location.name()) || containsNormalized(normalizedMessage, location.code()))
                .findFirst();
        if (byNameOrCode.isPresent()) {
            return byNameOrCode;
        }
        return locations.stream()
                .filter(location -> containsNormalized(normalizedMessage, location.commune()) || containsNormalized(normalizedMessage, location.city()))
                .findFirst();
    }
