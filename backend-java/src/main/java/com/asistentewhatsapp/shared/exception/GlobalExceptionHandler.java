...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiErrorResponse> handleValidationException(MethodArgumentNotValidException exception,
		...
		return buildResponse(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "La solicitud contiene datos invalidos.",
				request, fieldErrors);
	}
	...
	public ResponseEntity<ApiErrorResponse> handleConstraintViolation(ConstraintViolationException exception,
		...
		Map<String, String> violations = exception.getConstraintViolations().stream()
				.collect(Collectors.toMap(violation -> violation.getPropertyPath().toString(),
						violation -> violation.getMessage(), (first, second) -> first, LinkedHashMap::new));

		return buildResponse(HttpStatus.BAD_REQUEST, "CONSTRAINT_VIOLATION",
				"La solicitud contiene restricciones invalidas.", request, violations);
	}
	...
	public ResponseEntity<ApiErrorResponse> handleUnsupportedMessagingChannel(
			UnsupportedMessagingChannelException exception, HttpServletRequest request) {
		return buildResponse(HttpStatus.SERVICE_UNAVAILABLE, "CHANNEL_NOT_CONFIGURED", exception.getMessage(), request,
				Map.of());
	}

	@ExceptionHandler(MessagingChannelUnavailableException.class)
	public ResponseEntity<ApiErrorResponse> handleChannelUnavailable(MessagingChannelUnavailableException exception,
			HttpServletRequest request) {
		return buildResponse(HttpStatus.BAD_GATEWAY, "CHANNEL_UNAVAILABLE", exception.getMessage(), request, Map.of());
	}

	@ExceptionHandler(IllegalArgumentException.class)
	public ResponseEntity<ApiErrorResponse> handleIllegalArgument(IllegalArgumentException exception,
			HttpServletRequest request) {
		return buildResponse(HttpStatus.BAD_REQUEST, "BAD_REQUEST", exception.getMessage(), request, Map.of());
	}
	...
	public ResponseEntity<ApiErrorResponse> handleAuthenticationException(AuthenticationException exception,
			HttpServletRequest request) {
		return buildResponse(HttpStatus.UNAUTHORIZED, "UNAUTHORIZED", "Debes iniciar sesion para continuar.", request,
				Map.of());
	}
	...
	public ResponseEntity<ApiErrorResponse> handleDuplicateKeyException(DuplicateKeyException exception,
		...
		boolean isBookingDuplicate = message != null && message.contains("uq_booking_customer_professional_active");
		if (isBookingDuplicate) {
			LOGGER.warn("Intento de reserva duplicada: {}", message);
			return buildResponse(HttpStatus.CONFLICT, "BOOKING_DUPLICATE",
					"Ya existe una reserva activa para este cliente, profesional y horario.", request, Map.of());
		}
	...
	public ResponseEntity<ApiErrorResponse> handleUnexpectedException(Exception exception, HttpServletRequest request) {
		...
		if (slackNotifier != null)
			slackNotifier.notifyError("500 en " + request.getMethod() + " " + request.getRequestURI(),
					exception.toString());
		return buildResponse(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_ERROR",
				"Ocurrio un error inesperado. Intenta nuevamente.", request, Map.of());
	}
	...
	private ResponseEntity<ApiErrorResponse> buildResponse(HttpStatus status, String code, String message,
		...
		if (status.is5xxServerError()) {
			LOGGER.error(
					...
					code, status.value(), request.getRequestURI(), correlationId, message);
