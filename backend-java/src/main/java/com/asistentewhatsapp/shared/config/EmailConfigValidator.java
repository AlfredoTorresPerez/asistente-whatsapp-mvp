...
public class EmailConfigValidator implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) {
		...
		if (isPlaceholder(host) || isLocalhost(host)) {
			...
			if (!"local".equals(env) && !"dev".equals(env) && !"development".equals(env) && !"demo".equals(env)
					&& !"test".equals(env)) {
				throw new IllegalStateException("spring.mail.host=" + host + " no es válido para entorno " + env
