...
public class SystemController {
	...
	public SystemController(DataSource dataSource, @Value("${app.environment:local}") String environment,
	...
	public SystemStatusResponse status(@AuthenticationPrincipal AuthenticatedUser user) {
		...
		return new SystemStatusResponse(backendStatus, dbStatus, appName, environment, Instant.now(),
				aiAutoReplyEnabled, calendarGoogleEnabled, calendarSyncEnabled, emailEnabled, paymentProvider,
				"No disponible");
	}
	...
	public record SystemStatusResponse(String backendStatus, String databaseStatus, String appName, String environment,
			Instant serverTimestamp, boolean aiAutoReplyEnabled, boolean calendarIntegrationEnabled,
			boolean calendarSyncEnabled, boolean emailEnabled, String paymentProvider, String recentErrors) {
	}
}
