...
public final class LogSanitizer {
	...
	private static final Set<String> SENSITIVE_KEYS = Set.of("body", "customerName", "customer_name", "displayName",
			"display_name", "extractedData", "from", "lastInbound", "messageBody", "normalizedPhone", "originalText",
			"phone", "phoneNumber", "phone_number", "rawText", "recipientPhone", "responseText", "to",
			"ultimo_mensaje_cliente", "ultima_respuesta_ia");
	...
	public static String messageSummary(String label, String message) {
		...
		String summary = prefix + "Length=" + value.length() + " " + prefix + "ContainsLink=" + containsLink(value);
	...
	public static String sanitizeFreeText(String value) {
		...
		String sanitized = value.replaceAll("(?i)authorization=([^, ]+)", "authorization=***")
				...
				.replaceAll("(?i)(nombre|cliente|customerName)=([^,]+)", "$1=[REDACTADO]").replace('\n', ' ')
	...
	private static boolean isSensitiveKey(String key) {
		String normalized = key == null ? "" : key.replace("-", "").replace("_", "").toLowerCase(Locale.ROOT);
