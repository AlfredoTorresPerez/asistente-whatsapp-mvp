...
public class DashboardService {
	...
	public DashboardSummaryResponse getSummary(AuthenticatedUser authenticatedUser, OffsetDateTime from,
			OffsetDateTime to, UUID ownerUserId) {
		...
		DashboardKpisResponse kpis = new DashboardKpisResponse(
				dashboardJdbcRepository.countOpenConversations(authenticatedUser.businessId(), ownerUserId,
						dashboardRange.from(), dashboardRange.to()),
				dashboardJdbcRepository.countNewProspects(authenticatedUser.businessId(), ownerUserId,
						dashboardRange.from(), dashboardRange.to()),
				dashboardJdbcRepository.countOpenOrders(authenticatedUser.businessId(), ownerUserId,
						dashboardRange.from(), dashboardRange.to()),
				dashboardJdbcRepository.countPendingAppointments(authenticatedUser.businessId(), ownerUserId,
						dashboardRange.from(), dashboardRange.to()));

		List<DashboardSeriesPointResponse> conversationSeries = dashboardJdbcRepository.loadConversationSeries(
				authenticatedUser.businessId(), ownerUserId, dashboardRange.from(), dashboardRange.to());
		List<DashboardSeriesPointResponse> orderSeries = dashboardJdbcRepository.loadOrderSeries(
				authenticatedUser.businessId(), ownerUserId, dashboardRange.from(), dashboardRange.to());
		List<DashboardAppointmentResponse> todayAppointments = dashboardJdbcRepository
				.loadTodayAppointments(authenticatedUser.businessId(), ownerUserId, todayRange.from(), todayRange.to());
		List<DashboardActivityResponse> recentActivity = dashboardJdbcRepository.loadRecentActivity(
				authenticatedUser.businessId(), ownerUserId, dashboardRange.from(), dashboardRange.to());

		return new DashboardSummaryResponse(kpis, conversationSeries, orderSeries, todayAppointments, recentActivity);
	}

	private DashboardRange resolveRange(AuthenticatedUser authenticatedUser, OffsetDateTime from, OffsetDateTime to) {
		...
		OffsetDateTime resolvedFrom = from != null
				...
				: now.minusDays(6).with(LocalTime.MIN).withZoneSameInstant(ZoneOffset.UTC).toOffsetDateTime();
	...
	private DashboardRange resolveTodayRange(AuthenticatedUser authenticatedUser) {
		...
		return new DashboardRange(startOfDay.withZoneSameInstant(ZoneOffset.UTC).toOffsetDateTime(),
	...
	private record DashboardRange(OffsetDateTime from, OffsetDateTime to) {
	}
}
