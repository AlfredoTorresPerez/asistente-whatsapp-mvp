...
public class OrderService {
    ...
    public PagedResponse<OrderSummaryResponse> list(
        ...
        return orderJdbcRepository.findOrders(
                authenticatedUser.businessId(),
                resolvedPage,
                resolvedSize,
                search,
                normalizeOptionalStatus(status),
                normalizeOptionalPaymentStatus(paymentStatus));
    }
    ...
    public OrderDetailResponse create(AuthenticatedUser authenticatedUser, CreateOrderRequest request) {
        ...
        UUID orderId = orderJdbcRepository.insertOrder(
                authenticatedUser.businessId(),
                context.customerId(),
                context.leadId(),
    ...
    @Transactional
    public OrderDetailResponse createFromConversation(
            AuthenticatedUser authenticatedUser,
            UUID conversationId,
    ...
    @Transactional
    public OrderDetailResponse createFromLead(
            AuthenticatedUser authenticatedUser,
            UUID leadId,
            CreateOrderRequest request) {
        OrderJdbcRepository.OrderContext context = orderJdbcRepository
    ...
    public OrderDetailResponse update(AuthenticatedUser authenticatedUser, UUID orderId, UpdateOrderRequest request) {
        ...
        orderJdbcRepository.updateOrder(
                authenticatedUser.businessId(),
                orderId,
    ...
    public OrderDetailResponse registerPayment(
        ...
        orderJdbcRepository.findOrderDetail(authenticatedUser.businessId(), orderId);
        orderJdbcRepository.registerPayment(
                authenticatedUser.businessId(),
                orderId,
                request.amount(),
    ...
    public SendOrderSummaryResponse sendSummary(AuthenticatedUser authenticatedUser, UUID orderId) {
        ...
        ChannelDispatchResponse dispatchResponse = channelDispatchService.dispatch(new ChannelDispatchRequest(
                ...
                order.customerPhone(),
                body));
        return new SendOrderSummaryResponse(
                dispatchResponse.status(),
                dispatchResponse.externalMessageId(),
                dispatchResponse.acceptedAt(),
                body);
    }

    private OrderJdbcRepository.OrderContext resolveContext(AuthenticatedUser authenticatedUser, CreateOrderRequest request) {
        ...
        if (request.customerId() != null) {
            ...
            return new OrderJdbcRepository.OrderContext(
                    customer.id(), customer.displayName(), customer.phone(), customer.email(), null, null);
        }
        ...
        return new OrderJdbcRepository.OrderContext(customer.id(), customer.displayName(), customer.phone(), customer.email(), null, null);
    }
    ...
    private String buildWhatsappSummary(OrderDetailResponse order) {
        ...
        order.items().forEach(item -> builder.append("- ").append(item.productName())
