...
public class LeadService {
	...
	@Transactional
	public LeadDetailResponse createFromConversation(AuthenticatedUser authenticatedUser, UUID conversationId,
			CreateLeadFromConversationRequest request) {
		LeadJdbcRepository.ConversationLeadContextRecord conversation = leadJdbcRepository
				.findConversationLeadContext(authenticatedUser.businessId(), conversationId);

		UUID existingConversationLeadId = leadJdbcRepository
				.findLeadIdByConversation(authenticatedUser.businessId(), conversationId).orElse(null);
		...
		UUID existingPhoneLeadId = leadJdbcRepository
				.findActiveLeadIdByNormalizedPhone(authenticatedUser.businessId(), phone).orElse(null);
		if (existingPhoneLeadId != null) {
			leadJdbcRepository.linkLeadToConversationIfUnlinked(authenticatedUser.businessId(), existingPhoneLeadId,
					conversationId);
			return leadJdbcRepository.findLeadDetail(authenticatedUser.businessId(), existingPhoneLeadId);
		}

		String email = request.email() == null
				? normalizeOptionalEmail(conversation.customerEmail())
	...
	@Transactional
	public LeadDetailResponse update(AuthenticatedUser authenticatedUser, UUID leadId, UpdateLeadRequest request) {
		LeadJdbcRepository.LeadContextRecord currentLead = leadJdbcRepository
	...
	@Transactional
	public LeadNoteResponse addNote(AuthenticatedUser authenticatedUser, UUID leadId, AddLeadNoteRequest request) {
		leadJdbcRepository.findLeadContext(authenticatedUser.businessId(), leadId);
		UUID noteId = leadJdbcRepository.insertLeadNote(authenticatedUser.businessId(), leadId,
				authenticatedUser.userId(), normalizeRequiredValue(request.noteText(), "noteText", 2000));
		return leadJdbcRepository.findLeadNoteById(authenticatedUser.businessId(), leadId, noteId);
	}

	@Transactional
	public LeadDetailResponse updateStage(AuthenticatedUser authenticatedUser, UUID leadId,
			UpdateLeadStageRequest request) {
		leadJdbcRepository.updateLeadStage(authenticatedUser.businessId(), leadId,
				normalizeRequiredStage(request.stage()));
		return leadJdbcRepository.findLeadDetail(authenticatedUser.businessId(), leadId);
	}

	private LeadJdbcRepository.CustomerRecord resolveOrCreateCustomer(UUID businessId, UUID currentCustomerId,
			String firstName, String lastName, String phone, String email) {
		LeadJdbcRepository.CustomerRecord existingByPhone = leadJdbcRepository
				.findCustomerByNormalizedPhone(businessId, phone).orElse(null);

		if (existingByPhone != null) {
			leadJdbcRepository.updateCustomer(businessId, existingByPhone.id(), firstName, lastName,
					displayName(firstName, lastName), phone, email);
			return leadJdbcRepository.findCustomerById(businessId, existingByPhone.id());
		}

		if (currentCustomerId != null) {
			leadJdbcRepository.updateCustomer(businessId, currentCustomerId, firstName, lastName,
					displayName(firstName, lastName), phone, email);
			return leadJdbcRepository.findCustomerById(businessId, currentCustomerId);
		}

		UUID customerId = leadJdbcRepository.insertCustomer(businessId, firstName, lastName,
				displayName(firstName, lastName), phone, email);
