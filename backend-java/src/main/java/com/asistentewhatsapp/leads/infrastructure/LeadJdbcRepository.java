...
public class LeadJdbcRepository {
    ...
    public PagedResponse<LeadSummaryResponse> findLeads(
            UUID businessId,
            int page,
            int size,
            String search,
    ...
    public ConversationLeadContextRecord findConversationLeadContext(UUID businessId, UUID conversationId) {
        List<ConversationLeadContextRecord> items = jdbcTemplate.query(
                ...
                (resultSet, rowNum) -> new ConversationLeadContextRecord(
                        resultSet.getObject("id", UUID.class),
                        resultSet.getObject("customer_id", UUID.class),
        ...
        List<LeadNoteResponse> items = jdbcTemplate.query(
                """
                        ...
                        where ln.business_id = :businessId
                          ...
                          and ln.id = :noteId
                        """,
                leadParameters(businessId, leadId).addValue("noteId", noteId),
                leadNoteRowMapper());
        if (items.isEmpty()) {
    ...
    private QueryParts buildLeadListQuery(
            UUID businessId,
            String search,
            String stage,
            String origin,
    ...
    private RowMapper<LeadSummaryResponse> leadSummaryRowMapper() {
        return (resultSet, rowNum) -> new LeadSummaryResponse(
                resultSet.getObject("id", UUID.class),
                resultSet.getObject("customer_id", UUID.class),
                resultSet.getObject("conversation_id", UUID.class),
    ...
    private RowMapper<LeadDetailRow> leadDetailRowMapper() {
        return (resultSet, rowNum) -> new LeadDetailRow(
                resultSet.getObject("id", UUID.class),
                resultSet.getObject("customer_id", UUID.class),
                resultSet.getObject("conversation_id", UUID.class),
                resultSet.getString("first_name"),
    ...
    private RowMapper<LeadNoteResponse> leadNoteRowMapper() {
        return (resultSet, rowNum) -> new LeadNoteResponse(
                resultSet.getObject("id", UUID.class),
                resultSet.getObject("author_user_id", UUID.class),
                resultSet.getString("author_user_name"),
    ...
    public record ConversationLeadContextRecord(
            UUID id,
            UUID customerId,
            UUID assignedUserId,
