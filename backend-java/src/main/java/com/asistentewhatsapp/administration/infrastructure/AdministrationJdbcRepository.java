...
public class AdministrationJdbcRepository {
    ...
    public AdminSummaryResponse findSummary(UUID businessId) {
        ...
        return new AdminSummaryResponse(
                ...
                new AdminSummaryResponse.UsersSummary(
                        ((Number) usersProjection.get("total")).longValue(),
                        ((Number) usersProjection.get("active")).longValue()),
                new AdminSummaryResponse.WhatsAppWebSummary(whatsAppWebStatus == null ? "DISCONNECTED" : whatsAppWebStatus),
                new AdminSummaryResponse.SecuritySummary(sessionTimeoutMinutes == null ? 30 : sessionTimeoutMinutes));
    }

    public PagedResponse<AdminUserResponse> findAdminUsers(
            UUID businessId,
            int page,
            int size,
            String search,
    ...
    public List<AdminRoleResponse> findAdminRoles() {
        return namedParameterJdbcTemplate.query(
                """
                        ...
                        """,
                new MapSqlParameterSource(),
                (resultSet, rowNum) -> new AdminRoleResponse(
    ...
    public AdminUserResponse insertAdminUser(
            UUID businessId,
        ...
        MapSqlParameterSource parameters = new MapSqlParameterSource()
                ...
                .addValue("userRoleId", userRoleId)
        ...
        try {
            ...
            throw new ConflictException(
                    "Ya existe un usuario con ese correo en la empresa.",
    ...
    public SecurityPolicyResponse updateSecurityPolicy(UUID businessId, SecurityPolicyRequest request) {
        int updated = namedParameterJdbcTemplate.update(
                ...
                new MapSqlParameterSource()
                        ...
                        .addValue("requireNumber", request.requireNumber())
                        .addValue("requireSymbol", request.requireSymbol())
    ...
    private QueryParts buildAdminUserListQuery(
            UUID businessId,
            String search,
            String role,
            String status) {
    ...
    private RowMapper<AdminUserResponse> adminUserRowMapper() {
        return (resultSet, rowNum) -> new AdminUserResponse(
                resultSet.getObject("id", UUID.class),
    ...
    private RowMapper<SecurityPolicyResponse> securityPolicyRowMapper() {
        return (resultSet, rowNum) -> new SecurityPolicyResponse(
                resultSet.getObject("id", UUID.class),
                resultSet.getInt("session_timeout_minutes"),
                resultSet.getInt("password_min_length"),
                resultSet.getBoolean("require_uppercase"),
                resultSet.getBoolean("require_number"),
                resultSet.getBoolean("require_symbol"),
                resultSet.getInt("max_failed_login_attempts"),
                resultSet.getLong("active_users"),
                resultSet.getLong("locked_users"),
                resultSet.getLong("audit_events_last_7_days"),
                resultSet.getObject("updated_at", OffsetDateTime.class));
    }
    ...
    private static class CompanySummaryRowMapper implements RowMapper<CompanySummaryProjection> {
        ...
        public CompanySummaryProjection mapRow(ResultSet resultSet, int rowNum) throws SQLException {
            return new CompanySummaryProjection(
                    resultSet.getObject("id", UUID.class),
