...
public class WhatsAppWebAdministrationService {
	...
	public WhatsAppWebAdministrationService(WhatsAppWebChannelJdbcRepository repository,
	...
	public WhatsAppWebStatusResponse getStatus(AuthenticatedUser authenticatedUser) {
		...
		WhatsAppWebChannelJdbcRepository.ChannelAccountRecord channelAccount = loadChannelAccount(
		...
		try {
			...
			return new WhatsAppWebStatusResponse(channelAccount.status(),
					resolveDisplayPhone(channelAccount.phoneNumber()), channelAccount.lastQrCode(),
					channelAccount.lastEventAt(), false, "EXPERIMENTAL", WARNING_MESSAGE,
	...
	public WhatsAppWebActionResponse connect(AuthenticatedUser authenticatedUser) {
		...
		repository.updateChannelAccount(channelAccount.id(), normalizeSessionStatus(response.connectionStatus()),
		...
		auditLogJdbcRepository.insert(authenticatedUser.businessId(), authenticatedUser.userId(),
				"WHATSAPP_WEB_CONNECT_REQUESTED", "CHANNEL_ACCOUNT", channelAccount.id(),
				"Se solicito conexion del adaptador WhatsApp Web experimental.",
				Map.of("sessionStatus", normalizeSessionStatus(response.connectionStatus())), acceptedAt);
		return new WhatsAppWebActionResponse(normalizeSessionStatus(response.connectionStatus()), displayPhone,
				response.qrCode(), acceptedAt, true, canalWhatsApp.provider().name());
	}
	...
	public WhatsAppWebActionResponse refreshQr(AuthenticatedUser authenticatedUser) {
		...
		repository.updateChannelAccount(channelAccount.id(), normalizeSessionStatus(response.connectionStatus()),
				displayPhone, response.qrCode(), acceptedAt);
		auditLogJdbcRepository.insert(authenticatedUser.businessId(), authenticatedUser.userId(),
				"WHATSAPP_WEB_QR_REFRESHED", "CHANNEL_ACCOUNT", channelAccount.id(),
	...
	public WhatsAppWebActionResponse disconnect(AuthenticatedUser authenticatedUser) {
		...
		repository.updateChannelAccount(channelAccount.id(), normalizeSessionStatus(response.connectionStatus()),
				displayPhone, response.qrCode(), acceptedAt);
		auditLogJdbcRepository.insert(authenticatedUser.businessId(), authenticatedUser.userId(),
				"WHATSAPP_WEB_DISCONNECTED", "CHANNEL_ACCOUNT", channelAccount.id(),
	...
	@Transactional
	public WhatsAppWebTestMessageResponse sendTestMessage(AuthenticatedUser authenticatedUser,
		...
		WhatsAppWebChannelJdbcRepository.CustomerRecord customer = repository
				...
				.orElseGet(() -> new WhatsAppWebChannelJdbcRepository.CustomerRecord(repository
		...
		try {
			ChannelDelivery channelDelivery = resolveCanalWhatsApp()
			...
			auditLogJdbcRepository.insert(authenticatedUser.businessId(), authenticatedUser.userId(),
					"WHATSAPP_WEB_TEST_MESSAGE_SENT", "MESSAGE", messageId,
			...
			return new WhatsAppWebTestMessageResponse(conversation.id(), messageId, channelDelivery.externalMessageId(),
					normalizeDeliveryStatus(channelDelivery.status()), acceptedAt);
		} catch (MessagingChannelUnavailableException exception) {
			...
			throw new ApiException(HttpStatus.SERVICE_UNAVAILABLE, "WHATSAPP_WEB_UNAVAILABLE",
	...
	private CanalWhatsApp resolveCanalWhatsApp() {
		return canalesWhatsApp.stream().findFirst().orElseThrow(() -> new ApiException(HttpStatus.SERVICE_UNAVAILABLE,
				"WHATSAPP_CHANNEL_DISABLED", "El puerto interno CanalWhatsApp no esta disponible para este ambiente."));
	}
	...
	private WhatsAppWebChannelJdbcRepository.ChannelAccountRecord loadChannelAccount(UUID businessId) {
		return repository.findChannelAccountByBusinessId(businessId)
				.orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "WHATSAPP_WEB_CHANNEL_NOT_FOUND",
	...
	private WhatsAppWebStatusResponse toStatusResponse(
			WhatsAppWebChannelJdbcRepository.ChannelAccountRecord channelAccount, WhatsAppSessionStatus response,
			boolean adapterReachable) {
		String sessionStatus = normalizeSessionStatus(response.connectionStatus());
		return new WhatsAppWebStatusResponse(sessionStatus, resolveDisplayPhone(response.phoneNumber()),
				response.qrCode(), response.lastEventAt(), adapterReachable,
				response.adapterMode() == null || response.adapterMode().isBlank()
						? "EXPERIMENTAL"
						: response.adapterMode(),
				WARNING_MESSAGE, repository.findRecentEvents(channelAccount.businessId(), 6));
	}
