...
public class MetaOnboardingRepository {
	...
	public Optional<ChannelAccountRecord> findCentralizedChannel() {
		return jdbcTemplate.query("""
				...
				""", new ChannelAccountRowMapper()).stream().findFirst();
	}
	...
	public Optional<ChannelAccountRecord> findCloudApiChannel(UUID businessId) {
		return jdbcTemplate.query("""
				...
				where ca.business_id = ?
				  ...
				  and ca.active = true
				order by ca.updated_at desc
				""", new ChannelAccountRowMapper(), businessId).stream().findFirst();
	}

	public ChannelAccountRecord findOrCreateCentralChannel(UUID businessId) {
		return findCloudApiChannel(businessId).orElseGet(() -> {
			...
			return findCloudApiChannel(businessId).orElseThrow(() -> new ApiException(HttpStatus.INTERNAL_SERVER_ERROR,
					"CHANNEL_CREATE_FAILED", "No se pudo crear el canal central META_CLOUD_API."));
		});
	}
	...
	public void validatePhoneNumberIdNotUsed(String phoneNumberId, UUID excludeBusinessId) {
		...
		Optional<UUID> existing = jdbcTemplate.query("""
				...
				where phone_number_id = ?
				  ...
				  and business_id <> ?
				limit 1
				""", (rs, rn) -> rs.getObject("business_id", UUID.class), phoneNumberId, excludeBusinessId).stream()
				.findFirst();
	...
	public void updateAfterOnboarding(UUID channelId, String phoneNumberId, String businessAccountId,
			String encryptedAccessToken, OffsetDateTime tokenExpiresAt) {
		int updated = jdbcTemplate.update("""
				...
				where id = ?
				  and provider_name = 'META_CLOUD_API'
				""", phoneNumberId, businessAccountId, encryptedAccessToken, tokenExpiresAt, channelId);
		if (updated == 0) {
	...
	public void updatePhoneMetadata(UUID channelId, String displayPhoneNumber, String normalizedPhoneNumber,
			String verifiedName) {
		jdbcTemplate.update("""
				update channel_account
				set display_phone_number = ?,
				    ...
				    updated_at = current_timestamp
				where id = ?
				""", displayPhoneNumber, normalizedPhoneNumber, verifiedName, channelId);
	}
	...
	public void updateOperationalStatus(UUID channelId, String operationalStatus, String lastErrorCode) {
		jdbcTemplate.update("""
				...
				set operational_status = ?,
				    ...
				    updated_at = current_timestamp
				where id = ?
				""", operationalStatus, lastErrorCode, channelId);
	}
	...
	public record ChannelAccountRecord(UUID id, UUID businessId, String providerName, String channelType, String status,
			String phoneNumber, String displayPhoneNumber, String normalizedPhoneNumber, String phoneNumberId,
			String providerAccountId, String registrationStatus, String operationalStatus, String webhookStatus,
			String credentialStatus, boolean active, String encryptedAccessToken, String encryptedVerifyToken,
			OffsetDateTime tokenExpiresAt, UUID locationId, String routingMode, String adapterMode,
			String lastErrorCode, String lastError, OffsetDateTime lastHealthCheckAt, int version,
			OffsetDateTime createdAt, OffsetDateTime updatedAt, String locationName, String locationCode) {
	}
	...
	private static class ChannelAccountRowMapper implements RowMapper<ChannelAccountRecord> {
		...
		public ChannelAccountRecord mapRow(ResultSet rs, int rowNum) throws SQLException {
			return new ChannelAccountRecord(rs.getObject("id", UUID.class), rs.getObject("business_id", UUID.class),
					rs.getString("provider_name"), rs.getString("channel_type"), rs.getString("status"),
					rs.getString("phone_number"), rs.getString("display_phone_number"),
					rs.getString("normalized_phone_number"), rs.getString("phone_number_id"),
					rs.getString("provider_account_id"), rs.getString("registration_status"),
					rs.getString("operational_status"), rs.getString("webhook_status"),
					rs.getString("credential_status"), rs.getBoolean("active"), rs.getString("encrypted_access_token"),
					rs.getString("encrypted_verify_token"), rs.getObject("token_expires_at", OffsetDateTime.class),
					rs.getObject("location_id", UUID.class), rs.getString("routing_mode"), rs.getString("adapter_mode"),
					rs.getString("last_error_code"), rs.getString("last_error"),
					rs.getObject("last_health_check_at", OffsetDateTime.class), rs.getInt("version"),
					rs.getObject("created_at", OffsetDateTime.class), rs.getObject("updated_at", OffsetDateTime.class),
					rs.getString("location_name"), rs.getString("location_code"));
		}
	}
}
