...
public class MetaGraphApiClient {
	...
	public MetaGraphApiClient(RestClient.Builder restClientBuilder, ObjectMapper objectMapper) {
		...
		this.restClient = restClientBuilder.baseUrl(GRAPH_BASE_URL)
				.defaultHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE).build();
	}

	public MetaTokenExchangeResponse exchangeCodeForToken(String code, String appId, String appSecret,
			String redirectUri) {
		try {
			String body = restClient.get()
					.uri(uriBuilder -> uriBuilder.path("/v23.0/oauth/access_token").queryParam("client_id", appId)
							.queryParam("client_secret", appSecret).queryParam("code", code)
					...
					.onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), (request, response) -> {
						...
						LOG.error("Meta OAuth token exchange failed: HTTP {} body={}", response.getStatusCode(),
								errorBody);
	...
	public MetaWabaInfo fetchWabaInfo(String accessToken, String wabaId) {
		try {
			String body = restClient.get()
					.uri(uriBuilder -> uriBuilder.path("/v23.0/{wabaId}")
			...
			if (id == null) {
				throw new ApiException(HttpStatus.BAD_GATEWAY, "META_WABA_INVALID", "Meta no devolvio un WABA valido.");
			}
	...
	public MetaPhoneNumberInfo fetchPhoneNumber(String accessToken, String phoneNumberId) {
		try {
			String body = restClient.get()
					.uri(uriBuilder -> uriBuilder.path("/v23.0/{phoneNumberId}")
							.queryParam("fields", "id,display_phone_number,verified_name,quality_rating,code_verified")
							.build(phoneNumberId))
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken).retrieve()
					.onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), (request, response) -> {
						String errorBody = new String(response.getBody().readAllBytes(), StandardCharsets.UTF_8);
						LOG.error("Meta phone number fetch failed: {} body={}", response.getStatusCode(), errorBody);
			...
			String normalized = displayPhone != null ? displayPhone.replaceAll("\\D", "") : "";
	...
	public void subscribeAppToWaba(String accessToken, String wabaId) {
		try {
			restClient.post().uri("/v23.0/{wabaId}/subscribed_apps", wabaId)
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken).retrieve()
					.onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), (request, response) -> {
						String errorBody = new String(response.getBody().readAllBytes(), StandardCharsets.UTF_8);
						LOG.error("Meta subscribe app to WABA failed: {} body={}", response.getStatusCode(), errorBody);
	...
	public void registerPhoneNumber(String accessToken, String phoneNumberId, String pin) {
		try {
			...
			restClient.post().uri("/v23.0/{phoneNumberId}/register", phoneNumberId)
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken).body(requestBody).retrieve()
					.onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), (request, response) -> {
						String errorBody = new String(response.getBody().readAllBytes(), StandardCharsets.UTF_8);
						LOG.error("Meta register phone failed: {} body={}", response.getStatusCode(), errorBody);
	...
	public void setTwoStepPin(String accessToken, String phoneNumberId, String pin) {
		try {
			...
			restClient.post().uri("/v23.0/{phoneNumberId}/two_step_pin", phoneNumberId)
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken).body(requestBody).retrieve()
					.onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), (request, response) -> {
						String errorBody = new String(response.getBody().readAllBytes(), StandardCharsets.UTF_8);
						LOG.warn("Meta two-step PIN set failed (non-critical): {} body={}", response.getStatusCode(),
								errorBody);
					}).body(Void.class);
		} catch (Exception e) {
	...
	public void unsubscribeAppFromWaba(String accessToken, String wabaId) {
		try {
			restClient.delete().uri("/v23.0/{wabaId}/subscribed_apps", wabaId)
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken).retrieve()
					.onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), (request, response) -> {
						String errorBody = new String(response.getBody().readAllBytes(), StandardCharsets.UTF_8);
						LOG.warn("Meta unsubscribe app from WABA failed: {} body={}", response.getStatusCode(),
								errorBody);
					}).body(Void.class);
		} catch (Exception e) {
	...
	public record MetaPhoneNumberInfo(String id, String displayPhoneNumber, String normalizedPhoneNumber,
			String verifiedName) {
	}
}
