...
@RestController
@RequestMapping(path = "/api/v1/integrations/whatsapp-cloud",
...
public class MetaOnboardingController {
    ...
    public ResponseEntity<OnboardingResult> completeOnboarding(
        ...
        OnboardingResult result = onboardingService.completeOnboarding(
                authenticatedUser.businessId(), request);
    ...
    public ResponseEntity<OnboardingStatus> getStatus(
            @AuthenticationPrincipal AuthenticatedUser authenticatedUser) {
        OnboardingStatus status = onboardingService.getStatus(
    ...
    public ResponseEntity<OnboardingResult> revalidate(
            @AuthenticationPrincipal AuthenticatedUser authenticatedUser) {
        OnboardingResult result = onboardingService.revalidate(
                authenticatedUser.businessId());
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<Void> disconnectFromMeta(
            @AuthenticationPrincipal AuthenticatedUser authenticatedUser) {
