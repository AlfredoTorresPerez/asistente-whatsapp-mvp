...
public class MetaOnboardingService {
    ...
    public MetaOnboardingService(
            MetaOnboardingRepository repository,
        ...
        this.allowedRedirectUris = List.of(
                webhookPublicUrl,
                "https://localhost",
    ...
    private void validateRedirectUri(String redirectUri) {
        ...
        boolean allowed = allowedRedirectUris.stream().anyMatch(uri ->
                redirectUri.startsWith(uri) || uri.startsWith(redirectUri));
        if (!allowed) {
    ...
    public OnboardingResult completeOnboarding(UUID businessId, CompleteOnboardingRequest request) {
        ...
        MetaTokenExchangeResponse tokenResponse = graphApiClient.exchangeCodeForToken(
                request.code(), metaAppId, metaAppSecret, request.redirectUri());
        ...
        MetaPhoneNumberInfo phoneInfo = graphApiClient.fetchPhoneNumber(
                systemAccessToken, request.phoneNumberId());
        ...
        repository.updateAfterOnboarding(
                channel.id(),
        ...
        repository.updatePhoneMetadata(
                channel.id(),
                phoneInfo.displayPhoneNumber(),
                phoneInfo.normalizedPhoneNumber(),
        ...
        LOG.info("Onboarding completado exitosamente para negocio {} canal {}",
        ...
        return new OnboardingResult(
                ...
                phoneInfo.displayPhoneNumber(),
                "CONNECTED",
                maskString(request.phoneNumberId()),
                maskString(request.wabaId()),
                "SUBSCRIBED");
    }
    ...
    public OnboardingStatus getStatus(UUID businessId) {
        ...
        return new OnboardingStatus(
                ...
                channel.normalizedPhoneNumber(),
                channel.registrationStatus(),
                channel.operationalStatus(),
                webhookStatus,
                channel.credentialStatus(),
                channel.active(),
                maskString(channel.phoneNumberId()),
                maskString(channel.providerAccountId()),
                "CENTRALIZED",
    ...
    public OnboardingResult revalidate(UUID businessId) {
        ...
        try {
            ...
            MetaPhoneNumberInfo phoneInfo = graphApiClient.fetchPhoneNumber(
                    decryptedToken, channel.phoneNumberId());
    ...
    public void disconnect(UUID businessId) {
        ...
        LOG.info("Canal META_CLOUD_API desconectado (solo local) para negocio {} canal {}",
                businessId, channel.id());
    }
    ...
    public void disconnectFromMeta(UUID businessId) {
        ...
        LOG.info("Canal META_CLOUD_API desconectado (local y Meta) para negocio {} canal {}",
                businessId, channel.id());
    }
