...
public class WhatsAppCloudWebhookParser {
    ...
    public void parseAndProcess(String rawBody) {
        ...
        for (Entry entry : payload.entry()) {
            ...
            for (Change change : entry.changes()) {
                ...
                WhatsAppWebChannelJdbcRepository.ChannelAccountRecord channelAccount = resolveChannelAccount(phoneNumberId);
    ...
    private void processMessageIdempotent(
            Message message,
            Value value,
    ...
    private void processStatusIdempotent(
            Status status,
            WhatsAppWebChannelJdbcRepository.ChannelAccountRecord channelAccount,
            UUID businessId,
            UUID channelAccountId,
    ...
    private WhatsAppWebChannelJdbcRepository.ChannelAccountRecord resolveChannelAccount(String phoneNumberId) {
        if (phoneNumberId != null && !phoneNumberId.isBlank()) {
            Optional<WhatsAppWebChannelJdbcRepository.ChannelAccountRecord> byPhoneNumberId =
                    repository.findChannelAccountByPhoneNumberId(phoneNumberId);
    ...
    private void processMessage(
        ...
        WhatsAppInboundMessageEvent event = parseMessage(message, from, messageId, timestamp, contactName, channelAccount);

        inboundMessageService.processInboundMessage(event, businessId, channelAccountId, idempotencyKey);
    }

    private WhatsAppInboundMessageEvent parseMessage(
            Message message,
            String from,
            String messageId,
        ...
        switch (type) {
            ...
            case "document" -> {
                ...
                if (message.document() != null) {
                    ...
                    metadata.put("filename", message.document().filename() != null ? message.document().filename() : "");
                }
        ...
        return new WhatsAppInboundMessageEvent(
                messageId,
                from,
                null,
                body != null ? body : "",
    ...
    private void processStatus(
        ...
        WhatsAppDeliveryStatusEvent event = new WhatsAppDeliveryStatusEvent(
                externalMessageId,
                businessId.toString(),
