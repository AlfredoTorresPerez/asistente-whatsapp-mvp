...
public class WhatsAppWebSimulatorController {
	...
	public WhatsAppWebSimulatorController(WhatsAppWebWebhookService whatsAppWebWebhookService,
			WhatsAppWebChannelJdbcRepository repository, WhatsAppWebClientProperties properties,
	...
	public StatusResponse simulateInbound(@RequestBody DemoIncomingMessageRequest request) {
		...
		try {
			...
			payload.put("externalMessageId",
					request.externalMessageId() != null ? request.externalMessageId() : "sim-" + UUID.randomUUID());
			payload.put("to", "");
