...
public class WhatsAppWebWebhookService {
    ...
    public WhatsAppWebWebhookService(
    ...
    @Transactional
    public StatusResponse handleWebhook(
            String rawBody,
        ...
        try {
            ...
            try {
                ...
                repository.markChannelEventProcessed(
                        channelAccount.businessId(),
                        resolvedDeliveryId,
                        "PROCESSED",
    ...
    private void processEvent(
            ...
            WhatsAppWebWebhookPayload request,
            String deliveryId) {
        ...
        switch (request.eventType()) {
            ...
            case "MESSAGE_SENT_EXTERNAL" -> handleMessageSentExternal(channelAccount, request.payload(), deliveryId, occurredAt);
            case "MESSAGE_ACK_UPDATED" -> handleMessageAckUpdated(channelAccount, request.payload(), deliveryId, occurredAt);
            default -> throw new ApiException(
                    HttpStatus.BAD_REQUEST,
                    "WHATSAPP_WEB_EVENT_UNSUPPORTED",
    ...
    private void handleMessageReceived(
        ...
        UUID assignedUserId = repository.findLatestConversation(
                        channelAccount.businessId(),
                        channelAccount.id(),
                        customer.id())
                .map(WhatsAppWebChannelJdbcRepository.ConversationRecord::assignedUserId)
                .or(() -> repository.findFirstActiveUserId(channelAccount.businessId()))
                .orElse(null);
        ...
        UUID inboundMessageId = repository.insertInboundMessage(
                ...
                deliveryId,
                occurredAt);
        AiTraceLogger.info("CONVERSATION_CONTEXT_LOADED", traceId, conversation.id(), inboundMessageId, "WhatsAppWebWebhookService",
                "customerId=" + customer.id()
                        + " contactLocation=" + conversation.locationName()
                        + " conversationLocation=" + conversation.locationName()
        ...
        if (effectiveConversation.locationId() != null && conversation.locationId() == null) {
            AiTraceLogger.info("CONVERSATION_LOCATION_ASSIGNED", traceId, effectiveConversation.id(), inboundMessageId, "WhatsAppWebWebhookService",
                    "source=INBOUND_MESSAGE locationId=" + effectiveConversation.locationId()
                            + " locationName=" + effectiveConversation.locationName());
        }
        repository.updateChannelAccount(
                channelAccount.id(),
                "CONNECTED",
                companyPhone.isBlank() ? channelAccount.phoneNumber() : companyPhone,
                channelAccount.lastQrCode(),
                occurredAt);

        if (agentCoordinatorService.autoReplyEnabled()) {
            boolean enqueued = aiReplyOutboxJdbcRepository.enqueue(
                    new AiReplyOutboxJdbcRepository.InboundAiReplyJob(
                            channelAccount.businessId(),
                            channelAccount.id(),
                            ...
                            customer.displayName(),
                            body,
                            effectiveConversation.locationId(),
                            effectiveConversation.locationName(),
                            traceId),
                    occurredAt,
                    5);
            AiTraceLogger.info("AI_OUTBOX_ENQUEUED", traceId, effectiveConversation.id(), inboundMessageId, "WhatsAppWebWebhookService",
                    "enqueued=" + enqueued
                            + " phoneMasked=" + AiTraceLogger.maskPhone(normalizedPhone)
                            + " source=MESSAGE_RECEIVED");
        } else {
            AiTraceLogger.warn("AI_OUTBOX_SKIPPED", traceId, effectiveConversation.id(), inboundMessageId, "WhatsAppWebWebhookService",
    ...
    private void handleMessageSentExternal(
            WhatsAppWebChannelJdbcRepository.ChannelAccountRecord channelAccount,
            JsonNode payload,
            String deliveryId,
            OffsetDateTime occurredAt) {
        ...
        UUID messageId = repository.insertExternalOutboundMessage(
                channelAccount.businessId(),
                conversation.id(),
                body,
                externalMessageId,
                providerEventId == null ? deliveryId : providerEventId,
                occurredAt);
        repository.updateConversationOutboundActivity(conversation.id(), body, occurredAt);
        repository.updateChannelAccount(
                channelAccount.id(),
        ...
        auditLogJdbcRepository.insert(
                channelAccount.businessId(),
                assignedUserId,
                "WHATSAPP_WEB_EXTERNAL_MESSAGE_SENT",
                "CONVERSATION",
                conversation.id(),
    ...
    private void handleMessageAckUpdated(
            WhatsAppWebChannelJdbcRepository.ChannelAccountRecord channelAccount,
            JsonNode payload,
            String deliveryId,
            OffsetDateTime occurredAt) {
        ...
        repository.findMessageIdByExternalMessageId(channelAccount.businessId(), externalMessageId).ifPresent(messageId -> {
    ...
    private void validateHeaders(
        ...
        if (timestampHeader == null || signatureHeader == null || deliveryIdHeader == null) {
            throw new ApiException(
                    ...
                    "WHATSAPP_WEB_SIGNATURE_MISSING",
        ...
        try {
            ...
            throw new ApiException(
                    HttpStatus.UNAUTHORIZED,
                    "WHATSAPP_WEB_TIMESTAMP_INVALID",
    ...
    private record WhatsAppWebWebhookPayload(
            String eventType,
            String deliveryId,
            OffsetDateTime occurredAt,
            String sessionKey,
