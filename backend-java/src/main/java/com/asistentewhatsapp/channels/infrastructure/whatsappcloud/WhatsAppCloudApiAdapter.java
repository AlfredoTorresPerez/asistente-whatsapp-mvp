...
public class WhatsAppCloudApiAdapter implements CanalWhatsApp {
	...
	public WhatsAppCloudApiAdapter(RestClient.Builder restClientBuilder, WhatsAppCloudApiProperties properties,
			WhatsAppCloudApiMetrics metrics, ObjectMapper objectMapper, MetaOnboardingRepository onboardingRepository,
		...
		this.restClient = restClientBuilder.baseUrl(normalizeBaseUrl(properties.baseUrl()))
				.defaultHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
				.requestFactory(createRequestFactory()).build();
	}
	...
	public WhatsAppSessionStatus getStatus() {
		...
		return new WhatsAppSessionStatus("cloud-api", "CONNECTED",
				properties.defaultPhoneNumber() != null ? properties.defaultPhoneNumber() : "Multi-tenant", null,
				adapterMode, OffsetDateTime.now(ZoneOffset.UTC));
	}

	@Override
	public WhatsAppSessionAction connect() {
		WhatsAppSessionStatus status = getStatus();
		return new WhatsAppSessionAction(status.sessionId(), status.connectionStatus(), status.phoneNumber(), null,
	...
	public WhatsAppSessionAction disconnect() {
		return new WhatsAppSessionAction("cloud-api", "DISCONNECTED", properties.defaultPhoneNumber(), null,
				OffsetDateTime.now(ZoneOffset.UTC));
	}
	...
	public ChannelDelivery send(OutboundMessage outboundMessage) {
		if (properties.dryRunEnabled()) {
			...
			return new ChannelDelivery(MessageChannelType.WHATSAPP, "cloud-dry-run-" + UUID.randomUUID(), "SIMULATED",
					Instant.now());
		}
		...
		try {
			CloudApiSendMessageResponse response = restClient.post()
					.uri("/{apiVersion}/{phoneNumberId}/messages", properties.apiVersion(), credentials.phoneNumberId())
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + credentials.accessToken())
					.body(buildTextBody(recipient, outboundMessage.body(), null)).retrieve()
					.onStatus(HttpStatusCode::is4xxClientError, (request, resp) -> {
						...
						String errorType = statusCode == 429
								? "rate_limited"
								: statusCode == 401
										? "unauthorized"
										: statusCode == 400 ? "bad_request" : "client_error_" + statusCode;
						metrics.incrementApiErrors(errorType);
						LOG.warn("WhatsApp Cloud API {} error for business {}: HTTP {} metaCode={}", errorType,
								businessId, statusCode, metaErrorCode != null ? metaErrorCode : "unknown");
						if (statusCode == 429) {
						...
						if (retryAfter != null) {
							LOG.warn("WhatsApp Cloud API {} error for business {}, Retry-After: {}s", statusCode,
									businessId, retryAfter);
						}
						throw new MessagingChannelUnavailableException(
			...
			LOG.info("Message sent via Cloud API for business {}: externalMessageId={}", businessId, externalMessageId);

			return new ChannelDelivery(MessageChannelType.WHATSAPP, externalMessageId, "PROVIDER_ACCEPTED",
					Instant.now());
			...
			throw new MessagingChannelUnavailableException("No fue posible entregar el mensaje a WhatsApp Cloud API.");
		}
	}

	public String sendTemplate(UUID businessId, String recipientPhone, String templateName, String languageCode,
			List<TemplateComponent> components, String contextMessageId) {
		if (properties.dryRunEnabled()) {
			metrics.incrementMessagesSent();
		...
		try {
			Map<String, Object> body = buildTemplateBody(recipient, templateName,
					languageCode != null ? languageCode : "es", components, contextMessageId);

			CloudApiSendMessageResponse response = restClient.post()
					.uri("/{apiVersion}/{phoneNumberId}/messages", properties.apiVersion(), credentials.phoneNumberId())
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + credentials.accessToken()).body(body).retrieve()
					.onStatus(HttpStatusCode::is4xxClientError, (request, resp) -> {
						long elapsed = System.currentTimeMillis() - startTime;
						metrics.recordApiCall(elapsed, false);
						metrics.incrementApiErrors("template_send_error");
						throw new MessagingChannelUnavailableException("WhatsApp Cloud API rechazo la plantilla.");
					}).onStatus(HttpStatusCode::is5xxServerError, (request, resp) -> {
						long elapsed = System.currentTimeMillis() - startTime;
						metrics.recordApiCall(elapsed, false);
						metrics.incrementApiErrors("template_server_error");
						throw new MessagingChannelUnavailableException("Error del servidor al enviar plantilla.");
					}).body(CloudApiSendMessageResponse.class);
			...
			throw new MessagingChannelUnavailableException("No fue posible enviar la plantilla a WhatsApp Cloud API.");
		}
	}
	...
	private Map<String, Object> buildTemplateBody(String recipient, String templateName, String languageCode,
			List<TemplateComponent> components, String contextMessageId) {
		Map<String, Object> body = new java.util.LinkedHashMap<>();
		body.put("messaging_product", "whatsapp");
		body.put("to", recipient);
	...
	private ClientHttpRequestFactory createRequestFactory() {
		...
		int connectTimeout = properties.connectTimeoutSeconds() > 0 ? properties.connectTimeoutSeconds() * 1000 : 5000;
		int readTimeout = properties.readTimeoutSeconds() > 0 ? properties.readTimeoutSeconds() * 1000 : 15000;
		factory.setConnectTimeout(Duration.ofMillis(connectTimeout));
		factory.setReadTimeout(Duration.ofMillis(readTimeout));
		return factory;
	}

	public record TemplateComponent(String type, List<TemplateParameter> parameters) {
	}

	public record TemplateParameter(String type, String value, String parameterName) {
	}
