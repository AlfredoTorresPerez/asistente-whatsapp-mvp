...
public class OpenWaMessagingChannel implements MessagingChannel {
    ...
    public ChannelDelivery send(OutboundMessage outboundMessage) {
        try {
            OpenWaSendMessageResponse response = restClient.post()
                    .uri("/api/v1/messages/send")
                    .body(new OpenWaSendMessageRequest(
                            outboundMessage.businessId().toString(),
                            outboundMessage.recipientPhone(),
                            outboundMessage.body()))
                    .retrieve()
                    .body(OpenWaSendMessageResponse.class);

            if (response == null) {
                throw new MessagingChannelUnavailableException(
                        "El adaptador OpenWA no devolvio una respuesta valida.");
            }
            ...
            throw new MessagingChannelUnavailableException(
                    "No fue posible entregar el mensaje al adaptador OpenWA.");
        }
    }
