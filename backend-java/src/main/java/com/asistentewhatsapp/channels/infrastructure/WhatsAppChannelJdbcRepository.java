...
public class WhatsAppChannelJdbcRepository {
	...
	public WhatsAppChannelJdbcRepository(JdbcTemplate jdbcTemplate, ObjectMapper objectMapper) {
	...
	public Optional<ChannelAccountRecord> findChannelAccountBySessionKey(String sessionKey) {
		List<ChannelAccountRecord> items = jdbcTemplate.query("""
				...
				where session_key = ?
				limit 1
				""", new ChannelAccountRowMapper(), sessionKey);
		return items.stream().findFirst();
	}
	...
	public Optional<ChannelAccountRecord> findChannelAccountByPhoneNumber(String phoneNumber) {
		List<ChannelAccountRecord> items = jdbcTemplate.query("""
				...
				order by created_at desc
				limit 1
				""", new ChannelAccountRowMapper(), phoneNumber);
		return items.stream().findFirst();
	}
	...
	public Optional<ChannelAccountRecord> findChannelAccountByPhoneNumberId(String phoneNumberId) {
		...
		List<ChannelAccountRecord> items = jdbcTemplate.query("""
				...
				where provider_name = 'META_CLOUD_API'
				  and phone_number_id = ?
				limit 1
				""", new ChannelAccountRowMapper(), phoneNumberId);
		return items.stream().findFirst();
	}

	public UUID upsertSimulatedChannelAccount(UUID businessId, String sessionKey, String phoneNumber,
		...
		return jdbcTemplate.queryForObject("""
				...
				do update set
				    ...
				    updated_at = now()
				returning id
				""", UUID.class, UUID.randomUUID(), businessId, sessionKey, status, phoneNumber, phoneNumberId);
	}

	public void updateChannelAccount(UUID channelAccountId, String status, String phoneNumber, String qrCode,
		...
		jdbcTemplate.update(
				"""
						...
						where id = ?
						""",
				status, phoneNumber, qrCode, hasQrCode, lastEventAt, lastEventAt, status, lastEventAt, status,
				"DISCONNECTED".equals(status) ? lastEventAt : null, channelAccountId);
	}

	public boolean insertChannelEventLog(UUID businessId, UUID channelAccountId, String deliveryId, String eventType,
			String payloadJson, OffsetDateTime receivedAt) {
		...
		try {
			int inserted = jdbcTemplate.update("""
					...
					""", UUID.randomUUID(), businessId, channelAccountId, deliveryId, eventType, safePayload,
					receivedAt);
			return inserted > 0;
	...
	public void markChannelEventProcessed(UUID businessId, String deliveryId, String processingStatus,
		...
		jdbcTemplate.update("""
				...
				where business_id = ?
				  and delivery_id = ?
				""", processingStatus, processedAt, businessId, deliveryId);
	}

	public List<ChannelEventLogRecord> findRecentEvents(UUID businessId, int limit) {
		return jdbcTemplate.query("""
				...
				limit ?
				""",
				(resultSet, rowNum) -> new ChannelEventLogRecord(resultSet.getString("delivery_id"),
						resultSet.getString("event_type"), resultSet.getString("processing_status"),
						resultSet.getObject("received_at", OffsetDateTime.class),
						resultSet.getObject("processed_at", OffsetDateTime.class)),
				businessId, limit);
	}

	public int countRecentEventErrors(UUID businessId, int lastMinutes) {
		Integer count = jdbcTemplate.queryForObject("""
				select count(*) from channel_event_log
				where business_id = ?
				  and processing_status in ('FAILED', 'ERROR')
				  and received_at > current_timestamp - (? || ' minutes')::interval
				""", Integer.class, businessId, lastMinutes);
	...
	public Optional<String> findPhoneNumberIdByBusinessId(UUID businessId) {
		List<String> items = jdbcTemplate.query("""
				...
				limit 1
				""", (resultSet, rowNum) -> resultSet.getString("phone_number_id"), businessId);
		return items.stream().findFirst();
	}
	...
	public Optional<CustomerRecord> findCustomerByPhone(UUID businessId, String normalizedPhone) {
		List<CustomerRecord> items = jdbcTemplate.query("""
				...
				limit 1
				""",
				(resultSet, rowNum) -> new CustomerRecord(resultSet.getObject("id", UUID.class),
						resultSet.getString("display_name"), resultSet.getString("phone"),
						resultSet.getString("normalized_phone")),
				businessId, normalizedPhone);
		return items.stream().findFirst();
	}

	public UUID insertCustomer(UUID businessId, String normalizedPhone, String displayName) {
		...
		jdbcTemplate.update("""
				...
				""", customerId, businessId, firstName, lastName, displayName, normalizedPhone, normalizedPhone);
		return customerId;
	}

	public Optional<ConversationRecord> findLatestConversation(UUID businessId, UUID channelAccountId,
			UUID customerId) {
		List<ConversationRecord> items = jdbcTemplate.query("""
				...
				limit 1
				""", conversationRowMapper(), businessId, channelAccountId, customerId);
		return items.stream().findFirst();
	}
	...
	public Optional<ConversationRecord> findConversationById(UUID businessId, UUID conversationId) {
		List<ConversationRecord> items = jdbcTemplate.query("""
				...
				where c.business_id = ?
				  and c.id = ?
				limit 1
				""", conversationRowMapper(), businessId, conversationId);
		return items.stream().findFirst();
	}

	public Optional<ConversationRecord> assignConversationLocationFromMessageIfBlank(UUID businessId,
			UUID conversationId, String messageBody) {
		...
		int updated = jdbcTemplate.update(
				"""
						...
						""",
				businessId, messageBody, messageBody, messageBody, businessId, conversationId);
		if (updated == 0) {
			return Optional.empty();
		}
	...
	private RowMapper<ConversationRecord> conversationRowMapper() {
		return (resultSet, rowNum) -> new ConversationRecord(resultSet.getObject("id", UUID.class),
				resultSet.getObject("assigned_user_id", UUID.class), resultSet.getInt("unread_count"),
	...
	public UUID insertConversation(UUID businessId, UUID channelAccountId, UUID customerId, UUID assignedUserId,
		...
		jdbcTemplate.update("""
				...
				""", conversationId, businessId, channelAccountId, customerId, assignedUserId, customerName,
				customerPhone, openedAt, openedAt, null);
		return conversationId;
	}

	public UUID insertInboundMessage(UUID businessId, UUID conversationId, String body, String externalMessageId,
			String providerEventId, OffsetDateTime receivedAt) {
		...
		jdbcTemplate.update("""
				...
				""", messageId, businessId, conversationId, body, externalMessageId, providerEventId, receivedAt,
				receivedAt);
		return messageId;
	}
	...
	public UUID insertOutboundMessage(UUID businessId, UUID conversationId, UUID sentByUserId, String body,
		...
		jdbcTemplate.update("""
				...
				""", messageId, businessId, conversationId, sentByUserId, body, createdAt);
		return messageId;
	}

	public UUID insertExternalOutboundMessage(UUID businessId, UUID conversationId, String body,
			String externalMessageId, String providerEventId, OffsetDateTime sentAt) {
		UUID messageId = UUID.randomUUID();
		jdbcTemplate.update("""
				insert into message (
				    id,
				...
				""", messageId, businessId, conversationId, body, externalMessageId, providerEventId, sentAt);
		return messageId;
	}

	public void updateOutboundMessageAccepted(UUID messageId, String externalMessageId, String deliveryStatus,
		...
		jdbcTemplate.update("""
				...
				set external_message_id = ?,
				    ...
				    updated_at = current_timestamp
				where id = ?
				""", externalMessageId, deliveryStatus, sentAt, messageId);
	}
	...
	public void updateOutboundMessageFailed(UUID messageId, String errorCode, OffsetDateTime failedAt) {
		jdbcTemplate.update("""
				...
				set status = 'FAILED',
				    ...
				    updated_at = current_timestamp
				where id = ?
				""", errorCode, failedAt, messageId);
	}

	public void updateConversationInboundActivity(UUID conversationId, String preview, OffsetDateTime lastMessageAt) {
		jdbcTemplate.update("""
				update conversation
				set status = 'OPEN',
				    ...
				    updated_at = current_timestamp
				where id = ?
				""", lastMessageAt, truncatePreview(preview), conversationId);
	}

	public void updateConversationOutboundActivity(UUID conversationId, String preview, OffsetDateTime lastMessageAt) {
		jdbcTemplate.update("""
				update conversation
				set last_message_at = ?,
				    last_message_preview = ?,
	...
	public Optional<UUID> findFirstActiveUserId(UUID businessId) {
		List<UUID> items = jdbcTemplate.query("""
				...
				order by created_at asc
				limit 1
				""", (resultSet, rowNum) -> resultSet.getObject("id", UUID.class), businessId);
		return items.stream().findFirst();
	}

	public void insertNotification(UUID businessId, UUID userId, String title, String body, UUID conversationId) {
		jdbcTemplate.update("""
				...
				""", UUID.randomUUID(), businessId, userId, title, body, conversationId);
	}
	...
	public Optional<UUID> findMessageIdByExternalMessageId(UUID businessId, String externalMessageId) {
		...
		List<UUID> items = jdbcTemplate.query("""
				...
				limit 1
				""", (resultSet, rowNum) -> resultSet.getObject("id", UUID.class), businessId, externalMessageId);
		return items.stream().findFirst();
	}

	public void updateMessageStatus(UUID messageId, String status, String providerEventId, OffsetDateTime occurredAt) {
		jdbcTemplate.update(
				"""
						update message
						set status = ?,
						...
						where id = ?
						""",
				status, providerEventId, status, occurredAt, status, "FAILED".equals(status) ? occurredAt : null,
				messageId);
	}

	public void insertMessageDeliveryLog(UUID businessId, UUID messageId, String deliveryStatus, String providerEventId,
			Object providerPayload, OffsetDateTime occurredAt) {
		jdbcTemplate.update("""
				...
				""", UUID.randomUUID(), businessId, messageId, deliveryStatus, providerEventId, toJson(providerPayload),
				occurredAt);
	}
	...
	public record ChannelAccountRecord(UUID id, UUID businessId, String sessionKey, String status, String phoneNumber,
			String lastQrCode, OffsetDateTime lastQrAt, OffsetDateTime lastEventAt) {
	}

	public record CustomerRecord(UUID id, String displayName, String phone, String normalizedPhone) {
	}

	public record ConversationRecord(UUID id, UUID assignedUserId, int unreadCount, UUID locationId,
			String locationName) {
	}

	public record ChannelEventLogRecord(String deliveryId, String eventType, String processingStatus,
	...
	private static class ChannelAccountRowMapper implements RowMapper<ChannelAccountRecord> {
		...
		public ChannelAccountRecord mapRow(ResultSet resultSet, int rowNum) throws SQLException {
			return new ChannelAccountRecord(resultSet.getObject("id", UUID.class),
					resultSet.getObject("business_id", UUID.class), resultSet.getString("session_key"),
					resultSet.getString("status"), resultSet.getString("phone_number"),
					resultSet.getString("last_qr_code"), resultSet.getObject("last_qr_at", OffsetDateTime.class),
