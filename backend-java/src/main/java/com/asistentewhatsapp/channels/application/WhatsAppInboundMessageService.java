...
public class WhatsAppInboundMessageService {
    ...
    public WhatsAppInboundMessageService(
    ...
    public void processInboundMessage(
            WhatsAppInboundMessageEvent event,
            UUID businessId,
        ...
        OffsetDateTime occurredAt = event.timestamp() != null
                ? event.timestamp()
        ...
        WhatsAppWebChannelJdbcRepository.CustomerRecord customer = repository.findCustomerByPhone(businessId, normalizedPhone)
                .orElseGet(() -> new WhatsAppWebChannelJdbcRepository.CustomerRecord(
                        repository.insertCustomer(businessId, normalizedPhone, displayName),
                        displayName,
                        normalizedPhone,
                        normalizedPhone));
        ...
        if (event.externalMessageId() != null && !event.externalMessageId().isBlank()
            ...
            AiTraceLogger.info("DUPLICATE_MESSAGE_SKIPPED", traceId, conversation.id(), null,
                    ...
                    "externalMessageId=" + LogSanitizer.maskExternalId(event.externalMessageId())
                            + " phoneMasked=" + AiTraceLogger.maskPhone(normalizedPhone));
            return;
        }

        UUID inboundMessageId = repository.insertInboundMessage(
                businessId, conversation.id(), bodyText,
        ...
        if (agentCoordinatorService.autoReplyEnabled()) {
            boolean enqueued = aiReplyOutboxJdbcRepository.enqueue(
                    new AiReplyOutboxJdbcRepository.InboundAiReplyJob(
                            businessId, channelAccountId, conversation.id(),
                            customer.id(), inboundMessageId, normalizedPhone,
                            customer.displayName(), bodyText,
        ...
        if (assignedUserId != null) {
            repository.insertNotification(
                    businessId, assignedUserId,
                    "Nuevo mensaje recibido", bodyText, conversation.id());
        }

        auditLogJdbcRepository.insert(
                businessId, assignedUserId,
                "WHATSAPP_CLOUD_MESSAGE_RECEIVED",
                "CONVERSATION", conversation.id(),
                "Mensaje entrante recibido desde WhatsApp Cloud API.",
                Map.of(
                        "deliveryId", deliveryId,
                        "from", normalizedPhone,
                        "externalMessageId", event.externalMessageId() == null ? "" : event.externalMessageId(),
