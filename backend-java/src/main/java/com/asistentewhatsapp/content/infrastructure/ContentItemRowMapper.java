...
public class ContentItemRowMapper implements RowMapper<ContentItemRecord> {
	...
	public ContentItemRecord mapRow(ResultSet rs, int rowNum) throws SQLException {
		return new ContentItemRecord(UUID.fromString(rs.getString("id")), UUID.fromString(rs.getString("business_id")),
				ContentItemType.valueOf(rs.getString("type")), rs.getString("image_path"), rs.getString("text"),
