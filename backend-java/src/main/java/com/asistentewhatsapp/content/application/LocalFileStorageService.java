...
public class LocalFileStorageService implements FileStorageService {
	...
	public LocalFileStorageService(@Value("${app.content.storage.root:./storage/content}") String storageRoot,
	...
	@Override
	public String store(String businessId, String originalFilename, InputStream inputStream, long contentLength,
			String contentType) {
	...
	@Override
	public void validateImage(String originalFilename, String contentType, InputStream inputStream,
			long contentLength) {
		if (contentLength > maxFileSize) {
			throw new IllegalArgumentException(
					"El archivo supera el tamaño maximo permitido: " + (maxFileSize / 1024 / 1024) + " MB");
		}
		if (contentLength == 0) {
		...
		if (!Arrays.asList(allowedExtensions).contains(extension)) {
			throw new IllegalArgumentException(
					"Extension no permitida: " + extension + ". Permitidas: " + Arrays.toString(allowedExtensions));
		}

		if (contentType != null && !Arrays.asList(allowedMimeTypes).contains(contentType.toLowerCase())) {
			throw new IllegalArgumentException(
					"Tipo MIME no permitido: " + contentType + ". Permitidos: " + Arrays.toString(allowedMimeTypes));
		}
