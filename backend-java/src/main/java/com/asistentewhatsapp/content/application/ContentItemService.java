...
public class ContentItemService {
    ...
    public ContentItemService(
            ContentItemJdbcRepository repository,
    ...
    public ContentItemListResponse list(AuthenticatedUser user, ContentItemListRequest request) {
        ...
        List<ContentItemRecord> items = repository.findAll(
                businessId, type, status, request.page(), request.size());

        List<ContentItemSummaryResponse> summaries = items.stream()
                .map(this::toSummaryResponse)
                .toList();

        return new ContentItemListResponse(
                summaries,
                request.page(),
                request.size(),
                totalItems,
    ...
    public ContentItemDetailResponse create(AuthenticatedUser user, CreateContentItemRequest request, MultipartFile imageFile) {
    ...
    public ContentItemDetailResponse update(AuthenticatedUser user, UUID id, UpdateContentItemRequest request, MultipartFile imageFile) {
    ...
    public ContentItemImageUploadResponse uploadImage(AuthenticatedUser user, UUID id, MultipartFile imageFile) {
        ...
        auditService.record(user.businessId(), user.userId(), "CONTENT_ITEM_IMAGE_REPLACED", "CONTENT_ITEM", id,
                "Imagen reemplazada",
                java.util.Map.of("newImagePath", newImagePath));
    ...
    public void delete(AuthenticatedUser user, UUID id) {
        ...
        auditService.record(user.businessId(), user.userId(), "CONTENT_ITEM_DELETED", "CONTENT_ITEM", id,
                "Contenido eliminado",
                java.util.Map.of("type", existing.type().name()));
    }

    @Transactional(readOnly = true)
    public List<PublicContentItemResponse> getPublicContent(UUID businessId, ContentItemType type) {
        return repository.findPublicActive(businessId, type).stream()
    ...
    private void validateCreateRequest(CreateContentItemRequest request) {
        ...
        if (request.text().trim().length() > 200) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "TEXT_TOO_LONG", "El texto no puede superar 200 caracteres.");
        }
    ...
    private ContentItemSummaryResponse toSummaryResponse(ContentItemRecord record) {
        return new ContentItemSummaryResponse(
                ...
                record.imagePath(),
                record.imagePath() != null ? storageService.getPublicUrl(record.imagePath()) : null,
    ...
    private PublicContentItemResponse toPublicResponse(ContentItemRecord record) {
        return new PublicContentItemResponse(
                record.id(),
                record.type().name(),
                record.text(),
                record.imagePath() != null ? storageService.getPublicUrl(record.imagePath()) : null,
    ...
    private ContentItemStatsResponse getStats(UUID businessId) {
        return new ContentItemStatsResponse(
                repository.count(businessId, null, null),
                repository.countByTypeAndStatus(businessId, null, "ACTIVE"),
                repository.countByTypeAndStatus(businessId, null, "INACTIVE"),
                repository.findAll(businessId, null, null, 0, 1000).stream()
                        .filter(r -> r.imagePath() == null)
                        .count()
        );
    }
}