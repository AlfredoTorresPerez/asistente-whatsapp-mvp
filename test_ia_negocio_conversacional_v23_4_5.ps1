...
$cases = @(
    @{ Id="T01"; Categoria="Contacto sin sucursal con Providencia"; Pregunta="Hola, quiero reservar limpieza facial profunda manana a las 10:00 en Providencia"; Intent="reservar_hora"; Min=70; Must=@("providencia", "limpieza facial profunda", "10:00"); MustAny=@("reservas/confirmar", "hay disponibilidad", "no encontre disponibilidad", "no esta configurado"); MustNot=@("si falta sucursal", "que sucursal", "en que sucursal", "404", "api client error") },
...
Write-Host ""
Write-Host "Ejecutando test funcional V23.4.5 contra: $analyzeUrl"
...
$outDir = Join-Path (Get-Location) "resultados-test-ia-v23-4-5"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$csvPath = Join-Path $outDir "resultado_test_ia_negocio_v23_4_5_$timestamp.csv"
$jsonPath = Join-Path $outDir "resultado_test_ia_negocio_v23_4_5_$timestamp.json"
$htmlPath = Join-Path $outDir "resultado_test_ia_negocio_v23_4_5_$timestamp.html"
...
<head>
<meta charset="utf-8" />
<title>Resultado test IA negocio V23.4.5</title>
<style>
...
<body>
<h1>Resultado test IA negocio V23.4.5</h1>
...
if ($TestConversationRealSend -and -not [string]::IsNullOrWhiteSpace($ConversationId)) {
    ...
    try {
        ...
        $sendPayload = @{ body = $suggestedBody; idempotencyKey = "v23-4-5-real-send-$timestamp" } | ConvertTo-Json -Depth 5
        ...
        Write-Host "Valida en logs que aparezcan: AI_REAL_SEND_STARTED, TEMPORARY_BOOKING_CREATE_STARTED, TEMPORARY_BOOKING_CREATED, CONFIRMATION_LINK_CREATED, WHATSAPP_RESPONSE_SEND_RESULT" -ForegroundColor Green
    } catch {
...
Write-Host "docker compose -f docker-compose.local.yml logs --tail=1000 backend-java > logs_backend_v23_4_5.txt"
