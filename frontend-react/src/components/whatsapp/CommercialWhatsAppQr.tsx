...
type CommercialWhatsAppQrProps = {
  waUrl: string
...
export function CommercialWhatsAppQr({
  waUrl,
  displayPhoneNumber,
  prefilledMessage,
  locationName,
  size = 200,
}: CommercialWhatsAppQrProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 sm:flex-row">
      <div className="flex-shrink-0">
        <QRCodeSVG size={size} value={waUrl} />
      </div>
      <div className="flex flex-1 flex-col items-center gap-3 text-center sm:items-start sm:text-left">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Escanea el codigo QR o haz clic en el boton para reservar via WhatsApp
          </p>
          <p className="mt-2 text-sm font-semibold text-slate-900">{displayPhoneNumber}</p>
          <p className="mt-1 text-xs text-slate-500">{prefilledMessage}</p>
        </div>
        ...
        <p className="text-xs text-slate-400">
          Este codigo abre una conversacion; no vincula tu cuenta.
        </p>
      </div>
    </div>
  )
