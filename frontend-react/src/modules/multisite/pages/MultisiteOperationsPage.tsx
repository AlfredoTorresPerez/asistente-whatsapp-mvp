...
export function MultisiteOperationsPage() {
  ...
  const summaryQuery = useQuery({
    queryKey: ['multisite', 'summary'],
    queryFn: getMultisiteSummaryRequest,
  })
  const catalogQuery = useQuery({
    queryKey: ['multisite', 'catalog-availability', selectedLocationId],
    queryFn: () =>
      getMultisiteCatalogAvailabilityRequest({ locationId: selectedLocationId || null }),
  })
  const professionalsQuery = useQuery({
    queryKey: ['multisite', 'professionals'],
    queryFn: getMultisiteProfessionalsRequest,
  })
  ...
  const userAccessQuery = useQuery({
    queryKey: ['multisite', 'user-access'],
    queryFn: getMultisiteUserAccessRequest,
  })
  const channelsQuery = useQuery({
    queryKey: ['multisite', 'channels'],
    queryFn: getMultisiteChannelsRequest,
  })
  ...
  const saveScheduleMutation = useMutation({
    mutationFn: (payload: UpsertProfessionalScheduleRequest) =>
      createMultisiteScheduleRequest(payload),
    onSuccess: () => {
      showToast({
        title: 'Horario guardado',
        description: 'El horario por sede quedo disponible para validar agenda.',
        tone: 'success',
      })
    ...
    onError: () =>
      showToast({
        title: 'No se pudo guardar horario',
        description: 'Verifica sede, profesional y rango horario.',
        tone: 'error',
      }),
  })

  const saveCatalogMutation = useMutation({
    mutationFn: (payload: UpsertCatalogAvailabilityRequest) =>
      updateMultisiteCatalogAvailabilityRequest(payload),
    onSuccess: () => {
      showToast({
  ...
  const updateAccessMutation = useMutation({
    mutationFn: (access: UserLocationAccessResponse) =>
      updateMultisiteUserAccessRequest({
    ...
    onSuccess: () => {
      showToast({
        ...
        description: 'El acceso por sede fue actualizado.',
        tone: 'success',
      })
    ...
    onError: () =>
      showToast({
        title: 'No se pudo actualizar permiso',
        description: 'Revisa el usuario y la sede.',
        tone: 'error',
      }),
  })

  const updateChannelMutation = useMutation({
    mutationFn: (channel: MultisiteChannelResponse) =>
      updateMultisiteChannelLocationRequest(channel.channelId, {
    ...
    onSuccess: () => {
      showToast({
        ...
        description: 'La configuracion de canal por sede fue persistida.',
        tone: 'success',
      })
    ...
    onError: () =>
      showToast({
        title: 'No se pudo actualizar canal',
        description: 'El canal puede tener una restriccion de unicidad por sede.',
        tone: 'error',
      }),
  })
  ...
  const submitSchedule = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!scheduleForm.professionalId || !scheduleForm.locationId) {
      showToast({
        title: 'Faltan datos',
        description: 'Selecciona profesional y sede.',
        tone: 'error',
      })
      return
    }
  ...
  return (
    <section className="space-y-6">
      ...
      {isLoading ? (
        <LoadingState message="Cargando configuracion multisede." variant="page" />
      ) : null}
      {hasError ? (
        <ErrorState
          title="No fue posible cargar operacion multisede"
          description="Revisa que el backend tenga aplicada la migracion V18 y que la sesion este vigente."
        />
      ) : null}
      ...
      {!hasError ? (
        <>
          ...
          {activeTab === 'permissions' ? (
            <PermissionsPanel
              access={userAccess}
              onToggle={(access) => updateAccessMutation.mutate(access)}
              updating={updateAccessMutation.isPending}
            />
          ) : null}
          {activeTab === 'channels' ? (
            <ChannelsPanel
              channels={channels}
              locations={locations}
              onSave={(channel) => updateChannelMutation.mutate(channel)}
              updating={updateChannelMutation.isPending}
            />
          ) : null}
    ...
    <div className="grid gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
      <Card>
        ...
        <form className="mt-5 space-y-4" onSubmit={onSubmit}>
          ...
          <select
            className="h-12 w-full rounded-[14px] border border-[var(--color-border)] bg-white px-4 text-sm"
            value={catalogForm.locationId}
            onChange={(event) => onChange({ ...catalogForm, locationId: event.target.value })}
          >
          ...
          <Input
            label="Precio especifico de sede"
            type="number"
            value={catalogForm.priceOverride}
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
      <Card>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Asocia profesional, sucursal y horario para validar disponibilidad real.
        </p>
        <form className="mt-5 space-y-4" onSubmit={onSubmit}>
          ...
          <select
            ...
            onChange={(event) => onChange({ ...scheduleForm, professionalId: event.target.value })}
          >
          ...
          <select
            ...
            onChange={(event) => onChange({ ...scheduleForm, locationId: event.target.value })}
          >
            <option value="">Seleccionar sede</option>
            {locations.map((location) => (
              <option key={location.locationId} value={location.locationId}>
          ...
          <div className="grid grid-cols-3 gap-3">
            <Input
              ...
              onChange={(event) => onChange({ ...scheduleForm, dayOfWeek: event.target.value })}
            />
            <Input
              label="Inicio"
              type="time"
              value={scheduleForm.startTime}
              onChange={(event) => onChange({ ...scheduleForm, startTime: event.target.value })}
            />
            <Input
              label="Fin"
              type="time"
              value={scheduleForm.endTime}
              onChange={(event) => onChange({ ...scheduleForm, endTime: event.target.value })}
            />
          </div>
          <Button loading={saving} type="submit" fullWidth>
            Guardar horario
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {professional.locations.map((location) => (
                  <StatusBadge
                    ...
                    tone={location.active ? 'info' : 'neutral'}
                  />
                ))}
              </div>
            </div>
  ...
  return (
    <Card className="overflow-hidden p-0">
      <TableHeader
        ...
        description="La agenda debe validar sede, profesional, dia y rango horario antes de confirmar."
      />
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-[var(--color-border)] text-sm">
          <thead className="bg-slate-50 text-left text-xs uppercase tracking-[0.16em] text-slate-500">
          ...
          <tbody className="divide-y divide-[var(--color-border)]">
            {schedules.map((schedule) => (
              <tr key={schedule.id}>
                <td className="px-4 py-3 font-semibold">{schedule.professionalName}</td>
                <td className="px-4 py-3">{schedule.locationName}</td>
...
function PermissionsPanel({
  ...
  access: UserLocationAccessResponse[]
  onToggle: (access: UserLocationAccessResponse) => void
  updating: boolean
}) {
  return (
    <Card className="overflow-hidden p-0">
      <TableHeader
        title="Usuarios y permisos por sede"
        description="Controla acceso operativo para evitar que un usuario vea sedes ajenas."
      />
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-[var(--color-border)] text-sm">
          <thead className="bg-slate-50 text-left text-xs uppercase tracking-[0.16em] text-slate-500">
          ...
          <tbody className="divide-y divide-[var(--color-border)]">
            {access.map((item) => (
              <tr key={`${item.userId}-${item.locationId}`}>
                <td className="px-4 py-3 font-semibold">
                  {item.userName}
                  <p className="text-xs font-normal text-slate-500">{item.email}</p>
                </td>
                <td className="px-4 py-3">{item.locationName}</td>
                <td className="px-4 py-3">{item.roleScope}</td>
                <td className="px-4 py-3">
                  <BooleanLabel value={item.canViewConversations} />
                </td>
                <td className="px-4 py-3">
...
function ChannelsPanel({
  ...
  channels: MultisiteChannelResponse[]
  locations: MultisiteLocationSummaryResponse[]
  onSave: (channel: MultisiteChannelResponse) => void
  updating: boolean
}) {
  ...
  return (
    <Card className="overflow-hidden p-0">
      <TableHeader
        title="WhatsApp por sede"
        description="Permite canal centralizado o canal especifico por sucursal."
      />
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-[var(--color-border)] text-sm">
          <thead className="bg-slate-50 text-left text-xs uppercase tracking-[0.16em] text-slate-500">
            ...
            {channels.map((channel) => {
              ...
              return (
                <tr key={channel.channelId}>
                  <td className="px-4 py-3 font-semibold">
                    {channel.channelType}
                    <p className="text-xs font-normal text-slate-500">{channel.providerName}</p>
                  </td>
                  <td className="px-4 py-3">
                    <StatusBadge
                  ...
                  <td className="px-4 py-3">
                    <select
                      className="h-10 rounded-[12px] border border-[var(--color-border)] px-3 text-sm"
                      value={selected}
                      onChange={(event) =>
                        setDrafts((current) => ({
                          ...current,
                    ...
                    >
                      <option value="">Canal centralizado</option>
                      {locations.map((location) => (
                        <option key={location.locationId} value={location.locationId}>
                          {location.locationName}
                        </option>
                  ...
                  <td className="px-4 py-3">
                    <Button
                      disabled={updating}
                      size="sm"
                      onClick={() => onSave({ ...channel, locationId: selected || null })}
                    >
                      Guardar
                    </Button>
                  </td>
