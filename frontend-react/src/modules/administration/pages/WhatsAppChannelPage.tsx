...
import {
  connectWhatsAppChannelRequest,
  disconnectWhatsAppChannelRequest,
  getWhatsAppChannelStatusRequest,
  sendWhatsAppChannelTestMessageRequest,
...
function toProviderLabel(provider: string | null | undefined) {
  switch (provider) {
    case 'META_CLOUD_API':
...
export function WhatsAppChannelPage() {
  ...
  const whatsAppChannelStatusQuery = useQuery({
    queryKey: ['administration', 'whatsapp-channel', 'status'],
    queryFn: getWhatsAppChannelStatusRequest,
  ...
  const connectMutation = useMutation({
    mutationFn: connectWhatsAppChannelRequest,
    onSuccess: async () => {
      await invalidateStatus()
      showToast({
        title: 'Conexion solicitada',
        description: 'El canal WhatsApp respondio correctamente a la solicitud de conexion.',
        tone: 'success',
      })
    },
    onError: () => {
      showToast({
        ...
        description: 'Revisa la configuracion del proveedor activo y reintenta.',
        tone: 'error',
      })
    },
  })
  ...
  const disconnectMutation = useMutation({
    mutationFn: disconnectWhatsAppChannelRequest,
    onSuccess: async () => {
      await invalidateStatus()
      showToast({
        title: 'Canal desconectado',
        description: 'El canal WhatsApp quedo desconectado correctamente.',
        tone: 'success',
      })
    },
    onError: () => {
      showToast({
        title: 'No pudimos desconectar el canal',
  ...
  const testMessageMutation = useMutation({
    mutationFn: sendWhatsAppChannelTestMessageRequest,
    ...
    onError: () => {
      showToast({
        ...
        description: 'Revisa el telefono de destino o el estado del canal.',
        tone: 'error',
      })
    },
  })
  ...
  const status = whatsAppChannelStatusQuery.data
  const isSyncing =
    connectMutation.isPending || disconnectMutation.isPending || testMessageMutation.isPending
  const isSimulated = status?.provider === 'SIMULATED'
  ...
  return (
    <section className="space-y-6">
      ...
      {status ? (
        ...
        >
          <div className="flex flex-wrap items-center gap-3">
            <StatusBadge
              label={toProviderLabel(status.provider)}
              tone={isSimulated ? 'warning' : 'info'}
            />
            <StatusBadge
              label={toStatusLabel(isSyncing ? 'SYNCING' : status.connectionStatus)}
              tone={isSyncing ? 'info' : getStatusTone(status.connectionStatus)}
            />
          </div>
          <p className="mt-4 text-sm leading-7 text-slate-700">{status.message}</p>
        </Card>
      ) : null}

      {whatsAppChannelStatusQuery.isPending && !status ? (
        <LoadingState
          message="Consultando el estado del canal de WhatsApp y los ultimos eventos registrados."
          variant="page"
        />
      ) : null}

      {whatsAppChannelStatusQuery.isError && !status ? (
        <ErrorState
          description="No pudimos consultar el estado del canal. Reintenta para volver a sincronizarlo."
          onRetry={() => void whatsAppChannelStatusQuery.refetch()}
          title="No fue posible cargar el canal de WhatsApp"
        />
      ) : null}
          ...
          <Card className="space-y-5">
            ...
            <form className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]" onSubmit={onSubmit}>
              ...
              <Textarea
                ...
                hint="Solo texto. No se envian archivos ni multimedia."
                label="Mensaje de prueba"
                placeholder="Escribe un mensaje corto para validar el canal."
                {...register('body')}
              />
      ...
      <ConfirmDialog
        ...
        description="Esta accion desconectara el canal WhatsApp del proveedor activo. Podras conectarlo nuevamente cuando lo necesites."
        ...
        title="¿Desconectar el canal de WhatsApp?"
        tone="danger"
      />
    </section>
  )
