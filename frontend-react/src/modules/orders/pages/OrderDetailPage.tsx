...
export function OrderDetailPage() {
  ...
  if (orderQuery.isPending)
    return <LoadingState message="Cargando detalle del pedido." variant="detail" />
  if (orderQuery.isError || !orderQuery.data)
    return (
      <ErrorState
        title="No fue posible cargar el pedido"
        description="Verifica que el pedido exista y que el backend este disponible."
      />
    )
  ...
  return (
    <section className="space-y-6">
      <PageHeader
        actions={
          <>
            <Link className={buttonClassName()} to={`/orders/${order.id}/payments/new`}>
              Registrar pago
            </Link>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <Card className="space-y-5">
          ...
          <pre className="whitespace-pre-wrap rounded-[18px] bg-slate-50 p-4 text-sm leading-6 text-slate-700">
          ...
          {sendMutation.isSuccess ? (
            <p className="text-sm font-semibold text-emerald-700">
              Resumen enviado o encolado correctamente.
            </p>
          ) : null}
          {sendMutation.isError ? (
            <p className="text-sm font-semibold text-red-700">No fue posible enviar el resumen.</p>
          ) : null}
          <div className="grid gap-2">
            {orderStatusActions.map((status) => (
              <Button
                ...
                variant="secondary"
              >
                Cambiar a {formatEstadoPedido(status)}
              </Button>
            ))}
          </div>
        </Card>
