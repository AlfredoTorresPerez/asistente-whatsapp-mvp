...
function isFinalStatus(status: string) {
  return ['CANCELLED', 'CANCELADA', 'COMPLETED', 'ATTENDED', 'ATENDIDA', 'NO_SHOW', 'EXPIRADA', 'EXPIRED', 'RELEASED', 'LIBERADA'].includes(
...
export function BookingConfirmationPage() {
  ...
  const cancelMutation = useMutation({
    mutationFn: () => cancelPublicBookingFromConfirmationRequest(token ?? '', { reason: cancelReason.trim() }),
    onSuccess: async () => {
      setCancelReason('')
      setActionMode('none')
      await confirmationQuery.refetch()
  ...
  const paymentPending = Boolean(data?.requiresDeposit) && !['PAID', 'APPROVED'].includes(normalizeStatus(data?.paymentStatus))
  ...
  const expired = ['EXPIRADA', 'EXPIRED', 'RELEASED', 'LIBERADA'].includes(bookingStatus) || ['EXPIRED', 'INVALIDATED'].includes(linkStatus)
  ...
  const canConfirm = Boolean(data) && !confirmed && !closed && !expired && !paymentPending && !confirmMutation.isPending
      ...
      <section className="mx-auto max-w-2xl space-y-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Revisa los datos de tu cita. Desde esta pagina tambien puedes cancelar o reprogramar si la politica de anticipacion lo permite.
          </p>
        </div>

        {confirmationQuery.isPending ? (
          <Card className="text-center"><p className="text-sm font-medium text-slate-700">Cargando datos de la reserva...</p></Card>
          ...
          <Card className="border-rose-200 bg-rose-50 text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-rose-700">El enlace no existe, ya expiro o fue invalidado. Solicita un nuevo horario por WhatsApp.</p>
          </Card>
        ) : (
          <Card className="space-y-6">
            <div className="rounded-2xl bg-teal-50 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-teal-700">Estado</p>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Info label="Sucursal" value={data.locationName ?? data.location ?? 'Sucursal por confirmar'} />
              <Info label="Profesional" value={data.professionalName ?? 'Por asignar'} />
              <Info label="Cabina" value={data.roomName ?? 'No requerida'} />
              <Info
                label="Abono"
                value={data.requiresDeposit ? `$${Number(data.depositAmount ?? 0).toLocaleString('es-CL')} · ${data.paymentStatus}` : 'No requerido'}
              />
            ...
            {changeWindowClosed && !closed ? (
              <p className="rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800">
                La cancelacion y la reprogramacion desde enlace publico quedan bloqueadas con menos de 24 horas de anticipacion.
              </p>
            ) : null}

            <div className="flex flex-col gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:justify-end">
              <Button disabled={!canManage || actionMode === 'cancel'} onClick={() => setActionMode(actionMode === 'reschedule' ? 'none' : 'reschedule')} variant="secondary">
                Reprogramar reserva
              </Button>
              <Button disabled={!canManage || actionMode === 'reschedule'} onClick={() => setActionMode(actionMode === 'cancel' ? 'none' : 'cancel')} variant="danger">
                Cancelar reserva
              </Button>
              <Button disabled={!canConfirm} onClick={() => confirmMutation.mutate()}>
                {confirmMutation.isPending ? 'Confirmando...' : confirmed ? 'Reserva confirmada' : paymentPending ? 'Pago pendiente' : 'Confirmar reserva'}
              </Button>
            </div>
            ...
            {actionMode === 'cancel' ? (
              <div className="space-y-4 rounded-2xl border border-rose-100 bg-rose-50 p-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-rose-700">Indica el motivo. La accion quedara registrada como realizada por el cliente.</p>
                </div>
                <Textarea
                ...
                <div className="flex justify-end">
                  <Button disabled={cancelReason.trim().length < 5 || cancelMutation.isPending} onClick={() => cancelMutation.mutate()} variant="danger">
            ...
            {actionMode === 'reschedule' ? (
              <div className="space-y-4 rounded-2xl border border-blue-100 bg-blue-50 p-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-blue-700">Selecciona una nueva fecha. Se mantiene el mismo servicio y la misma sucursal.</p>
                </div>
                <Input label="Nueva fecha" onChange={(event) => setRescheduleDate(event.target.value)} type="date" value={rescheduleDate} />
                <Textarea
                ...
                <div className="rounded-2xl border border-blue-100 bg-white p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <Button loading={availabilityQuery.isFetching} onClick={() => availabilityQuery.refetch()} type="button" variant="secondary">
                      Actualizar
                    </Button>
                  </div>
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    {availabilityQuery.isPending ? (
                      ...
                      <p className="rounded-xl bg-rose-50 px-4 py-3 text-sm text-rose-700">
                        {getErrorMessage(availabilityQuery.error, 'No fue posible consultar horarios disponibles.')}
                      </p>
                    ) : availableSlots.length === 0 ? (
                      <p className="text-sm text-slate-500">No hay horarios disponibles para la fecha seleccionada.</p>
                    ) : (
                      availableSlots.map((slot) => (
                        <button
                          ...
                          type="button"
                        >
                          <strong className="text-slate-950">{formatTime(slot.startsAt)} - {formatTime(slot.endsAt)}</strong>
