...
function VisualTable({ caption, columns, rows }: VisualTableProps) {
  return (
    <Card className="p-0">
      ...
      <DataTableShell
        caption="Listado alineado al contrato visual con filtros, estados y accion contextual por fila."
        columns={columns}
        rows={rows}
      />
    </Card>
  ...
  return (
    <Card className="space-y-6">
      ...
      <div className="rounded-[20px] border border-blue-100 bg-blue-50/70 p-4 text-sm leading-6 text-blue-900">
        Los campos estan organizados como panel blanco, con bordes suaves, jerarquia de etiquetas y
        acciones al pie para mantener la misma lectura visual del contrato.
      </div>
...
function Timeline({ items }: { items: Array<{ title: string; detail: string; tone: BadgeTone }> }) {
  return (
    <div className="space-y-3">
      {items.map((item) => (
        <div
          className="rounded-[18px] border border-[var(--color-border)] bg-white p-4"
          key={item.title}
        >
          <div className="flex items-start justify-between gap-3">
            <div>
...
export function ConversationOrderCreatePage() {
  return (
    <ModuleLayout
      actions={[
        {
          label: 'Volver a conversacion',
          to: '/conversations/demo-conversation',
          variant: 'secondary',
        },
      ]}
...
const orderRows: DataTableShellRow[] = [
  {
    id: 'order-1',
    cells: [
      '#PED-00006 · Sofia Rojas',
      <StatusBadge key="s1" label="Pago parcial" tone="warning" />,
      <Money key="m1" value="$54.980" />,
    ],
  ...
  },
  {
    id: 'order-2',
    cells: [
      '#PED-00007 · Carlos Mendez',
      <StatusBadge key="s2" label="Pagado" tone="success" />,
      <Money key="m2" value="$89.990" />,
    ],
    href: '/orders/demo-order',
  },
  {
    id: 'order-3',
    cells: [
      '#PED-00008 · Ana Torres',
      <StatusBadge key="s3" label="Pendiente" tone="info" />,
      <Money key="m3" value="$29.990" />,
    ],
    href: '/orders/demo-order',
  },
]
...
export function OrdersPage() {
  return (
    <ModuleLayout
      actions={[
        { label: 'Crear pedido', to: '/orders/new' },
        { label: 'Exportar', to: '/reports', variant: 'secondary' },
      ]}
      aside={
        <PanelCard title="Resumen de cobros">
          <Timeline
            items={[
              {
                title: 'Pago recibido',
                detail: 'Transferencia confirmada para #PED-00007.',
                tone: 'success',
              },
              {
                title: 'Saldo pendiente',
                detail: 'Sofia Rojas mantiene saldo por $18.000.',
                tone: 'warning',
              },
              {
                title: 'Despacho coordinado',
                detail: 'Pedido preparado para retiro en sucursal.',
                tone: 'info',
              },
            ]}
          />
...
export function OrderDetailPage() {
  return (
    <ModuleLayout
      actions={[
        { label: 'Registrar pago', to: '/orders/demo-order/payments/new' },
        { label: 'Volver a pedidos', to: '/orders', variant: 'secondary' },
      ]}
      aside={
        <PanelCard title="Acciones rapidas">
          <div className="grid gap-3">
            <Link
              ...
              to="/orders/demo-order/payments/new"
            >
              Registrar pago
            </Link>
...
const catalogRows: DataTableShellRow[] = [
  {
    ...
    cells: [
      ...
      <StatusBadge key="c1" label="Activa" tone="success" />,
      <Money key="cm1" value="$34.990" />,
    ],
    href: '/catalog/products/demo-product/edit',
  },
  {
    id: 'p2',
    cells: [
      'Masaje relajante',
      <StatusBadge key="c2" label="Activa" tone="success" />,
      <Money key="cm2" value="$29.990" />,
    ],
    href: '/catalog/products/demo-product/edit',
  },
  {
    ...
    cells: [
      ...
      <StatusBadge key="c3" label="Pausado" tone="warning" />,
      <Money key="cm3" value="$44.990" />,
    ],
    href: '/catalog/products/demo-product/edit',
  },
]
...
export function CatalogPage() {
  return (
    <ModuleLayout
      actions={[
        { label: 'Crear producto', to: '/catalog/products/new' },
        { label: 'Importar catalogo', to: '/catalog', variant: 'secondary' },
      ]}
      aside={
            ...
            items={[
              {
                ...
                detail: '12 productos activos con disponibilidad inmediata.',
                tone: 'success',
              },
              {
                title: 'Corporales',
                detail: '5 servicios requieren revisar stock de insumos.',
                tone: 'warning',
              },
    ...
    >
      <VisualTable
        ...
        columns={['Producto o servicio', 'Estado', 'Precio']}
        rows={catalogRows}
      />
    </ModuleLayout>
  )
...
export function CatalogCreatePage() {
  return (
    <ModuleLayout
      actions={[{ label: 'Volver al catalogo', to: '/catalog', variant: 'secondary' }]}
      description="Alta de producto o servicio con precio, categoria, inventario y visibilidad."
      eyebrow="Catalogo"
      title="Crear producto"
    >
      <FormShell
        cancelTo="/catalog"
        fields={['Nombre', 'Categoria', 'Precio', 'Duracion', 'Stock disponible', 'Estado']}
        saveLabel="Guardar producto"
        title="Informacion del catalogo"
      />
    </ModuleLayout>
  )
}
...
export function CatalogEditPage() {
  return (
    <ModuleLayout
      actions={[{ label: 'Volver al catalogo', to: '/catalog', variant: 'secondary' }]}
      description="Actualiza precio, categoria, disponibilidad y estado del producto."
      eyebrow="Catalogo"
      title="Editar producto"
    >
      <FormShell
        cancelTo="/catalog"
        fields={['Nombre', 'SKU', 'Categoria', 'Precio', 'Stock minimo', 'Estado']}
...
const ruleRows: DataTableShellRow[] = [
  {
    id: 'r1',
    cells: [
      'Seguimiento palabra precio',
      <StatusBadge key="r1" label="Activa" tone="success" />,
      'KEYWORD_MATCH',
    ],
    href: '/automation-rules/demo-rule/edit',
  },
  {
    id: 'r2',
    cells: [
      'Recordatorio de cita',
      <StatusBadge key="r2" label="Activa" tone="success" />,
  ...
  },
  {
    ...
    cells: [
      'Fuera de horario',
      <StatusBadge key="r3" label="Pausada" tone="warning" />,
      'SCHEDULE_WINDOW',
    ],
    href: '/automation-rules/demo-rule/edit',
  },
  ...
  return (
            ...
            items={[
              {
                ...
                detail: 'Respuesta fuera de horario ejecutada correctamente.',
                tone: 'success',
              },
              {
                title: 'Condicion validada',
                detail: 'Keyword precio detectada en conversacion.',
                tone: 'info',
              },
              {
                title: 'Requiere revision',
                detail: 'Plantilla sin variable obligatoria.',
                tone: 'warning',
              },
            ]}
          />
    ...
    >
      <VisualTable
        ...
        columns={['Regla', 'Estado', 'Disparador']}
        rows={ruleRows}
      />
    </ModuleLayout>
  )
...
export function AutomationRuleCreatePage() {
  return (
    <ModuleLayout
      actions={[{ label: 'Volver a reglas', to: '/automation-rules', variant: 'secondary' }]}
      description="Configura nombre, disparador, condiciones y acciones de la regla."
      eyebrow="Reglas"
      title="Crear regla"
    >
      <FormShell
        cancelTo="/automation-rules"
        fields={[
          'Nombre de la regla',
          'Disparador',
          'Condicion principal',
        ...
        saveLabel="Guardar regla"
        title="Asistente de creacion"
      />
    </ModuleLayout>
  )
}
...
export function AutomationRuleEditPage() {
  return (
    <ModuleLayout
      actions={[
        { label: 'Probar regla', to: '/automation-rules/demo-rule/test' },
        { label: 'Volver a reglas', to: '/automation-rules', variant: 'secondary' },
      ]}
      description="Edita condiciones, acciones y plantillas asociadas a la automatizacion."
      eyebrow="Reglas"
      title="Editar regla"
    >
      <FormShell
        cancelTo="/automation-rules"
        fields={['Nombre', 'Estado', 'Condicion', 'Accion', 'Respuesta', 'Limites']}
        saveLabel="Guardar cambios"
        title="Seguimiento palabra precio"
      />
    </ModuleLayout>
  )
}
...
export function AutomationRuleRunPage() {
  return (
    <ModuleLayout
      actions={[{ label: 'Volver a reglas', to: '/automation-rules', variant: 'secondary' }]}
      description="Simula entrada de cliente y revisa la accion resultante antes de activar cambios."
      eyebrow="Reglas"
      title="Probar regla"
    >
      <Card className="space-y-5">
        <FieldGrid
          fields={['Mensaje de entrada', 'Cliente de prueba', 'Canal', 'Resultado esperado']}
        />
        <PanelCard title="Resultado de simulacion">
          <StatusBadge label="Coincidencia detectada" tone="success" />
          <p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">
            La condicion se cumple y la plantilla sera enviada segun la ventana horaria configurada.
          </p>
        </PanelCard>
      </Card>
    </ModuleLayout>
...
const userRows: DataTableShellRow[] = [
  {
    ...
    cells: [
      'Ana Admin',
      <StatusBadge key="u1" label="OWNER" tone="info" />,
      <StatusBadge key="u1s" label="Activo" tone="success" />,
    ],
    href: '/admin/users/demo-user/edit',
  },
  {
    id: 'u2',
    cells: [
      'Carla Mendez',
      <StatusBadge key="u2" label="AGENT" tone="neutral" />,
      <StatusBadge key="u2s" label="Activo" tone="success" />,
    ],
    href: '/admin/users/demo-user/edit',
  },
  {
    ...
    cells: [
      'Pablo Rivera',
      <StatusBadge key="u3" label="SUPERVISOR" tone="info" />,
      <StatusBadge key="u3s" label="Invitado" tone="warning" />,
    ],
    href: '/admin/users/demo-user/edit',
  },
]
  ...
  return (
    <ModuleLayout
      actions={[
        ...
        { label: 'Volver a administracion', to: '/admin', variant: 'secondary' },
      ]}
      description="Usuarios, roles, estados de acceso y acciones de administracion."
...
export function AdminUserCreatePage() {
  return (
    <ModuleLayout
      actions={[{ label: 'Volver a usuarios', to: '/admin/users', variant: 'secondary' }]}
      description="Alta de usuario con rol y permisos base."
      eyebrow="Administracion"
      title="Crear usuario"
    >
      <FormShell
        cancelTo="/admin/users"
        fields={['Nombre', 'Correo', 'Telefono', 'Rol', 'Estado', 'Sucursal']}
...
export function AdminUserEditPage() {
  return (
    <ModuleLayout
      actions={[{ label: 'Volver a usuarios', to: '/admin/users', variant: 'secondary' }]}
      description="Actualiza datos personales, rol y estado de acceso."
      eyebrow="Administracion"
      title="Editar usuario"
    >
      <FormShell
        cancelTo="/admin/users"
        fields={['Nombre', 'Correo', 'Telefono', 'Rol', 'Estado', 'Permisos']}
        saveLabel="Guardar cambios"
        title="Carla Mendez"
      />
    </ModuleLayout>
          ...
          <Timeline
            items={[
              { title: 'Sesion actual', detail: 'Chrome en Windows · Santiago.', tone: 'success' },
              {
                ...
                detail: 'Android autorizado hace 2 dias.',
                tone: 'info',
              },
              {
                title: 'Acceso antiguo',
                detail: 'Sesion expirada automaticamente.',
                tone: 'neutral',
              },
            ]}
          />
