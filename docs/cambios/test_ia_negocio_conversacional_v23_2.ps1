...
function Repair-TextEncoding([string]$Value) {
    ...
    if ($Value -match "Ã|Â|â") {
        try {
            return [System.Text.Encoding]::UTF8.GetString([System.Text.Encoding]::GetEncoding("ISO-8859-1").GetBytes($Value))
        } catch {
            return $Value
        }
    }
...
Write-Host ""
Write-Host "Ejecutando test funcional V23.2 contra: $analyzeUrl"
...
$outDir = Join-Path (Get-Location) "resultados-test-ia-v23-2"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$csvPath = Join-Path $outDir "resultado_test_ia_negocio_v23_2_$timestamp.csv"
$jsonPath = Join-Path $outDir "resultado_test_ia_negocio_v23_2_$timestamp.json"
$htmlPath = Join-Path $outDir "resultado_test_ia_negocio_v23_2_$timestamp.html"
...
<head>
<meta charset="utf-8" />
<title>Resultado test IA negocio V23.2</title>
<style>
...
<body>
<h1>Resultado test IA negocio V23.2</h1>
