...
Definir el contrato de integracion entre el backend Java y `openwa-service` para Fase 1.

## Regla critica

- No usar WhatsApp Business Platform en Fase 1.
- `openwa-service` se usa solo como adaptador experimental para demos, validacion temprana y pilotos controlados.
- El backend Java nunca depende del runtime de OpenWA dentro del dominio. Depende solo de interfaces internas.
...
- recibir webhooks firmados desde `openwa-service`.
...
5. El adaptador HTTP del backend llama a `openwa-service`.
6. `openwa-service` devuelve `acceptedAt`, `messageId` y `status`.
7. El backend actualiza el mensaje y el outbox.
8. Los cambios de entrega posteriores llegan por webhook firmado.

## Flujo de mensaje entrante

1. `openwa-service` recibe un mensaje desde WhatsApp.
2. `openwa-service` firma el evento con HMAC.
3. `openwa-service` hace `POST` al webhook del backend.
...
## Flujo de sesion OpenWA

1. El frontend llama al backend, nunca al servicio Node.
2. El backend llama a `openwa-service` para pedir estado, QR, conexion o desconexion.
3. `openwa-service` responde con estado tecnico.
...
## Endpoints esperados en openwa-service
...
## Endpoints esperados en backend para OpenWA

### Frontend -> backend

| Metodo | Ruta | Uso |
| --- | --- | --- |
| `GET` | `/api/v1/openwa/status` | Estado consumible por UI. |
| `POST` | `/api/v1/openwa/connect` | Solicita conexion. |
| `POST` | `/api/v1/openwa/refresh-qr` | Solicita nuevo QR. |
| `POST` | `/api/v1/openwa/disconnect` | Solicita desconexion. |

### openwa-service -> backend

| Metodo | Ruta | Uso |
| --- | --- | --- |
| `POST` | `/api/v1/integrations/openwa/webhook` | Evento firmado de sesion o mensaje. |

## Seguridad entre backend y openwa-service

### Backend -> openwa-service
...
- El backend debe tolerar que `openwa-service` no este disponible.
...
- Sin automatizaciones ejecutadas dentro de `openwa-service`.
...
sequenceDiagram
    ...
    participant OWA as openwa-service
    ...
    OWA->>API: POST /api/v1/integrations/openwa/webhook
...
- Si en el futuro se reemplaza OpenWA, el dominio no cambia.
